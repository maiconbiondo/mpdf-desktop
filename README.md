...
- **OCR / PDF pesquisável:** reconhecimento de texto de PDFs digitalizados e
  geração de PDF pesquisável, totalmente offline (nenhum documento sai da
  máquina).
- **Exportar Word/Excel:** exportação do PDF para Word (`.docx`) e para Excel
  (`.xlsx`).
...
- **Aparência e manutenção:** tema claro e escuro, e atualização silenciosa
  (o app se mantém atualizado sozinho, sem interromper o trabalho).
...
## Abrir PDFs do navegador no mPDF

O instalador já registra o mPDF como uma opção de leitor de PDF do Windows
(aparece em **Abrir com** e em **Aplicativos padrão**). Para que PDFs vindos do
navegador abram no mPDF:

1. **Definir o mPDF como padrão** — em Configurações (⚙) do mPDF, clique em
   *Definir mPDF como leitor padrão…* (abre os Aplicativos padrão do Windows) e
   escolha o mPDF para o tipo `.pdf`. O Windows 11 não permite que um app se
   auto-defina como padrão; a escolha é sempre do usuário.
2. **Ajustar o navegador** — em Chrome/Edge, ligue *Baixar PDFs em vez de
   abri-los* (Configurações → Downloads, ou a opção de PDF do navegador). Assim,
   ao abrir um PDF baixado, ele vai para o mPDF em vez do visualizador interno
   do navegador.
