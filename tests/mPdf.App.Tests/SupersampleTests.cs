...
public class SupersampleTests
{
    ...
    [Fact] // default (nenhuma View setou nada ainda) — comportamento de hoje: fator 1.0.
    ...
    private static void RunSupersampleScenario()
    {
        ...
        try
        {
            doc = new DocumentViewModel(DocumentSession.Open(Path.Combine(Fixtures.Root, "fixture-a4.pdf")));
    ...
    /// Task 2 (Plano 13) — STA ponta a ponta pela pilha REAL de UI (não injeção direta em
    /// `DocumentViewModel.SupersampleFactor` como o teste acima, herdado da Task 1): abre uma página
    /// REALIZADA numa `PdfViewerControl`, liga "Nitidez extra" através de um `SobreViewModel` de verdade
    /// (mesmo `applySupersampleFactor` que `MainViewModel.Sobre` injeta em produção), e prova que o
    /// BITMAP JÁ NA TELA é re-renderizado mais denso — sem isto, o toggle só "faria efeito" no próximo
    /// evento que já disparasse render por outro motivo (zoom, DPI), mesma classe de bug real que
    /// `DocumentViewModel.OnSupersampleFactorChanged` (Task 1) foi criado para evitar.
    [Fact]
    public void SobreViewModel_ToggleNitidezExtra_RerendersRealizedPageAtFactorTwo()
    {
    ...
    private static void RunToggleRerenderScenario()
    {
        ...
        try
        {
            ...
            // mesmo callback que MainViewModel.Sobre() monta em produção: aplica o fator a TODO
