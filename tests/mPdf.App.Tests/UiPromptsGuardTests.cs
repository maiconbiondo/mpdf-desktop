...
file sealed class SpyPdfADialogService(PdfAEscolha? result) : IPdfADialogService
{
    public int CallCount { get; private set; }
    public PdfAEscolha? Perguntar() { CallCount++; return result; }
}
...
public class UiPromptsGuardTests
{
    ...
    public void ModuleInitializer_SwappedAllSeamMembers_BeforeThisTestRan()
    {
        ...
        // Task 7 (SDD "Salvar como PDF/A"):
        AssertFactorySwapped<ThrowingPdfADialogService>(() => UiPrompts.CreatePdfADialog(), nameof(UiPrompts.CreatePdfADialog));
    }
    ...
    [Fact] // Task 7 (SDD "Salvar como PDF/A"): pdfADialog OMITIDO -> SalvarComoPdfACommand alcança o
    // diálogo de produção via UiPrompts.CreatePdfADialog, trocado pelo guard pra uma versão que LANÇA.
    public async Task DocumentViewModel_PdfADialogOmitted_SalvarComoPdfACommand_ThrowsViaUiPrompts()
    {
        AssertFactorySwapped<ThrowingPdfADialogService>(() => UiPrompts.CreatePdfADialog(), nameof(UiPrompts.CreatePdfADialog));

        using var doc = new DocumentViewModel(DocumentSession.Open(A4Path)); // pdfADialog OMITIDO

        var ex = await Record.ExceptionAsync(() => doc.SalvarComoPdfACommand.ExecuteAsync(null));

        var ioe = Assert.IsType<InvalidOperationException>(ex);
        Assert.Contains(nameof(UiPrompts.CreatePdfADialog), ioe.Message);
    }

    [Fact] // controle negativo (pdfADialog): fake benigno local devolvendo `null` (cancelado) -> não
    // lança, diálogo "mostrado" 1x, nenhuma conversão iniciada.
    public async Task DocumentViewModel_PdfADialogOmitted_NegativeControl_WithBenignFactory_DoesNotThrow()
    {
        var original = UiPrompts.CreatePdfADialog;
        try
        {
            var spy = new SpyPdfADialogService(null);
            UiPrompts.CreatePdfADialog = () => spy;
            using var doc = new DocumentViewModel(DocumentSession.Open(A4Path));
        ...
        finally { UiPrompts.CreatePdfADialog = original; }
    }
