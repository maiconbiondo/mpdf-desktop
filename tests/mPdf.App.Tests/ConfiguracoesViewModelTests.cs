...
public class ConfiguracoesViewModelTests
{
    ...
    private static ConfiguracoesViewModel BuildVm(
        ...
        Action? abrirDefaultApps = null) => new(
        ...
        abrirDefaultApps ?? (() => { })); // default de teste: nunca abre Configurações do Windows de verdade
    ...
    // Plano 23: os testes do toggle de tema saíram daqui — o tema virou um botão sol/lua na barra
    // superior (MainViewModel.ToggleTema, testado em MainViewModelTests).

    // ---- Plano 23: leitor padrão de PDF (abrir Aplicativos padrão do Windows) --------------------------

    [Fact] // "Definir mPDF como leitor padrão" invoca o callback (produção abre ms-settings:defaultapps).
    public void DefinirLeitorPadrao_InvokesCallback()
    {
        var (_, config) = TempConfig();
        int chamadas = 0;
        var vm = BuildVm(config: config, abrirDefaultApps: () => chamadas++);

        vm.DefinirLeitorPadraoCommand.Execute(null);

        Assert.Equal(1, chamadas);
    }

    // ---- posição do menu de anotação (MenuAnotacaoNaBarraLateral) --------------------------------------
