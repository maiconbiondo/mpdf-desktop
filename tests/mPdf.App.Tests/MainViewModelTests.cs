...
public class MainViewModelTests : IDisposable
{
    ...
    // ---- Plano 23: toggle de tema na barra superior (movido de Configurações) ----------------------

    [Fact] // Escuro é o padrão de fábrica -> ícone SOL ("ir pro claro"); alternar persiste Claro, troca o
    // ícone pra LUA e volta. (AplicarNoApp é guardado por Application.Current null em teste headless.)
    public void ToggleTema_FlipsPersistsAndUpdatesIcon()
    {
        var vm = VmFull();
        Assert.True(vm.TemaEscuro); // default de fábrica = escuro
        Assert.Equal(mPdf.App.Icons.Ph.Glyph("sun"), vm.TemaGlifo);

        vm.ToggleTemaCommand.Execute(null);

        Assert.False(vm.TemaEscuro);
        Assert.Equal(mPdf.App.Icons.Ph.Glyph("moon"), vm.TemaGlifo);
        Assert.Equal(mPdf.Documents.ThemeMode.Claro, new AppConfig(Path.Combine(_dir, "config")).ThemeMode);

        vm.ToggleTemaCommand.Execute(null); // volta pro escuro
        Assert.True(vm.TemaEscuro);
        Assert.Equal(mPdf.Documents.ThemeMode.Escuro, new AppConfig(Path.Combine(_dir, "config")).ThemeMode);
    }
