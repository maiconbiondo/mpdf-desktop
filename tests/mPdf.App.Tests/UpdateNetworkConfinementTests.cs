...
///      `ConfiguracoesViewModel.VerificarAtualizacao` (o comando "Verificar atualização", MIGRADO de
///      `SobreViewModel` na Task 2 do Plano 17), o ÚNICO ponto que constrói o serviço; o comando de
///      download subsequente REUSA essa mesma instância via campo,
...
public class UpdateNetworkConfinementTests
{
    ...
    // ---- guarda 1c (Plano 16): mPdf.Export — OFFLINE TOTAL, sem arquivo autorizado --------------------

    // Mesmo raciocínio da guarda 1b (mPdf.Ocr): o assembly novo mPdf.Export (escritor DOCX/XLSX via
    // DocumentFormat.OpenXml) precisa ser VARRIDO também, senão escapa da confinação de rede só por
    // não estar sob src/mPdf.App. mPdf.Export lê texto NEUTRO (ExportPage/ExportChar) já extraído
    // pelo App e escreve bytes localmente com uma lib Office Open XML (MIT) — nenhum arquivo é
    // autorizado a tocar rede. Prova de disparo (task-1-report.md): plantar
    // `new System.Net.Http.HttpClient()` em DocxExporter.cs -> NoNetworkTokenInExportModule reprova
    // nomeando arquivo:linha; restaurado.
    private static string ExportSrcRoot => Path.Combine(RepoRoot, "src", "mPdf.Export");

    private static IEnumerable<string> ExportScannableFiles() =>
        Directory.EnumerateFiles(ExportSrcRoot, "*.cs", SearchOption.AllDirectories).Where(f => !IsBuildArtifact(f));

    [Fact]
    public void ExportScanIsNotEmpty() // piso de varredura vazia do assembly novo — capability vs coverage
    {
        int count = ExportScannableFiles().Count();
        Assert.True(count >= 3, $"varredura de mPdf.Export vazia ou quase vazia ({count} arquivo(s)) = guarda morta");
    }

    [Fact]
    public void NoNetworkTokenInExportModule()
    {
        var offenders = new List<string>();

        foreach (var file in ExportScannableFiles())
        {
            foreach (var (lineNumber, line) in File.ReadLines(file).Select((l, i) => (i + 1, l)))
            {
                string trimmed = line.TrimStart();
        ...
        Assert.True(offenders.Count == 0,
            "OFFLINE (Plano 16): mPdf.Export NÃO pode tocar rede — escreve .docx/.xlsx localmente com " +
            "libs Office Open XML MIT, nenhum arquivo é autorizado. Token de rede do BCL (" +
            string.Join("/", NetworkTokens) + ") encontrado em mPdf.Export:\n" + string.Join("\n", offenders));
    }
    ...
    public void UpdateServiceConstruction_OccursInExactlyOnePlace()
    {
        ...
        Assert.True(totalOccurrences == 2,
            "UpdateService deve ser construido em EXATAMENTE 2 sitios deliberados (ConfiguracoesViewModel." +
            "VerificarAtualizacao E SilentUpdateRunner) — ambos pontos auditaveis de acesso a rede; " +
            $"aliases: {string.Join(", ", aliasNames)} — encontrado {totalOccurrences}x:\n" + string.Join("\n", offenders));
        Assert.Contains(offenders, o => o.Contains("ConfiguracoesViewModel.cs"));
        Assert.Contains(offenders, o => o.Contains("SilentUpdateRunner.cs"));
    }
