...
public class StampBoxPlacementTests
{
    ...
    // ---- EndStampDraw: mínimo 60x20pt -- abaixo disso solta uma caixa PADRÃO no ponto clicado --------

    [Fact] // Plano 21 (fix dogfood): clique/arrasto pequeno demais -> caixa de tamanho PADRÃO (180x60) no
    // ponto clicado, já em Adjusting (antes ficava em Drawing com aviso). O usuário move/redimensiona/confirma.
    public void EndStampDraw_BelowMinimum_CreatesDefaultBox_EntersAdjusting()
    {
        var (doc, _) = BuildForStampBox();
        using var d = doc;
        d.ActiveTool = AnnotationTool.SignatureStamp;
        ...
        Assert.Equal(60, d.StampBoxRect.TopPt - d.StampBoxRect.BottomPt, 0.01);
        Assert.Equal(100, d.StampBoxRect.LeftPt, 0.01); // canto superior-esquerdo na âncora do clique
        Assert.Equal(100, d.StampBoxRect.TopPt, 0.01);
    }
    ...
    [Fact] // Plano 21: a caixa PADRÃO nascida de um clique é totalmente ajustável (mover), como qualquer
    // outra — o clique não é um beco sem saída, é o começo do ajuste.
    public void EndStampDraw_DefaultBoxFromClick_CanBeMoved()
    {
        var (doc, _) = BuildForStampBox();
        using var d = doc;
        d.ActiveTool = AnnotationTool.SignatureStamp;
        ...
        d.MoveBoxBy(new PdfPoint(20, 0));

        Assert.Equal(leftAntes + 20, d.StampBoxRect.LeftPt, 0.01);
    }
