...
public class ShapeEditBoxTests
{
    ...
    private static AnnotationData MakeRect(string id = "caixa-1", uint? colorArgb = DocumentViewModel.ColorAmarelo,
        uint? fillArgb = null, double? strokeWidthPt = null) =>
        new()
        {
            Id = id, Kind = AnnotationKind.Rectangle, PageIndex = 0,
            LeftPt = 100, BottomPt = 100, RightPt = 300, TopPt = 250,
            ColorArgb = colorArgb, FillArgb = fillArgb, StrokeWidthPt = strokeWidthPt, Author = "Autor de Teste",
        };
    ...
    // ---- Task D (SDD desenho-caixa-revisao): controles de cor/espessura/preenchimento na EDIÇÃO --------

    [Fact] // BeginShapeEditBox captura o estilo CORRENTE da caixa nas 3 propriedades de edição, e
    // EmEdicaoDeForma fica true (só nesse propósito+fase — ver doc XML da propriedade no VM).
    public void BeginShapeEditBox_InitializesEdicaoStyleProps_AndEmEdicaoDeForma()
    {
        var (doc, _, _) = Build();
        using var d = doc;
        var rect = MakeRect(colorArgb: DocumentViewModel.ColorAmarelo, fillArgb: DocumentViewModel.ColorVerde, strokeWidthPt: 3.0);

        Assert.False(d.EmEdicaoDeForma); // fora de qualquer edição, começa false
        d.SelectedAnnotation = rect;
        d.BeginShapeEditBox(rect);

        Assert.True(d.EmEdicaoDeForma);
        Assert.Equal(DocumentViewModel.ColorAmarelo, d.EdicaoCorArgb);
        Assert.Equal(DocumentViewModel.ColorVerde, d.EdicaoFillArgb);
        Assert.Equal(3.0, d.EdicaoEspessuraPt);
    }

    [Fact] // Mudar cor/fill/espessura via os comandos Edicao* (sem lift na hora) + Confirmar -> 1 Remove +
    // 1 Add com o rect NOVO **e** o estilo NOVO (não o estilo original da anotação).
    public async Task ConfirmShapeEditBox_WithChangedStyle_LiftsWithNewRectAndNewStyle()
    {
        var (doc, fake, errors) = Build();
        using var d = doc;
        var rect = MakeRect(colorArgb: DocumentViewModel.ColorAmarelo, fillArgb: null, strokeWidthPt: null);
        d.SelectedAnnotation = rect;
        d.BeginShapeEditBox(rect);

        d.EdicaoContornoAzulCommand.Execute(null);
        d.EdicaoPreenchVerdeCommand.Execute(null);
        d.EdicaoEspessuraGrossaCommand.Execute(null);
        d.MoveBoxBy(new PdfPoint(10, 5)); // também muda a geometria, mesmo gesto dos outros testes
        ...
        Assert.Equal(DocumentViewModel.ColorVerde, lifted.FillArgb);
        Assert.Equal(DocumentViewModel.EspessuraGrossa, lifted.StrokeWidthPt);
        Assert.Equal(110, lifted.LeftPt, 0.5);  // rect moveu junto (mesmo Aplicar único)
        Assert.Equal(105, lifted.BottomPt, 0.5);
    }

    [Fact] // Cancelar descarta ESTILO e GEOMETRIA — nenhum Remove/Add, e EmEdicaoDeForma volta a false.
    public void Cancel_DiscardsStyleChanges_AndEmEdicaoDeFormaFalse()
    {
        var (doc, fake, _) = Build();
        using var d = doc;
        var rect = MakeRect(colorArgb: DocumentViewModel.ColorAmarelo, fillArgb: null, strokeWidthPt: null);
        ...
        d.BeginShapeEditBox(rect);
        d.EdicaoContornoAzulCommand.Execute(null);
        d.EdicaoEspessuraFinaCommand.Execute(null);

        d.CancelStampBox();

        Assert.False(d.EmEdicaoDeForma);
        Assert.Equal(0, fake.RemoveAnnotationCallCount);
        Assert.Equal(0, fake.AddAnnotationCallCount);
    }

    [Fact] // "Sem cor" no contorno durante a edição -> ColorArgb null gravado no Aplicar (mesma regra da
    // Task A: sem cor de contorno = sem borda no motor).
    public async Task ConfirmShapeEditBox_ContornoNenhum_WritesNullColorArgb()
    {
        var (doc, fake, errors) = Build();
        using var d = doc;
        var rect = MakeRect(colorArgb: DocumentViewModel.ColorAmarelo, fillArgb: DocumentViewModel.ColorVerde);
        ...
        d.EdicaoContornoNenhumCommand.Execute(null);
        ...
        var lifted = fake.LastAnnotation!;
        Assert.Null(lifted.ColorArgb);
        Assert.Equal(DocumentViewModel.ColorVerde, lifted.FillArgb); // preenchimento intocado, preservado
    }
}
