...
internal sealed class FakePdfEditor : IPdfEditor
{
    ...
    // Plano 25: imposição em PDF — nenhum teste deste fake a exercita; devolve um PDF válido qualquer.
    public byte[] ImporEmFolhas(byte[] pdf, IReadOnlyList<ColocacaoFolha> colocacoes, double folhaLarguraPt, double folhaAlturaPt)
        => Fixtures.A4();
    ...
    // Task 2 (SDD "Salvar como PDF/A"): stub mínimo pra este fake satisfazer a interface — registra a
    // última chamada pra asserção na Task 6 (mesmo espírito de ApplyOcrTextLayerInputs acima).
    public IReadOnlyList<PdfAPaginaImagem>? UltimoPdfAPaginas;
    public PdfANivel? UltimoPdfANivel;
    public byte[]? UltimoPdfAFonte;
    public byte[] CriarPdfADeImagens(
        IReadOnlyList<PdfAPaginaImagem> paginas, PdfANivel nivel, PdfAMetadados metadados,
        byte[]? fonteInvisivelTtf)
    {
        UltimoPdfAPaginas = paginas;
        UltimoPdfANivel = nivel;
        UltimoPdfAFonte = fonteInvisivelTtf;
        return mPdf.App.Tests.Fixtures.A4(); // bytes de PDF válidos quaisquer
    }
}
...
public class DocumentViewModelTests
{
    ...
    // ==== Task 2 (SDD desenho-caixa-melhorias): cor de preenchimento na UI ===========================
    //
    // SelectedFillColorArgb (padrão de nova caixa) + SelectFill* (comandos da toolbar) + inheritance em
    // CommitDrawingAsync (só Rectangle) + "aplicar à selecionada" (lift, mesmo pipeline de
    // MoveSelectedAnnotationAsync/EditSelectedAnnotation acima).

    [Fact] // default null (sem preenchimento — preserva o comportamento de toda caixa desenhada ANTES
    // desta task); os 4 comandos trocam o padrão sem exigir seleção ativa nem CanEdit (mesmo espírito de
    // SelectColorCommands_ChangeSelectedMarkupColorArgb acima — escolher a cor não é uma edição em si).
    public async Task SelectFillCommands_ChangeSelectedFillColorArgb_NoSelectionNeeded()
    {
        var (doc, fake, _, _) = BuildForAnnotations();
        using var d = doc;
        Assert.Null(d.SelectedFillColorArgb); // default

        await d.SelectFillVerdeCommand.ExecuteAsync(null);
        Assert.Equal(DocumentViewModel.ColorVerde, d.SelectedFillColorArgb);

        await d.SelectFillVermelhoCommand.ExecuteAsync(null);
        Assert.Equal(DocumentViewModel.ColorVermelho, d.SelectedFillColorArgb);

        await d.SelectFillAmareloCommand.ExecuteAsync(null);
        Assert.Equal(DocumentViewModel.ColorAmarelo, d.SelectedFillColorArgb);

        await d.SelectFillNenhumCommand.ExecuteAsync(null);
        Assert.Null(d.SelectedFillColorArgb);

        Assert.Equal(0, fake.AddAnnotationCallCount); // sem seleção -> nenhum lift tentado em nenhum dos 4
    }

    [Fact] // CommitDrawingAsync de um Rectangle carrega o FillArgb SELECIONADO no momento do commit.
    public async Task CommitDrawingAsync_RectangleTool_InheritsSelectedFillColorArgb()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        // Fix v2.12.2: fill escolhido DEPOIS de ativar o Retângulo (o ativar preseta branco).
        d.ActiveTool = AnnotationTool.Rectangle;
        await d.SelectFillVerdeCommand.ExecuteAsync(null);

        await d.CommitDrawingAsync(0, new[] { new PdfPoint(50, 100), new PdfPoint(200, 250) });

        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.ColorVerde, fake.LastAnnotation!.FillArgb);
    }

    [Fact] // Fix v2.12.2: a caixa "vem com padrão" ao ATIVAR o Retângulo — SEM borda (ColorArgb null) +
    // preenchimento BRANCO (FillArgb branco). Sem escolher nada depois, o desenho sai assim.
    public async Task CommitDrawingAsync_RectangleTool_DefaultNoBorderWhiteFill()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        d.ActiveTool = AnnotationTool.Rectangle;

        Assert.Null(d.SelectedMarkupColorArgb);                       // preset: sem borda
        Assert.Equal(DocumentViewModel.ColorBranco, d.SelectedFillColorArgb); // preset: branco

        await d.CommitDrawingAsync(0, new[] { new PdfPoint(50, 100), new PdfPoint(200, 250) });

        Assert.Empty(errors);
        Assert.Null(fake.LastAnnotation!.ColorArgb);
        Assert.Equal(DocumentViewModel.ColorBranco, fake.LastAnnotation!.FillArgb);
    }

    [Theory] // Ink/Line/Arrow NUNCA carregam FillArgb, mesmo com uma cor de preenchimento selecionada —
    // só Rectangle tem essa noção (Task 1: FillArgb só é lido pelo motor pra esse Kind).
    [InlineData(AnnotationTool.Ink)]
    [InlineData(AnnotationTool.Line)]
    [InlineData(AnnotationTool.Arrow)]
    public async Task CommitDrawingAsync_NonRectangleTool_FillArgbAlwaysNull_EvenWithFillSelected(AnnotationTool tool)
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        await d.SelectFillVermelhoCommand.ExecuteAsync(null);
        ...
        await d.CommitDrawingAsync(0, new[] { new PdfPoint(10, 10), new PdfPoint(100, 150) });

        Assert.Empty(errors);
        Assert.Null(fake.LastAnnotation!.FillArgb);
    }

    [Fact] // "aplicar à selecionada" (decisão de produto do brief): com uma Rectangle SELECIONADA,
    // SelectFillVermelho faz lift (Remove+Add do MESMO Id) trocando só o FillArgb — ColorArgb/rect
    // preservados, mesmo pipeline de MoveSelectedAnnotationAsync/EditSelectedAnnotation acima.
    public async Task SelectFillCommand_WithRectangleSelected_LiftsAndAppliesFillToThatBox()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData
        {
            Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0,
            LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30,
            ColorArgb = DocumentViewModel.ColorAmarelo, FillArgb = null,
        };
        fake.ReadAnnotationsResult = new[] { original };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectAnnotationAt(0, 20, 20);
        ...
        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.ColorVermelho, d.SelectedFillColorArgb); // padrão TAMBÉM mudou
        Assert.Equal(1, fake.RemoveAnnotationCallCount);
        Assert.Equal("caixa-1", fake.LastRemovedId);
        var lifted = fake.LastAnnotation!;
        Assert.Equal("caixa-1", lifted.Id);
        ...
        Assert.Equal(10, lifted.LeftPt, 0.01); Assert.Equal(30, lifted.RightPt, 0.01); // rect preservado
    }

    [Fact] // SEM seleção: SelectFill* só muda o padrão — nenhum lift tentado.
    public async Task SelectFillCommand_NoSelection_OnlyChangesDefault_NoLift()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
    ...
    [Fact] // documento ASSINADO (CanEdit=false): padrão ainda muda (não é uma edição do PDF em si), mas
    // NENHUM lift é tentado na caixa selecionada.
    public async Task SelectFillCommand_SignedDocument_ChangesDefault_NoLift()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData { Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0, LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30 };
        fake.ReadAnnotationsResult = new[] { original };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectAnnotationAt(0, 20, 20);
        Assert.NotNull(d.SelectedAnnotation);
        ...
        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.ColorVerde, d.SelectedFillColorArgb);
        Assert.Equal(0, fake.RemoveAnnotationCallCount); // nunca tentou o lift
    }

    [Fact] // página GIRADA: mesmo gate silencioso de MoveSelectedAnnotationAsync/CommitDrawingAsync —
    // aqui SEM aviso (_notifyError) porque o comando sempre teve efeito (mudou o padrão), diferente dos
    // 4 pontos de ESCRITA que avisam "Página girada" por não terem feito nada de jeito nenhum.
    // `HitTestAnnotation`/`SelectAnnotationAt` já são gateados por rotação (nunca devolvem uma anotação
    // de página girada — ver doc XML de IsPageRotated) — este teste seta `SelectedAnnotation` DIRETO
    // (setter público, mesmo cenário M1 documentado em MoveSelectedAnnotationAsync) pra alcançar o guard
    // de dentro de SelectFillAsync mesmo assim.
    public async Task SelectFillCommand_RotatedPage_ChangesDefault_NoLift_NoNotice()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        fake.ReadAnnotationsResult = Array.Empty<AnnotationData>();
        fake.PageRotationsResult = new[] { 90 };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectedAnnotation = new AnnotationData { Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0, LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30 };

        await d.SelectFillVerdeCommand.ExecuteAsync(null);

        Assert.Equal(DocumentViewModel.ColorVerde, d.SelectedFillColorArgb);
        Assert.Equal(0, fake.RemoveAnnotationCallCount);
        Assert.Empty(errors); // no-op silencioso — ver doc XML de SelectFillAsync
    }

    [Fact] // seleção existe mas NÃO é Rectangle (ex.: StickyNote) -> no-op, mesmo espírito do filtro de
    // EditSelectedAnnotationCommand acima (Task 1: FillArgb só tem efeito em Rectangle).
    public async Task SelectFillCommand_NonRectangleSelected_NoLift()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData { Id = "nota-1", Kind = AnnotationKind.StickyNote, PageIndex = 0, LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30 };
    ...
    // ==== Task B (SDD desenho-caixa-revisao): paleta ampliada + contorno "sem cor" + espessura =========
    //
    // SelectedMarkupColorArgb (uint?, "sem cor" nova opção) + paleta de 7 cores + SelectedStrokeWidthPt
    // (double?) + SelectColor*/SelectEspessura* aplicando à forma SELECIONADA via lift — mesmo funil de
    // SelectFillAsync acima, só que o allowlist de kinds liftáveis é as 4 FORMAS (Rectangle/Line/Arrow/
    // Ink), não só Rectangle (ColorArgb/StrokeWidthPt existem nas 4; FillArgb só em Rectangle).

    [Fact] // valores ARGB exatos da paleta nova (brief Step 1) — 0xFF de alfa (opaco), mesmo formato dos
    // 3 originais (ColorAmarelo/Verde/Vermelho, já cobertos em SelectColorCommands_ChangeSelectedMarkupColorArgb).
    public void NovasConstantesDeCor_TemOsValoresArgbDoBrief()
    {
        Assert.Equal(0xFF000000u, DocumentViewModel.ColorPreto);
        Assert.Equal(0xFFFFFFFFu, DocumentViewModel.ColorBranco);
        Assert.Equal(0xFFFF8800u, DocumentViewModel.ColorLaranja);
        Assert.Equal(0xFF1E6FE0u, DocumentViewModel.ColorAzul);
    }

    [Fact] // constantes de espessura (brief Step 3): fina=1pt, média=2pt (mesmo valor do default do
    // motor, PdfEditor.StrokeWidthPt), grossa=4pt.
    public void NovasConstantesDeEspessura_TemOsValoresDoBrief()
    {
        Assert.Equal(1.0, DocumentViewModel.EspessuraFina);
        Assert.Equal(2.0, DocumentViewModel.EspessuraMedia);
        Assert.Equal(4.0, DocumentViewModel.EspessuraGrossa);
    }

    [Fact] // SelectColorNenhum (botão novo "Sem cor") -> SelectedMarkupColorArgb vira null — sem seleção
    // ativa, só o padrão muda (mesmo espírito de SelectFillCommands_ChangeSelectedFillColorArgb_NoSelectionNeeded).
    public async Task SelectColorNenhumCommand_SetsSelectedMarkupColorArgbToNull_NoSelectionNeeded()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        Assert.Equal(DocumentViewModel.ColorAmarelo, d.SelectedMarkupColorArgb); // default visível preservado

        await d.SelectColorNenhumCommand.ExecuteAsync(null);

        Assert.Empty(errors);
        Assert.Null(d.SelectedMarkupColorArgb);
        Assert.Equal(0, fake.AddAnnotationCallCount); // sem seleção -> nenhum lift tentado
    }

    [Fact] // SelectColorAzul (cor nova da paleta) -> SelectedMarkupColorArgb vira azul.
    public async Task SelectColorAzulCommand_SetsSelectedMarkupColorArgbToAzul()
    {
        var (doc, _, _, errors) = BuildForAnnotations();
        using var d = doc;

        await d.SelectColorAzulCommand.ExecuteAsync(null);

        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.ColorAzul, d.SelectedMarkupColorArgb);
    }

    [Fact] // SelectEspessuraGrossa -> SelectedStrokeWidthPt vira 4pt (default null preservado antes).
    public async Task SelectEspessuraGrossaCommand_SetsSelectedStrokeWidthPtTo4()
    {
        var (doc, _, _, errors) = BuildForAnnotations();
        using var d = doc;
        Assert.Null(d.SelectedStrokeWidthPt); // default

        await d.SelectEspessuraGrossaCommand.ExecuteAsync(null);

        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.EspessuraGrossa, d.SelectedStrokeWidthPt);
    }

    [Fact] // CommitDrawingAsync de um Rectangle carrega o ColorArgb (incl. null, "sem cor") E o
    // StrokeWidthPt SELECIONADOS no momento do commit — mesmo espírito de
    // CommitDrawingAsync_RectangleTool_InheritsSelectedFillColorArgb (Task 2) só que pros 2 campos novos.
    public async Task CommitDrawingAsync_RectangleTool_InheritsSelectedColorAndStrokeWidth()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        // Fix v2.12.2: ativar o Retângulo PRESETA o padrão da caixa (sem borda + branco); a cor/espessura
        // escolhidas DEPOIS de ativar a ferramenta é que são herdadas pelo desenho.
        d.ActiveTool = AnnotationTool.Rectangle;
        await d.SelectColorAzulCommand.ExecuteAsync(null);
        await d.SelectEspessuraGrossaCommand.ExecuteAsync(null);
        ...
        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.ColorAzul, fake.LastAnnotation!.ColorArgb);
        Assert.Equal(DocumentViewModel.EspessuraGrossa, fake.LastAnnotation!.StrokeWidthPt);
    }

    [Fact] // "sem cor" (SelectColorNenhum) propaga null pro ColorArgb do Rectangle commitado — regra da
    // Task A (ColorArgb==null -> PdfEditor grava largura de borda 0) começa aqui, na origem do valor.
    public async Task CommitDrawingAsync_RectangleTool_NoColorSelected_ColorArgbNull()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        await d.SelectColorNenhumCommand.ExecuteAsync(null);
    ...
    [Fact] // "aplicar à selecionada" (mesma decisão de produto de SelectFillAsync): com uma Rectangle
    // SELECIONADA, SelectColorAzul faz lift (Remove+Add do MESMO Id) trocando só o ColorArgb — resto
    // preservado (StrokeWidthPt/geometria/FillArgb).
    public async Task SelectColorCommand_WithRectangleSelected_LiftsAndAppliesColorToThatBox()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData
        {
            Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0,
            LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30,
            ColorArgb = DocumentViewModel.ColorAmarelo, StrokeWidthPt = DocumentViewModel.EspessuraFina,
        };
        fake.ReadAnnotationsResult = new[] { original };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectAnnotationAt(0, 20, 20);
        ...
        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.ColorAzul, d.SelectedMarkupColorArgb); // padrão TAMBÉM mudou
        Assert.Equal(1, fake.RemoveAnnotationCallCount);
        Assert.Equal("caixa-1", fake.LastRemovedId);
        var lifted = fake.LastAnnotation!;
        Assert.Equal("caixa-1", lifted.Id);
        Assert.Equal(DocumentViewModel.ColorAzul, lifted.ColorArgb);
        Assert.Equal(DocumentViewModel.EspessuraFina, lifted.StrokeWidthPt); // espessura preservada
        Assert.Equal(10, lifted.LeftPt, 0.01); Assert.Equal(30, lifted.RightPt, 0.01); // rect preservado
    }

    [Fact] // "sem cor" (SelectColorNenhum) numa Rectangle SELECIONADA faz lift trocando ColorArgb pra
    // null (regra da Task A: sem borda).
    public async Task SelectColorNenhumCommand_WithRectangleSelected_LiftsColorArgbToNull()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData
        {
            Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0,
            LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30,
            ColorArgb = DocumentViewModel.ColorAmarelo,
        };
        fake.ReadAnnotationsResult = new[] { original };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectAnnotationAt(0, 20, 20);
    ...
    [Fact] // "aplicar à selecionada" pra espessura: com uma Rectangle SELECIONADA, SelectEspessuraGrossa
    // faz lift trocando só o StrokeWidthPt — ColorArgb/geometria preservados.
    public async Task SelectEspessuraCommand_WithRectangleSelected_LiftsAndAppliesWidthToThatBox()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData
        {
            Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0,
            LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30,
            ColorArgb = DocumentViewModel.ColorVerde,
        };
        fake.ReadAnnotationsResult = new[] { original };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectAnnotationAt(0, 20, 20);
        ...
        Assert.Empty(errors);
        Assert.Equal(DocumentViewModel.EspessuraGrossa, d.SelectedStrokeWidthPt); // padrão TAMBÉM mudou
        Assert.Equal(1, fake.RemoveAnnotationCallCount);
        var lifted = fake.LastAnnotation!;
        Assert.Equal(DocumentViewModel.EspessuraGrossa, lifted.StrokeWidthPt);
        Assert.Equal(DocumentViewModel.ColorVerde, lifted.ColorArgb); // cor preservada
    }

    [Theory] // Line/Arrow/Ink também são liftáveis por SelectColor*/SelectEspessura* (diferente de
    // SelectFillAsync, que só liga pra Rectangle) — as 4 formas têm ColorArgb/StrokeWidthPt.
    [InlineData(AnnotationKind.Line)]
    [InlineData(AnnotationKind.Arrow)]
    [InlineData(AnnotationKind.Ink)]
    public async Task SelectColorCommand_WithNonRectangleShapeSelected_StillLifts(AnnotationKind kind)
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData { Id = "forma-1", Kind = kind, PageIndex = 0, LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30 };
        fake.ReadAnnotationsResult = new[] { original };
        await d.RefreshAnnotationsByPageAsync();
        d.SelectAnnotationAt(0, 20, 20);
        Assert.NotNull(d.SelectedAnnotation);
    ...
    [Fact] // SEM seleção: SelectColor*/SelectEspessura* só mudam o padrão — nenhum lift tentado.
    public async Task SelectColorAndEspessuraCommands_NoSelection_OnlyChangeDefault_NoLift()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
    ...
    [Fact] // documento ASSINADO (CanEdit=false): padrão ainda muda, mas nenhum lift é tentado — mesmo
    // gate de SelectFillCommand_SignedDocument_ChangesDefault_NoLift.
    public async Task SelectColorCommand_SignedDocument_ChangesDefault_NoLift()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData { Id = "caixa-1", Kind = AnnotationKind.Rectangle, PageIndex = 0, LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30 };
    ...
    [Fact] // página GIRADA: mesmo gate silencioso de SelectFillCommand_RotatedPage_ChangesDefault_NoLift_NoNotice
    // — padrão muda, nenhum lift, SEM aviso (_notifyError).
    public async Task SelectEspessuraCommand_RotatedPage_ChangesDefault_NoLift_NoNotice()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        fake.ReadAnnotationsResult = Array.Empty<AnnotationData>();
        ...
        Assert.Equal(DocumentViewModel.EspessuraGrossa, d.SelectedStrokeWidthPt);
        Assert.Equal(0, fake.RemoveAnnotationCallCount);
        Assert.Empty(errors); // no-op silencioso
    }

    [Fact] // seleção existe mas NÃO é forma (StickyNote) -> no-op, mesmo espírito de
    // SelectFillCommand_NonRectangleSelected_NoLift.
    public async Task SelectColorCommand_NonShapeSelected_NoLift()
    {
        var (doc, fake, _, errors) = BuildForAnnotations();
        using var d = doc;
        var original = new AnnotationData { Id = "nota-1", Kind = AnnotationKind.StickyNote, PageIndex = 0, LeftPt = 10, BottomPt = 10, RightPt = 30, TopPt = 30 };
