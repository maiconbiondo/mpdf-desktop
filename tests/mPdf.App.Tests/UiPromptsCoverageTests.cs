...
public class UiPromptsCoverageTests
{
    ...
    private static readonly Dictionary<(string Type, string Param), string> KnownOptionalServiceParams = new()
    {
        ...
        // Task 3 (Plano 16): diálogo "Exportar como Word/Excel" -- mesma disciplina de injeção via
        // UiPrompts. `documentHasText` (Func<byte[], bool>, namespace System) NÃO entra nesta varredura
        // (mesma isenção estrutural de `rasterizerFactory`/`Func<IUpdateSource>`: não mora em
        // mPdf.App.Services e não mostra UI — só leitura de texto).
        [("DocumentViewModel", "exportDocumentDialog")] = "UiPrompts.CreateExportDocumentDialog",
        // Task 6/7 (SDD "Salvar como PDF/A"): diálogo "Salvar como PDF/A" -- mesma disciplina de injeção
        // via UiPrompts. A Task 7 ligou a view real (`PdfADialogService`/`Views.PdfADialog`) -- mesma
        // classe de risco de `CreateExportImageDialog`, coberta em UiPromptsTestGuard/UiPromptsGuardTests
        // igual aos demais diálogos reais desta seam. `pickPdfToSave` (Func<string,string?>) e
        // `writeAllBytes` (Action<string,byte[]>) NÃO entram nesta varredura -- mesma isenção estrutural
        // de `rasterizerFactory`/`documentHasText` (tipo Func/Action de namespace System com aridade !=
        // Action<string>, não mora em mPdf.App.Services).
        [("DocumentViewModel", "pdfADialog")] = "UiPrompts.CreatePdfADialog",
        ...
        // reflexão abaixo só varre parâmetros cujo PRÓPRIO tipo mora em mPdf.App.Services (Namespace de
        // Func<IUpdateSource> é "System", não "mPdf.App.Services" — a forma Func é DELIBERADA, ver doc
        // XML de ConfiguracoesViewModel: precisa ser deferida até o clique em "Verificar atualização",
        // não resolvida no momento da construção do VM/abertura do diálogo, diferente de todo outro
        // default `?? UiPrompts.CreateXxx()` desta classe). Cobertura equivalente vem de
        // `UiPromptsGuardTests` (fire/negative-control) + `ConfiguracoesViewModelTests`/
        // `UpdateNetworkConfinementTests` (prova comportamental/textual de "rede só por clique").
        [("ConfiguracoesViewModel", "confirmInstall")] = "UiPrompts.CreateConfirmInstallUpdate",
        ...
        // Plano 22: galeria de rubricas — mesmo exemplar de StampGallery (só filesystem, nenhum ShowDialog).
        [("DocumentViewModel", "rubricas")] = "ISENTO -- RubricaGallery não mostra UI (só filesystem)",
    };
