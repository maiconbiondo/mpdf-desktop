...
public class ThemeWiringTests
{
    ...
    public void SobreDialog_Buttons_ResolveDialogStyle()
    {
        RunSta(() =>
        {
            var dialog = new SobreDialog(new mPdf.App.ViewModels.SobreViewModel());
            try
            {
                dialog.Show();
                dialog.UpdateLayout();
    ...
    // Task 2 (Plano 17): MIGRADO de SobreDialog_Buttons_ResolveDialogStyle — o botão "Verificar
    // atualização" agora mora no ConfiguracoesDialog.
    [Fact]
    public void ConfiguracoesDialog_Buttons_ResolveDialogStyle()
    {
        RunSta(() =>
        {
            var vm = new mPdf.App.ViewModels.ConfiguracoesViewModel(
    ...
    public void SignDialog_Buttons_ResolveDialogStyle()
    {
        RunSta(() =>
        {
            var dialog = new SignDialog(Array.Empty<mPdf.Signing.SigningCertificateInfo>(), allowDocMdp: true, hasRubrica: false);
            try
            {
                dialog.Show();
                dialog.UpdateLayout();
