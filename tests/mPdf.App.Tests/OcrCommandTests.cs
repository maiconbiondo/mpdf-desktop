...
internal sealed class FakeOcrRasterizer : IOcrPageRasterizer
{
    ...
    // Task 6 (SDD "Salvar como PDF/A"): tamanho de página FIXO (default A4 em pt, 595x842) -- os testes
    // de orquestração do PDF/A não olham o valor exato, só que ele chega ao editor sem alteração.
    private readonly PdfPageSize _pageSize;
    public int RasterizeCallCount { get; private set; }
    public bool Disposed { get; private set; }

    public FakeOcrRasterizer(bool[] hasText, RenderedPage? rendered = null, PdfPageSize? pageSize = null)
    {
        _hasText = hasText;
        _rendered = rendered ?? new RenderedPage(8, 8, new byte[8 * 8 * 4]);
        _pageSize = pageSize ?? new PdfPageSize(595, 842);
    }
    ...
    public PdfPageSize GetPageSize(int pageIndex) => _pageSize;
