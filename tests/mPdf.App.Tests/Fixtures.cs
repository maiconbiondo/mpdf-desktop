...
{
    // Config ISOLADO num diretório temporário (Plano 17, Task 1) — nunca toca o %AppData%\mPDF\config.json
    // REAL da máquina. Usado por todo teste que constrói `DocumentViewModel` e assere dimensão de render/
    // `SupersampleFactor`/`DpiFactor`: sem isto o ctor cai em `new AppConfig()` sobre o config real, e a
    // máquina do usuário com `NitidezExtra=true` renderiza em 2× (bitmap dobrado, testes vermelhos). Os
    // defaults do config isolado (NitidezExtra=false) mantêm o render no fator 1.0 esperado por esses testes.
    public static AppConfig IsolatedConfig() =>
        new(Path.Combine(Path.GetTempPath(), $"mpdf-test-cfg-{Guid.NewGuid():N}"));
