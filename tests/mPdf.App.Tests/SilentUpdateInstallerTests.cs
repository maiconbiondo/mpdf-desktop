...
public class SilentUpdateInstallerTests
{
    ...
    // ---- (1b) modo "sem reabrir" (/NORELAUNCH) para o update do RMM --------------------------------------

    [Fact] // reabrir default (true, botão da UI): as 3 flags de sempre, SEM /NORELAUNCH
    public void BuildStartInfo_Default_NaoInclui_NoRelaunch()
    {
        var psi = SilentUpdateInstaller.BuildStartInfo(@"C:\Temp\mPDF-Setup-9.9.9.exe");
        Assert.Equal("/VERYSILENT /SUPPRESSMSGBOXES /NORESTART", psi.Arguments);
        Assert.DoesNotContain("/NORELAUNCH", psi.Arguments);
    }

    [Fact] // reabrir:false (update do RMM): acrescenta /NORELAUNCH, mantendo as 3 flags silenciosas
    public void BuildStartInfo_SemReabrir_Inclui_NoRelaunch()
    {
        var psi = SilentUpdateInstaller.BuildStartInfo(@"C:\Temp\mPDF-Setup-9.9.9.exe", reabrir: false);
        Assert.Equal("/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /NORELAUNCH", psi.Arguments);
        Assert.Equal(SilentUpdateInstaller.ParametroNaoReabrir, "/NORELAUNCH");
        Assert.True(psi.UseShellExecute);
    }

    // Guarda: o update do RMM (SilentUpdateRunner) lança o instalador com reabrir:false — sem isto, o
    // /update reabriria o mPDF em toda estação (uma janela pipocando por máquina no update em massa). O
    // botão da UI (MainViewModel) NÃO passa reabrir:false (prova de que só o RMM é sem-reabrir).
    private static string SilentUpdateRunnerPath => Path.Combine(RepoRoot, "src", "mPdf.App", "Services", "SilentUpdateRunner.cs");

    [Fact]
    public void UpdateDoRmm_LancaInstalador_SemReabrir()
    {
        var runner = File.ReadAllText(SilentUpdateRunnerPath);
        Assert.Contains("BuildStartInfo(p, reabrir: false)", runner);

        var mainVm = File.ReadAllText(MainViewModelPath);
        Assert.DoesNotContain("reabrir: false", mainVm); // o botão da UI continua reabrindo
    }

    // ---- (6) o .iss honra /NORELAUNCH: função [Code] DeveReabrir + Check no [Run] de relaunch -----------

    [Fact]
    public void Iss_RelaunchTemCheckDeveReabrir_EFuncaoLeNoRelaunch()
    {
        var runLine = IssRelaunchRunLine();
        Assert.NotNull(runLine);
        Assert.Contains("Check: DeveReabrir", runLine!); // o relaunch é pulado quando DeveReabrir=False

        var issTexto = File.ReadAllText(IssPath);
        Assert.Contains("[Code]", issTexto);
        Assert.Contains("function DeveReabrir", issTexto);
        Assert.Contains("/NORELAUNCH", issTexto); // a função compara ParamStr contra /NORELAUNCH
    }
