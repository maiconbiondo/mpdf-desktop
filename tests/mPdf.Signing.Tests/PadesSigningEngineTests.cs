...
public class PadesSigningEngineTests
{
    ...
    [Fact] // Bug de campo: assinar um PDF/A COM carimbo visível quebrava a conformidade (a fonte do
    // carimbo, Helvetica padrão-14, NÃO era embutida — PDF/A exige TODA fonte embutida). O
    // `PdfPadesSigner` abre um documento PDF/A de entrada como PDF/A e RECUSA gravar a revisão assinada
    // com conteúdo não-conforme (`PdfAConformanceException` -> `PdfSigningException` genérica pro
    // usuário). Oráculo: se `Sign` RETORNA (não lança) sobre uma entrada PDF/A, a revisão assinada é
    // conforme por construção (mesma disciplina de `CriarPdfADeImagens`) — a fonte do carimbo agora é
    // embutida. Também prova que o texto do carimbo continua extraível (ToUnicode do subset embutido).
    public void Sign_PdfA_WithVisibleStamp_StaysConformantAndKeepsAppearanceText()
    {
        var stamp = new VisibleStampSpec(0, new PdfQuad(StampLeft, StampBottom, StampRight, StampTop));
        var signed = SignFixture(stamp: stamp, pdf: Fixtures.PdfA());
    ...
    // 8x8 PNG RGBA em xadrez com metade dos pixels TRANSPARENTES (alfa=0) — caso realista de rubrica
    // (traço sobre fundo transparente). É o pior caso pro PDF/A: o alfa vira um SMask no XObject de
    // imagem, e a imagem RGB depende do OutputIntent sRGB do documento pro DeviceRGB ser permitido.
    private static byte[] RubricaPngTransparent() => Convert.FromBase64String(
        "iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUA" +
        "AAAJcEhZcwAADsMAAA7DAcdvqGQAAAAdSURBVChTY2BgYGDgEjnxHyeNVRCJBgN0wSFnAgAMKTyhBiyh5wAAAABJRU5ErkJggg==");
    ...
    [Fact] // Companheiro de Sign_PdfA_WithVisibleStamp_... pro modo RUBRICA: assinar um PDF/A com uma
    // rubrica de imagem (PNG RGBA com transparência — pior caso: SMask + DeviceRGB dependente do
    // OutputIntent) tem que CONFORMAR (o `PdfPadesSigner` recusaria a revisão não-conforme, como faz
    // com a fonte não-embutida do carimbo padrão) E renderizar a imagem no PDFium. Diferente do carimbo
    // de texto, a rubrica NÃO adiciona fonte, então NUNCA dependeu do fix de fonte embutida — este teste
    // é a GUARDA de que continua PDF/A-safe (empiricamente já era, inclusive no A3A real do usuário; um
    // futuro refactor da aparência da rubrica não pode regredir isso em silêncio).
    public void Sign_PdfA_WithRubrica_StaysConformantAndRendersImage()
    {
        var stamp = new VisibleStampSpec(0, new PdfQuad(100, 100, 280, 160), RubricaPngTransparent());
        var signed = SignFixture(stamp: stamp, pdf: Fixtures.PdfA());

        Assert.Equal("%PDF", System.Text.Encoding.ASCII.GetString(signed, 0, 4));
        Assert.True(AppearanceHasImageXObject(signed), "aparência da rubrica não tem XObject de imagem");

        // Varre o retângulo INTEIRO do carimbo (não um único pixel: a rubrica é xadrez com metade
        // transparente, o centro pode calhar numa célula vazada) — basta 1 pixel com tinta pra provar
        // que a imagem renderizou no PDFium. Retângulo em PX de tela (Y invertido: origem PDF embaixo).
        using var renderer = new PdfDocumentRenderer(signed);
        var page = renderer.RenderPage(0, 1.0);
        int yTop = page.HeightPx - 160, yBot = page.HeightPx - 100;
        bool anyNonWhite = false;
        for (int y = yTop; y < yBot && !anyNonWhite; y++)
            for (int x = 100; x < 280; x++)
            {
                int i = (y * page.WidthPx + x) * 4;
                if (page.Bgra[i] < 250 || page.Bgra[i + 1] < 250 || page.Bgra[i + 2] < 250) { anyNonWhite = true; break; }
            }
        Assert.True(anyNonWhite, "retângulo da rubrica ficou todo branco no PDF/A — imagem não renderizou");
    }
