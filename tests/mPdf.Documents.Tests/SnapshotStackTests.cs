...
public class SnapshotStackTests
{
    ...
    // ---- Plano 19: corrida de concorrência do SpillableStack (Clear/Dispose sem lock) --------------
    //
    // CAUSA-RAIZ: `DocumentViewModel.Dispose` enfileira `session.Dispose()` na fila SERIAL de
    // `PendingDisposals` (thread de background do TPL), enquanto o teste — ou qualquer segundo dono da
    // sessão — também chama `session.Dispose()` na sua PRÓPRIA thread (`using var session`). As duas
    // chegam em `SpillableStack.Clear()`, cujo `foreach (var entry in _entries)` iterava a `List<Entry>`
    // SEM sincronização; a segunda thread mutando `_entries` durante a iteração da primeira disparava
    // "Collection was modified" — o flaky histórico de `FormFlattenIntegrationTests`/
    // `FormFillIncrementalRoutingTests`. `Dispose` é "melhor esforço, nunca lança": este martelo prova
    // que operar a pilha de duas threads (Push/ClearAll/Dispose concorrentes) NUNCA lança.

    [Fact] // MUTATION-PROVÁVEL: reverter o lock de `SpillableStack` (voltar Clear/Push/etc. sem
    // sincronização) reintroduz o throw "Collection was modified" e este teste fica vermelho de novo.
    public void ConcurrentPushAndClear_NeverThrows()
    {
        var failures = new ConcurrentQueue<Exception>();

        // maxInMemory=1 -> quase toda entrada espalha em disco, então o `foreach` do Clear faz IO por
        // entrada (TryDeleteSilently), ALARGANDO a janela em que a outra thread muta `_entries` no meio
        // da iteração -> vermelho CONFIÁVEL no código sem lock.
        for (int round = 0; round < 100 && failures.IsEmpty; round++)
        {
            var dir = NewSpillDir();
            using var stack = new SnapshotStack(1, dir);
            using var start = new ManualResetEventSlim(false);

            var pusher = new Thread(() =>
            {
                start.Wait();
                try { for (int i = 0; i < 300; i++) stack.Push([(byte)i]); }
                catch (Exception ex) { failures.Enqueue(ex); }
            });
            var clearer = new Thread(() =>
            {
                start.Wait();
                try { for (int i = 0; i < 300; i++) stack.ClearAll(); }
                catch (Exception ex) { failures.Enqueue(ex); }
            });
        ...
        Assert.True(failures.IsEmpty,
            $"a pilha lançou sob acesso concorrente (Dispose deve ser melhor-esforço): " +
            $"{(failures.TryPeek(out var e) ? e.ToString() : "")}");
    }

    [Fact] // Duplo-Dispose de DUAS threads (o cenário REAL: fila de PendingDisposals + `using var
    // session` do dono) — a pilha tem entradas espalhadas em disco quando as duas threads entram no
    // Clear ao mesmo tempo. Nunca lança; Dispose é idempotente e melhor-esforço.
    public void ConcurrentDisposeFromTwoThreads_NeverThrows()
    {
        var failures = new ConcurrentQueue<Exception>();
        ...
        {
            var dir = NewSpillDir();
            var stack = new SnapshotStack(1, dir);
            for (int i = 0; i < 50; i++) stack.Push([(byte)i]); // popula (a maioria espalha em disco)
            ...
            var a = new Thread(DisposeIt);
            var b = new Thread(DisposeIt);
            a.Start();
            b.Start();
            start.Set();
            a.Join();
        ...
        Assert.True(failures.IsEmpty,
            $"Dispose concorrente lançou (deve ser idempotente e nunca lançar): " +
            $"{(failures.TryPeek(out var e) ? e.ToString() : "")}");
    }
}
