...
public class PdfEditorTests
{
    ...
    // --- Preenchimento (fill / `/IC`) do Retângulo, Task 1 (Plano desenho-caixa-melhorias) --------

    [Fact] // exemplar AddAnnotation_Rectangle_RoundTrips acima — FillArgb é um campo A MAIS no mesmo
    // round-trip: prova que a cor de PREENCHIMENTO (/IC) sobrevive independente da cor de BORDA (/C).
    public void AddAnnotation_Rectangle_FillArgb_RoundTrips()
    {
        var data = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
            PageIndex = 0,
            LeftPt = 50, BottomPt = 500, RightPt = 250, TopPt = 600,
            ColorArgb = 0xFFFF5555, // borda — continua preservada independente do fill
            FillArgb = 0xFF3366CC,
            Author = "Autor de Teste",
        };
        ...
        Assert.Equal(AnnotationKind.Rectangle, a.Kind);
        Assert.Equal(0xFFFF5555u, a.ColorArgb); // borda intacta
        Assert.Equal(0xFF3366CCu, a.FillArgb);  // preenchimento (mesma cor exata, alfa 0xFF)
    }

    [Fact] // Sentinela (mesmo espírito de ColorArgb/`/C`): FillArgb null -> NENHUM /IC escrito -> lido
    // de volta null, não um "preto" ou qualquer outro palpite.
    public void AddAnnotation_Rectangle_NoFillArgb_ReadsBackNull()
    {
        var data = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
    ...
    [Fact] // FillArgb só se aplica a Kind==Rectangle (Contract.cs) — um Highlight com FillArgb setado
    // NUNCA grava /IC (BuildAnnotation/o bloco de fill em AddAnnotation só olha FillArgb dentro do
    // ramo Rectangle); lido de volta, o campo vem null pros demais kinds.
    public void AddAnnotation_NonRectangleKind_FillArgb_IsIgnored()
    {
        var data = new AnnotationData
        {
            Kind = AnnotationKind.Highlight,
    ...
    [Fact] // Render PDFium (disciplina do projeto: todo write iText conferido contra um motor
    // INDEPENDENTE do iText — mesmo padrão de ImageToPdf_Jpeg_RendersMatchingKnownCornerColors acima).
    // Amostra o CENTRO do bbox (longe do traço/borda de 2pt) num retângulo COM fill vs um retângulo
    // idêntico SEM fill — prova que o /IC de fato pinta o interior, não só aparece no dicionário.
    public void AddAnnotation_Rectangle_FillArgb_RendersFillColorInsideBbox()
    {
        var fillArgb = 0xFF3366CCu;
        var (r, g, b) = ((byte)0x33, (byte)0x66, (byte)0xCC);
        var baseData = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
            PageIndex = 0,
            LeftPt = 50, BottomPt = 500, RightPt = 250, TopPt = 600,
        ...
        var filled = Editor.AddAnnotation(Fixtures.A4(), baseData with { FillArgb = fillArgb });
        var unfilled = Editor.AddAnnotation(Fixtures.A4(), baseData with { FillArgb = null });

        using var rFilled = new PdfDocumentRenderer(filled);
        using var rUnfilled = new PdfDocumentRenderer(unfilled);
        var pageFilled = rFilled.RenderPage(0, 1.0);
        var pageUnfilled = rUnfilled.RenderPage(0, 1.0);

        // Centro do bbox em pt: (150, 550). scale 1.0 = 1px/pt (RenderPage); eixo Y do PDFium é
        // TOPO-esquerda (mesma conversão de GetTextPage acima) -> pixelY = heightPt - ptY.
        double heightPt = rFilled.GetPageSize(0).HeightPt;
        int cx = 150;
        int cy = (int)Math.Round(heightPt - 550);

        AssertCornerColor(pageFilled, cx, cy, (r, g, b), 12);
        // Sem fill: mesmo ponto cai no interior do retângulo (não na borda) -> cor de página (branco).
        AssertCornerColor(pageUnfilled, cx, cy, (255, 255, 255), 12);
    }

    // --- Espessura de borda (StrokeWidthPt), Task (Plano desenho-caixa-revisao) --------------------
    // Espelha o padrão de FillArgb acima: round-trip do valor, sentinela (aqui: ColorArgb==null =
    // largura 0/sem borda) e prova por render (motor INDEPENDENTE do iText que escreveu).

    [Fact] // StrokeWidthPt customizado sobrevive ao round-trip (grava /Border [0 0 W], ReadStrokeWidth
    // lê o 3º elemento de volta).
    public void AddAnnotation_Rectangle_StrokeWidthPt_RoundTrips()
    {
        var data = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
        ...
        Assert.Equal(4.0, a.StrokeWidthPt!.Value, 0.01);
    }

    [Fact] // StrokeWidthPt null na ENTRADA -> grava o DEFAULT legado (2pt, const StrokeWidthPt em
    // PdfEditor) -> lido de volta 2 (o valor REAL gravado no /Border), nunca null — só a AUSÊNCIA de
    // /Border volta null (ver AddAnnotation_NonShapeKind_StrokeWidthPt_IsIgnored abaixo).
    public void AddAnnotation_Rectangle_NoStrokeWidthPt_DefaultsToTwoOnReadBack()
    {
        var data = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
        ...
        Assert.Equal(2.0, a.StrokeWidthPt!.Value, 0.01);
    }

    [Fact] // REGRA "sem borda" (Contract.cs): ColorArgb==null numa forma -> largura 0 -> contorno
    // INVISÍVEL no render PDFium, mesmo com FillArgb pintando o interior até a borda do bbox. Compara
    // com um Rectangle de borda normal (ColorArgb preto) no MESMO ponto da moldura esquerda.
    public void AddAnnotation_Rectangle_NoColorArgb_RendersNoOutlineButFillShows()
    {
        var baseData = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
        ...
        var bordered = Editor.AddAnnotation(Fixtures.A4(), baseData with { ColorArgb = 0xFF000000 });
        var noBorder = Editor.AddAnnotation(Fixtures.A4(), baseData with { ColorArgb = null, FillArgb = 0xFF3366CC });

        using var rBordered = new PdfDocumentRenderer(bordered);
        using var rNoBorder = new PdfDocumentRenderer(noBorder);
        var pageBordered = rBordered.RenderPage(0, 1.0);
        var pageNoBorder = rNoBorder.RenderPage(0, 1.0);

        // x=51: dentro da banda de borda default de 2pt ([50,52]) — longe o bastante do canto pra
        // evitar antialiasing da quina. y=550: meio da altura do bbox (500..600), mesma conversão de
        // eixo (topo-esquerda no PDFium) do teste de fill acima.
        double heightPt = rBordered.GetPageSize(0).HeightPt;
        int cx = 51;
        int cy = (int)Math.Round(heightPt - 550);

        // Borda normal: a moldura é preta (ColorArgb).
        AssertCornerColor(pageBordered, cx, cy, (0, 0, 0), 30);
        // Sem borda (ColorArgb null): o MESMO ponto mostra a cor do PREENCHIMENTO (0x3366CC), nunca
        // preto — o contorno não foi desenhado (largura 0 já basta; não foi preciso pular SetBorder).
        AssertCornerColor(pageNoBorder, cx, cy, (0x33, 0x66, 0xCC), 30);
    }

    [Fact] // Espessura no render: StrokeWidthPt maior -> banda de borda MAIS LARGA (mais pixels da cor
    // da borda numa faixa horizontal perpendicular à moldura esquerda) — prova que StrokeWidthPt afeta
    // o visual desenhado pelo PDFium, não só o valor gravado no /Border.
    public void AddAnnotation_Rectangle_ThickerStrokeWidthPt_RendersWiderBorderBand()
    {
        var baseData = new AnnotationData
        {
            Kind = AnnotationKind.Rectangle,
        ...
        var thick = Editor.AddAnnotation(Fixtures.A4(), baseData with { StrokeWidthPt = 6 });
        var thin = Editor.AddAnnotation(Fixtures.A4(), baseData with { StrokeWidthPt = 1 });

        using var rThick = new PdfDocumentRenderer(thick);
        using var rThin = new PdfDocumentRenderer(thin);
        var pageThick = rThick.RenderPage(0, 1.0);
        var pageThin = rThin.RenderPage(0, 1.0);

        double heightPt = rThick.GetPageSize(0).HeightPt;
        int y = (int)Math.Round(heightPt - 550);

        // Faixa horizontal cobrindo a moldura esquerda (x=50) e um pouco além pra dentro/fora —
        // suficiente pra conter a banda de borda dos dois casos (até 6px de largura).
        int thickCount = CountMatchingPixels(pageThick, y, 45, 65, (0, 0, 0), 30);
        int thinCount = CountMatchingPixels(pageThin, y, 45, 65, (0, 0, 0), 30);

        Assert.True(thickCount > thinCount,
            $"esperava banda de borda mais larga com StrokeWidthPt maior: thick={thickCount}px, thin={thinCount}px");
    }

    [Fact] // StrokeWidthPt só se aplica a formas com contorno (Contract.cs) — um StickyNote com
    // StrokeWidthPt setado NUNCA grava /Border (o bloco de borda em AddAnnotation só olha Kind
    // Ink/Rectangle/Line/Arrow); lido de volta, o campo vem null — nenhum /Border pra ReadStrokeWidth
    // ler, mesmo espírito de FillArgb ignorado em kind != Rectangle.
    public void AddAnnotation_NonShapeKind_StrokeWidthPt_IsIgnored()
    {
        var data = new AnnotationData
        {
            Kind = AnnotationKind.StickyNote,
            PageIndex = 0,
            LeftPt = 50, BottomPt = 500, RightPt = 70, TopPt = 520,
            StrokeWidthPt = 5,
        };
    ...
    /// Conta quantos pixels numa faixa HORIZONTAL (y fixo, x de xStart a xEnd, inclusive) batem com
    /// `expected` dentro da tolerância — usado por `AddAnnotation_Rectangle_ThickerStrokeWidthPt_
    /// RendersWiderBorderBand` pra medir a LARGURA da banda de borda sem depender de 1 único pixel de
    /// amostra (frágil contra antialiasing na borda da banda).
    private static int CountMatchingPixels(RenderedPage page, int y, int xStart, int xEnd, (byte R, byte G, byte B) expected, int tolerance)
    {
        y = Math.Clamp(y, 0, page.HeightPx - 1);
        int count = 0;
        for (int x = Math.Max(0, xStart); x <= Math.Min(page.WidthPx - 1, xEnd); x++)
        {
            int i = (y * page.WidthPx + x) * 4;
            byte b = page.Bgra[i], g = page.Bgra[i + 1], r = page.Bgra[i + 2];
            if (Math.Abs(r - expected.R) <= tolerance && Math.Abs(g - expected.G) <= tolerance && Math.Abs(b - expected.B) <= tolerance)
                count++;
        }
        return count;
    }
