...
public static class Fixtures
{
    ...
    // --- Task 3 (SDD "Salvar como PDF/A"): fixture de imagem para CriarPdfADeImagens ------------
    // 850x1100px, JPEG RGB qualidade 90, fundo claro (245,245,240) com um retangulo escuro
    // (30,30,30) de 160x160px no canto superior-esquerdo — gerada 1x via probe temporario
    // (scratchpad, System.Drawing.Common DIRETO, apagado depois; mesmo protocolo de
    // fixture-foto/fixture-transparente acima). Usada por PdfAEditorTests pra checagem de pixel
    // (fidelidade visual apos compor em PDF/A).
    public static byte[] PaginaSolida() => File.ReadAllBytes(Path.Combine(Root, "pagina-solida.jpg"));

    // --- Task 4 (SDD "Salvar como PDF/A"): fonte embutivel para a camada de texto invisivel -----
    // Copia de src/mPdf.App/Assets/Fonts/Inter-400.ttf (mesmo protocolo de PaginaSolida acima: o
    // teste de mPdf.Editing.Tests nao pode depender de mPdf.App, entao a fonte real usada pelo App
    // eh copiada 1x pra tests/fixtures). Usada por PdfAEditorTests pra CriarPdfADeImagens embutir
    // fonte de verdade (TTF valido) no ramo de texto OCR.
    public static byte[] InterTtf() => File.ReadAllBytes(Path.Combine(Root, "Inter-400.ttf"));
}
