...
internal sealed class PadesSigningEngine : ISigningEngine
{
    public byte[] Sign(SignRequest request)
    {
        ...
        // Falha da CAMADA CRIPTOGRÁFICA ao usar a chave (token A3/smartcard): PIN cancelado/errado,
        // cartão removido no meio, ou o CSP/middleware (ex.: SafeSign) recusando a operação. O
        // `X509Certificate2Signature.Sign` chama `RSACng.SignData` bem no fundo de
        // `SignWithBaselineBProfile`; sem este catch, uma `CryptographicException` crua escaparia a
        // fronteira neutra (e viraria crash na UI). Vira mensagem pt-BR acionável, tipada do módulo.
        catch (System.Security.Cryptography.CryptographicException ex) { throw WrapCrypto(ex); }
        ...
        string? location)
    {
        ...
        // padrão (só o conteúdo do `Draw` muda), inclusive o zeramento dos 4 paddings da aparência — ver
        // doc XML abaixo. A imagem já foi decodificada/validada em `ValidateStamp` (`ImageDataFactory`).
        if (stamp.ImageBytes is { } rubricaBytes)
        {
            var imageData = iText.IO.Image.ImageDataFactory.Create(rubricaBytes);
            var rubricaDiv = new Div().SetWidth(widthPt).SetHeight(heightPt);
            rubricaDiv.SetNextRenderer(new RubricaAppearanceRenderer(rubricaDiv, imageData));

            var rubricaAppearance = new SignatureFieldAppearance(SignerProperties.IGNORED_ID).SetContent(rubricaDiv);
            rubricaAppearance.SetProperty(Property.PADDING_TOP, UnitValue.CreatePointValue(0));
            rubricaAppearance.SetProperty(Property.PADDING_RIGHT, UnitValue.CreatePointValue(0));
        ...
        // PDF/A-safe: fonte EMBUTIDA como subset (nunca a Helvetica padrão-14, que NÃO é embutível e
        // fazia o `PdfPadesSigner` recusar a revisão assinada sobre uma entrada PDF/A —
        // `PdfAConformanceException: All the fonts must be embedded`). Ver LoadEmbeddedStampFont.
        var font = LoadEmbeddedStampFont("Inter-400.ttf");
        var fontBold = LoadEmbeddedStampFont("Inter-700.ttf");
    ...
    /// Carrega uma fonte TrueType embutida no assembly (Assets/Fonts/*.ttf, ver .csproj) como subset
    /// EMBUTIDO no PDF assinado — `FORCE_EMBEDDED` + `IDENTITY_H`, mesma mecânica de
    /// `PdfEditor.CriarPdfADeImagens` (mPdf.Editing). É isto que torna o carimbo visível PDF/A-safe: a
    /// Helvetica padrão-14 usada antes NÃO era embutível, e o `PdfPadesSigner` recusa gravar uma revisão
    /// não-conforme sobre uma entrada PDF/A. Resolve o recurso por SUFIXO (não depende do namespace-raiz
    /// exato) — mesmo padrão de `PdfEditor.ResolveIccResourceName`.
    private static PdfFont LoadEmbeddedStampFont(string fileName)
    {
        var assembly = typeof(PadesSigningEngine).Assembly;
        ...
        return PdfFontFactory.CreateFont(
            buffer.ToArray(), PdfEncodings.IDENTITY_H, PdfFontFactory.EmbeddingStrategy.FORCE_EMBEDDED);
    }
    ...
    /// Plano 21: renderer da RUBRICA — desenha SÓ a imagem (aspect-fit centralizada no bbox), nada de
    /// moldura/selo/texto. Mesmo contrato de `IRenderer`/`DivRenderer`/`GetNextRenderer` de
    /// `StampAppearanceRenderer` (o iText pode reusar o modelo pra relayout — cada `Div` novo precisa de
    /// uma instância nova). O aspect-fit centraliza a imagem sem distorção QUALQUER que seja a proporção
    /// da caixa (o usuário pode desenhar a caixa em qualquer razão — a imagem nunca estica).
    private sealed class RubricaAppearanceRenderer(Div modelDiv, iText.IO.Image.ImageData imageData)
        : DivRenderer(modelDiv)
    {
        public override IRenderer GetNextRenderer() =>
            new RubricaAppearanceRenderer((Div)GetModelElement(), imageData);
        ...
        public override void Draw(DrawContext drawContext)
        {
            ...
            float iw = imageData.GetWidth(), ih = imageData.GetHeight();
            if (iw <= 0 || ih <= 0) return; // imagem degenerada -- nada a desenhar (ValidateStamp já barra as inválidas)
            float scale = Math.Min(bbox.GetWidth() / iw, bbox.GetHeight() / ih);
            float w = iw * scale, h = ih * scale;
            float x = bbox.GetLeft() + (bbox.GetWidth() - w) / 2f;
            float y = bbox.GetBottom() + (bbox.GetHeight() - h) / 2f;

            canvas.SaveState();
            canvas.AddImageFittedIntoRectangle(imageData, new Rectangle(x, y, w, h), false);
            canvas.RestoreState();
        }
    }
    ...
    private static void ValidateStamp(VisibleStampSpec stamp, int pageCount)
    {
        ...
        // Plano 21: valida a imagem da rubrica ANTES de assinar — bytes vazios/corrompidos/formato não
        // suportado viram `PdfSigningException` pt-BR acionável (nunca uma exceção crua do iText escapando
        // a fronteira, nem um carimbo silenciosamente em branco). `ImageDataFactory.Create` decodifica o
        // cabeçalho e lança se inválido — mesma imagem que `ApplyVisibleStamp` vai usar depois.
        if (stamp.ImageBytes is { } imageBytes)
        {
            try { iText.IO.Image.ImageDataFactory.Create(imageBytes); }
            catch (Exception ex)
            {
                throw new PdfSigningException(
                    "A imagem da rubrica é inválida ou não pôde ser lida — use um PNG ou JPG válido.", ex);
            }
        }
    }
    ...
    private static PdfSigningException WrapCrypto(System.Security.Cryptography.CryptographicException ex) =>
        new("Não foi possível usar a chave do certificado para assinar. Se for um token/cartão " +
            "(A3), confira que ele está conectado, que o PIN foi digitado corretamente e que o " +
            "driver/middleware (ex.: SafeSign) está instalado; então tente novamente.", ex);
