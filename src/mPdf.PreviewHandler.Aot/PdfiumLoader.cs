...
internal static partial class PdfiumLoader
{
    ...
    /// função nossa — não do processo host (prevhost/dllhost) nem do CWD. `internal`: reusado por
    /// `MpdfExplorerCommand.CaminhoExe` (o comando de menu roda sob o COM surrogate `dllhost.exe`, onde
    /// `AppContext.BaseDirectory` aponta pra System32, não pra pasta da DLL).
    internal static unsafe string DiretorioDestaDll()
    {
