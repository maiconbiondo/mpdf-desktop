...
internal sealed partial class PreviewWindow
{
    ...
    public nint Criar(nint parent, RECT rect)
    {
        ...
        lock (_lockBmp) { _bgra = null; _mensagem = "Carregando pré-visualização…"; }
        InvalidateRect(_hwnd, 0, false);
        IniciarRenderEmFundo(w);
        return _hwnd;
    }
    ...
    public void Destruir()
    {
        ...
        DestroyWindow(_hwnd);
        _hwnd = 0;
        lock (_lockBmp) { _bgra = null; }
    }

    private void IniciarRenderEmFundo(int larguraPainelPx)
    {
        int geracao = ++_geracao;
        int alvo = Math.Max(1, larguraPainelPx - 16);
        var pdf = PendingPdf;
        var t = new Thread(() =>
        {
            byte[]? bgra = null; int bw = 0, bh = 0, count = 1; string? msg = null;
            try
            {
                if (pdf is not { Length: > 0 }) { msg = "Não foi possível pré-visualizar este PDF."; }
                else
                {
                    RenderedPage page; int c;
                    lock (_renderLock) { (page, c) = PreviewRenderer.RenderFitWidth(pdf, 0, alvo); }
                    bgra = page.Bgra; bw = page.WidthPx; bh = page.HeightPx; count = c;
                    Diag.Trace($"render(fundo): OK {bw}x{bh}, {c} pág, bgra={bgra?.Length ?? -1}");
                }
            }
            ...
            var h = _hwnd;
            if (h == 0) return;
            lock (_lockBmp) { _bgra = bgra; _bmpW = bw; _bmpH = bh; _pageCount = count; _mensagem = bgra is null ? msg : null; }
            InvalidateRect(h, 0, false);
        })
        { IsBackground = true, Name = "mPdfPreviewRenderAot" };
        t.Start();
    }
    ...
    private void OnWheel(nint wParam)
    {
        ...
        int maxScroll = Math.Max(0, _bmpH + 16 - visivel);
    ...
    private void OnPaint()
    {
        ...
        try
        {
            ...
            byte[]? bgra; int bw, bh, pageCount; string? msg;
            lock (_lockBmp) { bgra = _bgra; bw = _bmpW; bh = _bmpH; pageCount = _pageCount; msg = _mensagem; }

            if (bgra is not null && bw > 0 && bh > 0)
            {
                int x = Math.Max(8, (cw - bw) / 2);
                int y = 8 - _scrollY;
                DesenharBitmap(hdc, bgra, bw, bh, x, y);
                if (pageCount > 1) DesenharTexto(hdc, $"Página 1 de {pageCount}", 8, ch - 22);
            }
            else if (msg is not null)
            {
                DesenharTexto(hdc, msg, 12, 12);
            }
        }
