...
/// Plano 25: uma página-fonte posicionada numa FOLHA de saída da imposição (N-up / livreto). Neutro
/// (só int/double; nenhum tipo iText/WPF atravessa). Coordenadas em pontos PDF, mas com origem no CANTO
/// SUPERIOR-ESQUERDO da folha e Y para BAIXO (mesma convenção do motor de imposição do App) — o editor
/// converte para a origem inferior-esquerda do PDF ao compor. `PaginaFonte` é 0-based; `-1` = célula em
/// branco (padding do livreto, nada é desenhado). `Rotacao` ∈ {0,90,180,270}, sentido HORÁRIO.
public readonly record struct ColocacaoFolha(
    int Folha,
    int PaginaFonte,
    double XPt, double YPt,
    double LarguraPt, double AlturaPt,
    int Rotacao);
...
public sealed record AnnotationData
{
    ...
    /// Cor de PREENCHIMENTO (interior) ARGB do Retângulo — `/IC` no PDF. `null` = SEM preenchimento
    /// (não escreve `/IC`; sentinela igual ao `ColorArgb`/`/C` da borda). Só se aplica a
    /// `Kind == Rectangle` (Line/Arrow/Ink/marcação de texto não têm interior) — ignorado nos demais.
    /// Ao LER, reflete a ausência real de `/IC` (null) ou a cor lida. Sólido no v1 (sem opacidade).
    public uint? FillArgb { get; init; }
    /// Espessura da BORDA/contorno em pontos (`/Border [0 0 W]`). `null` = default 2pt (comportamento
    /// legado). Só se aplica às formas com contorno (Ink/Rectangle/Line/Arrow). REGRA "sem borda":
    /// quando `ColorArgb==null` (sem cor no contorno) numa forma, a borda é gravada com LARGURA 0
    /// (contorno invisível) — permite uma caixa só com preenchimento. `0` explícito também = sem borda.
...
/// Nível de conformidade PDF/A oferecido pela conversão "Salvar como PDF/A". Só os níveis "b"
/// (fidelidade visual, sem exigência de estrutura/tags) — os "a" (acessíveis/marcados) não são
/// fabricáveis a partir de uma página rasterizada.
public enum PdfANivel { A1B, A2B, A3B }

/// Uma página a compor no PDF/A: a imagem RGB (JPEG, sem alfa), o tamanho da página em PONTOS, a
/// resolução do bitmap de origem (para mapear px->pt do texto invisível) e, opcional, as caixas de
/// texto do OCR (px, topo-esquerda — mesma convenção de `OcrTextBox`). `Texto` null/vazio = página
/// sem camada pesquisável.
public sealed record PdfAPaginaImagem(
    byte[] JpegRgb, double LarguraPt, double AlturaPt,
    int FonteLarguraPx, int FonteAlturaPx,
    IReadOnlyList<OcrTextBox>? Texto);

/// Metadados do documento PDF/A (title/author entram no XMP e no DocumentInfo — o PdfADocument os
/// sincroniza). Campos null viram vazio.
public sealed record PdfAMetadados(string? Titulo, string? Autor);
...
public interface IPdfEditor
{
    ...
    /// Plano 25 (IMPOSIÇÃO): produz um PDF NOVO onde cada FOLHA (0..maxFolha das `colocacoes`) é uma
    /// página de `folhaLarguraPt`×`folhaAlturaPt`, com as páginas de `pdf` posicionadas segundo
    /// `colocacoes` (retângulo destino + rotação; proporção já preservada por quem calculou os
    /// retângulos). Cada página-fonte entra como XObject. Leitura pura de `pdf` (nunca alterado);
    /// `HasSignatures(resultado) == false`. `colocacoes` vazia, folha/tamanho inválidos, ou
    /// `PaginaFonte` fora do documento -> `ArgumentException` pt-BR antes de compor.
    byte[] ImporEmFolhas(byte[] pdf, IReadOnlyList<ColocacaoFolha> colocacoes, double folhaLarguraPt, double folhaAlturaPt);
    ...
    // --- Task 2 (SDD "Salvar como PDF/A"): contrato neutro de fabricação de PDF/A --------------

    /// Cria um PDF/A NOVO (níveis "b") compondo uma imagem por página + OutputIntent sRGB + XMP de
    /// conformidade. Cada página recebe a imagem ocupando a página inteira; se `PdfAPaginaImagem.Texto`
    /// tiver caixas, desenha texto INVISÍVEL (render mode 3) por cima com a fonte embutida
    /// `fonteInvisivelTtf` (reusa a matemática px->pt de `ComputeOcrTextMatrix`, rotação 0). O
    /// `PdfADocument` do iText RECUSA gerar fora das regras do PDF/A — se retorna bytes, o documento é
    /// válido por construção. PRECONDIÇÃO: se alguma página traz `Texto` não-vazio, `fonteInvisivelTtf`
    /// (TTF embutível) é obrigatório; caso contrário pode ser null.
    byte[] CriarPdfADeImagens(
