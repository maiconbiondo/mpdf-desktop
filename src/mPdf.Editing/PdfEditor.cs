...
internal sealed class PdfEditor : IPdfEditor
{
            ...
            for (int pageNumber = 1; pageNumber <= doc.GetNumberOfPages(); pageNumber++)
            {
                ...
                foreach (var annot in page.GetAnnotations())
                {
                    ...
                    result.Add(new AnnotationData
                    {
                        ...
                        FillArgb = ReadFillArgb(annot),
                        StrokeWidthPt = ReadStrokeWidth(annot),
    ...
    public byte[] AddAnnotation(byte[] pdf, AnnotationData annotation)
    {
        ...
        try
        {
            using (var doc = new PdfDocument(new PdfReader(input), new PdfWriter(output)))
            {
                ...
                // Espessura de traço (Task: StrokeWidthPt, Plano desenho-caixa-revisao). Só os 4 kinds
                // cujo visual é uma BORDA/TRAÇO precisam disto: Highlight/Underline/Strikeout marcam
                // texto (sem contorno); StickyNote/FreeText não têm contorno nesta v1. `SetBorder` mora
                // em `PdfAnnotation` (a raiz de toda a hierarquia, não só `PdfMarkupAnnotation`) —
                // reconciliado empiricamente (ver task-8-report.md): grava `/Border [0 0 W]` (raios de
                // canto 0, sem dash), servindo Ink/Square/Line/Arrow de uma vez sem um caso por kind.
                // REGRA "sem borda" (mesmo espírito do sentinela ColorArgb/FillArgb acima): ColorArgb==
                // null (sem cor no contorno) grava largura 0 — contorno invisível, permite uma caixa só
                // com preenchimento (FillArgb). Caso contrário, StrokeWidthPt ?? default (2pt, const
                // StrokeWidthPt abaixo — legado v1). Confirmado por render PDFium (AddAnnotation_Rectangle_
                // NoBorder_RendersNoOutline): largura 0 já é suficiente para o contorno não aparecer —
                // não foi preciso pular SetBorder.
                if (annotation.Kind is AnnotationKind.Ink or AnnotationKind.Rectangle or AnnotationKind.Line or AnnotationKind.Arrow)
                {
                    float w = annotation.ColorArgb is null ? 0f : (float)(annotation.StrokeWidthPt ?? StrokeWidthPt);
                    markup.SetBorder(new PdfAnnotationBorder(0, 0, w));
                }

                // Preenchimento (fill) do Retângulo — `/IC` (interior color), Task 1 (Plano
                // desenho-caixa-melhorias). Sentinela igual ao ColorArgb/`/C` da borda: FillArgb==null
                // -> NENHUM /IC escrito. Só Kind==Rectangle (o único que carrega este campo no
                // contrato); demais kinds nunca chegam aqui mesmo que FillArgb venha preenchido por
                // engano no caller (contrato documenta "ignorado nos demais" em Contract.cs).
                // `PdfSquareAnnotation.SetInteriorColor` NÃO tem overload `(Color)` no iText 9.7 —
                // só `(PdfArray)` e `(float[])` (confirmado por reflexão contra itext.kernel.dll
                // 9.7.0: GetMethods() de PdfSquareAnnotation lista exatamente essas 2 assinaturas).
                // Usamos `float[]` (mesma aritmética 0..255 -> 0..1 de ArgbToRgb/SetColor acima).
                if (annotation.Kind == AnnotationKind.Rectangle && annotation.FillArgb is { } fillArgb
                    && markup is PdfSquareAnnotation square)
                {
                    var (fr, fg, fb) = ArgbToRgb(fillArgb);
                    square.SetInteriorColor(new float[] { fr / 255f, fg / 255f, fb / 255f });
                }
    ...
    // Plano 25 (IMPOSIÇÃO): compõe um PDF novo — cada folha vira uma página, com as páginas-fonte
    // colocadas como XObjects nos retângulos/rotações que o App calculou (motor de imposição). Mesmo
    // padrão de doc-building de ExtractPages/Merge (PdfReader->PdfWriter, StripSignatures defensivo).
    public byte[] ImporEmFolhas(byte[] pdf, IReadOnlyList<ColocacaoFolha> colocacoes, double folhaLarguraPt, double folhaAlturaPt)
    {
        if (colocacoes.Count == 0)
            throw new ArgumentException("ImporEmFolhas requer ao menos 1 colocação.", nameof(colocacoes));
        if (folhaLarguraPt <= 0 || folhaAlturaPt <= 0)
            throw new ArgumentException("Tamanho de folha inválido (largura/altura devem ser > 0).");
        ...
        try
        {
            using (var srcDoc = new PdfDocument(new PdfReader(input)))
            {
                int origem = srcDoc.GetNumberOfPages();
                int folhas = 0;
                foreach (var c in colocacoes)
                {
                    if (c.PaginaFonte >= origem)
                        throw new ArgumentException($"Página-fonte {c.PaginaFonte} fora do documento (tem {origem} páginas).");
                    if (c.Folha + 1 > folhas) folhas = c.Folha + 1;
                }

                using (var destDoc = new PdfDocument(new PdfWriter(output)))
                {
                    var tamanho = new PageSize((float)folhaLarguraPt, (float)folhaAlturaPt);
                    var cache = new Dictionary<int, PdfFormXObject>();
                    for (int f = 0; f < folhas; f++)
                    {
                        var page = destDoc.AddNewPage(tamanho);
                        var canvas = new PdfCanvas(page);
                        foreach (var c in colocacoes)
                        {
                            if (c.Folha != f || c.PaginaFonte < 0 || c.LarguraPt <= 0 || c.AlturaPt <= 0) continue;
                            if (!cache.TryGetValue(c.PaginaFonte, out var xobj))
                            {
                                xobj = srcDoc.GetPage(c.PaginaFonte + 1).CopyAsFormXObject(destDoc);
                                cache[c.PaginaFonte] = xobj;
                            }
                            var m = MatrizImposicao(xobj, c, folhaAlturaPt);
        ...
        return StripSignatures(output.ToArray());
    }

    /// Matriz [a b c d e f] que mapeia o XObject (bbox = página-fonte) no retângulo destino, com rotação
    /// HORÁRIA, convertendo a origem topo-esquerda (Y p/ baixo) da imposição para a origem inferior-
    /// esquerda (Y p/ cima) do PDF. Proporção preservada pela imposição (s = destino/lado-correto).
    private static float[] MatrizImposicao(PdfFormXObject xobj, ColocacaoFolha c, double folhaAlturaPt)
    {
        var bbox = xobj.GetBBox().ToRectangle();
        double bx = bbox.GetX(), by = bbox.GetY(), sw = bbox.GetWidth(), sh = bbox.GetHeight();
        double dx = c.XPt;                                 // borda ESQUERDA do destino
        double dy = folhaAlturaPt - (c.YPt + c.AlturaPt);  // borda INFERIOR do destino (converte Y p/ cima)
        double s = (c.Rotacao is 90 or 270) ? c.LarguraPt / sh : c.LarguraPt / sw;

        return c.Rotacao switch
        {
            90  => [0f, (float)(-s), (float)s, 0f, (float)(dx - s * by), (float)(dy + s * sw + s * bx)],
            180 => [(float)(-s), 0f, 0f, (float)(-s), (float)(dx + s * sw + s * bx), (float)(dy + s * sh + s * by)],
            270 => [0f, (float)s, (float)(-s), 0f, (float)(dx + s * sh + s * by), (float)(dy - s * bx)],
            _   => [(float)s, 0f, 0f, (float)s, (float)(dx - s * bx), (float)(dy - s * by)],
        };
    }
    ...
    /// Espelha `ReadColorArgb`, mas para `/IC` (interior/fill, Task 1 — Plano desenho-caixa-
    /// melhorias). `GetInteriorColor()` só existe em `PdfSquareAnnotation` na hierarquia do iText
    /// 9.7 (não em `PdfMarkupAnnotation`/`PdfAnnotation` genérico — confirmado por reflexão contra
    /// itext.kernel.dll 9.7.0), daí o `is PdfSquareAnnotation` em vez de um `GetColorObject()`-like
    /// disponível pra qualquer anotação; devolve `Color` (não `PdfArray`, ao contrário da hipótese
    /// inicial do brief) — `null` quando `/IC` está ausente. `GetColorValue()` extrai os componentes
    /// 0..1 do `Color`; só RGB (3 componentes) é interpretado, mesma aritmética inversa de
    /// `ReadColorArgb` (alfa sempre 0xFF, sem opacidade no v1). try/catch -> `null`, mesmo espírito
    /// de `ResolvePageIndex` acima — um `/IC` malformado (PDF de origem externa) não derruba a
    /// leitura da anotação inteira.
    private static uint? ReadFillArgb(PdfAnnotation annot)
    {
        if (annot is not PdfSquareAnnotation square) return null;
        try
        {
            var f = square.GetInteriorColor()?.GetColorValue();
            if (f is null || f.Length != 3) return null;
            byte r = (byte)Math.Round(Math.Clamp(f[0], 0f, 1f) * 255);
            byte g = (byte)Math.Round(Math.Clamp(f[1], 0f, 1f) * 255);
            byte b = (byte)Math.Round(Math.Clamp(f[2], 0f, 1f) * 255);
            return 0xFF000000u | ((uint)r << 16) | ((uint)g << 8) | b;
    ...
    /// Lê a largura de borda gravada por este módulo — 3º elemento do array `/Border [Rh Rv W]` (o
    /// formato que o ramo de escrita acima sempre grava: raios de canto 0, sem dash, só `W` varia).
    /// `GetBorder()` devolve o `PdfArray` cru (reflexão contra itext.kernel.dll 9.7.0 — não existe
    /// `GetBorderStyle()`-like dedicado em `PdfAnnotation`; `/BS /W` é a forma ALTERNATIVA do spec PDF
    /// pra largura de borda, checada como fallback pra um PDF de origem EXTERNA que use essa forma em
    /// vez de `/Border`). Ausente/tamanho errado/malformado -> `null` (mesmo espírito de `ReadFillArgb`
    /// acima — nunca derruba a leitura da anotação inteira); não distingue "nunca escrito" de "0
    /// explícito" (ambos voltam como o valor real gravado, 0 pra sem-borda).
    private static double? ReadStrokeWidth(PdfAnnotation annot)
    {
        try
        {
            var border = annot.GetBorder();
            if (border is not null && border.Size() >= 3)
                return border.GetAsNumber(2)?.DoubleValue();

            var bs = annot.GetPdfObject().GetAsDictionary(PdfName.BS);
            return bs?.GetAsNumber(PdfName.W)?.DoubleValue();
        }
        catch (ITextException) { return null; }
    }
    ...
    /// Task 3/4 (SDD "Salvar como PDF/A"): compõe uma imagem por página num PDF/A-1b/2b/3b válido
    /// (OutputIntent sRGB embutido, título/autor sincronizados no XMP pelo próprio `PdfADocument`).
    /// Se `pag.Texto` tiver caixas, desenha por cima uma camada de texto INVISÍVEL (render mode 3)
    /// com a fonte embutida `fonteInvisivelTtf` — reusa a MESMA matemática px→pt de
    /// `ComputeOcrTextMatrix` (rotação 0, já que estas páginas são recém-criadas, nunca giradas).
    /// `PdfADocument` RECUSA gerar bytes fora das regras do PDF/A (lança em vez de devolver um
    /// documento inválido) — se este método retorna, o documento é válido por construção.
        ...
        byte[]? fonteInvisivelTtf)
    {
        if (paginas is null || paginas.Count == 0)
            throw new ArgumentException("PDF/A precisa de ao menos uma pagina.", nameof(paginas));

        // Precondição do contrato (Contract.cs): página com texto OCR exige fonte embutível — sem
        // isso não há como gravar texto extraível dentro das regras do PDF/A (fonte-base não
        // embutida é PROIBIDA). Checada ANTES de abrir o PdfADocument (mesma disciplina de
        // "nada é gravado se a validação falhar" do resto do módulo).
        bool temTexto = paginas.Any(p => p.Texto is { Count: > 0 });
        if (temTexto && (fonteInvisivelTtf is null || fonteInvisivelTtf.Length == 0))
            throw new ArgumentException(
                "Paginas com texto OCR exigem uma fonte embutivel (fonteInvisivelTtf).", nameof(fonteInvisivelTtf));

        var conformance = nivel switch
        {
            PdfANivel.A1B => PdfAConformance.PDF_A_1B,
            PdfANivel.A2B => PdfAConformance.PDF_A_2B,
            PdfANivel.A3B => PdfAConformance.PDF_A_3B,
            _ => throw new ArgumentOutOfRangeException(nameof(nivel)),
        };

        using var iccStream = typeof(PdfEditor).Assembly.GetManifestResourceStream(ResolveIccResourceName());
        if (iccStream is null) throw new PdfEditingException("Perfil de cor sRGB embutido nao encontrado.");

        using var ms = new MemoryStream();
        var outputIntent = new PdfOutputIntent("Custom", "", "https://www.color.org", "sRGB IEC61966-2.1", iccStream);
        try
        {
            using (var pdfDoc = new PdfADocument(new PdfWriter(ms), conformance, outputIntent))
            {
                var titulo = string.IsNullOrWhiteSpace(metadados.Titulo) ? "Documento" : metadados.Titulo!;
                pdfDoc.GetDocumentInfo().SetTitle(titulo);
                if (!string.IsNullOrWhiteSpace(metadados.Autor)) pdfDoc.GetDocumentInfo().SetAuthor(metadados.Autor);

                // Fonte embutida (só criada se alguma página tiver texto — obrigatório embutir bytes
                // TTF: uma fonte-base como Helvetica NÃO é permitida em PDF/A). Criada 1x e reusada
                // em todas as páginas deste documento (mesma disciplina de ApplyOcrTextLayer).
                PdfFont? fonteInv = temTexto
                    ? PdfFontFactory.CreateFont(fonteInvisivelTtf!, PdfEncodings.IDENTITY_H,
                          PdfFontFactory.EmbeddingStrategy.FORCE_EMBEDDED)
                    : null;

                foreach (var pag in paginas)
                {
                    var page = pdfDoc.AddNewPage(new PageSize((float)pag.LarguraPt, (float)pag.AlturaPt));
                    var canvas = new PdfCanvas(page);
                    var img = ImageDataFactory.Create(pag.JpegRgb);
                    canvas.AddImageFittedIntoRectangle(img, new Rectangle(0, 0, (float)pag.LarguraPt, (float)pag.AlturaPt), false);

                    if (pag.Texto is { Count: > 0 } && fonteInv is not null)
                    {
                        // Página recém-criada: sem /Rotate (rotação 0), MediaBox começa em (0,0) —
                        // dispensa a reconciliação de orientação-exibida de ApplyOcrTextLayer, mas
                        // REUSA a MESMA `ComputeOcrTextMatrix` (rotation:0, llx:0, lly:0) pra manter
                        // uma única fonte de verdade do mapeamento px->pt (fator = pt/px da própria
                        // página, igual à derivação de fatorX/fatorY em ApplyOcrTextLayer).
                        double fatorX = pag.LarguraPt / pag.FonteLarguraPx;
                        double fatorY = pag.AlturaPt / pag.FonteAlturaPx;

                        canvas.BeginText();
                        canvas.SetFontAndSize(fonteInv, 1f);
                        canvas.SetTextRenderingMode(PdfCanvasConstants.TextRenderingMode.INVISIBLE);
                        foreach (var box in pag.Texto)
                        {
                            if (string.IsNullOrWhiteSpace(box.Text)) continue; // texto vazio -> nada a gravar

                            double xbDisp = box.LeftPx * fatorX;
                            double ybDisp = pag.AlturaPt - (box.TopPx + box.HeightPx) * fatorY;
                            double fontSize = box.HeightPx * fatorY;
                            if (fontSize <= 0) continue;

                            var m = ComputeOcrTextMatrix(0, pag.LarguraPt, pag.AlturaPt, xbDisp, ybDisp, 0, 0);
                            canvas.SetFontAndSize(fonteInv, (float)fontSize);
                            canvas.SetTextMatrix((float)m.A, (float)m.B, (float)m.C, (float)m.D, (float)m.E, (float)m.F);
                            canvas.ShowText(box.Text);
                        }
                        canvas.EndText();
    ...
    // Nome lógico do recurso ICC (namespace-raiz + caminho pontuado). Resolve por sufixo pra não
    // depender do namespace-raiz exato.
    private static string ResolveIccResourceName() =>
        typeof(PdfEditor).Assembly.GetManifestResourceNames()
            .Single(n => n.EndsWith("sRGB2014.icc"));
