...
public sealed class AppConfig
{
    ...
    private readonly string _rubricaFile;

    public AppConfig(string directory)
    {
        Directory.CreateDirectory(directory);
        _file = Path.Combine(directory, "config.json");
        _rubricaFile = Path.Combine(directory, "rubrica.png");
    }
    ...
    // ==== Plano 21: rubrica salva (imagem reusada como aparência do carimbo) =========================
    //
    // A rubrica é guardada como um ARQUIVO (`rubrica.png`) no MESMO diretório do config (isolado por
    // `MPDF_CONFIG_DIR`/construtor de teste, igual ao config.json) — não como bytes no JSON (uma imagem
    // em base64 incharia o config.json lido/gravado a cada toggle). Esta classe é "burra" de propósito:
    // só faz I/O de bytes; a VALIDAÇÃO da imagem (PNG/JPG suportado) é responsabilidade da camada App
    // (via `IPdfEditor.IsSupportedImage`, ANTES de chamar `SalvarRubrica`) — `mPdf.Documents` não
    // referencia `mPdf.Editing` (seria acoplar a fronteira iText a este módulo).

    /// `true` quando existe uma rubrica salva neste diretório de config.
    public bool TemRubrica => File.Exists(_rubricaFile);

    /// Caminho absoluto do arquivo da rubrica (exista ou não) — usado pela UI só pra montar a prévia
    /// (BitmapImage por caminho). Nunca lança.
    public string CaminhoRubrica => _rubricaFile;

    /// Bytes da rubrica salva, ou `null` se não houver nenhuma. Best-effort: um arquivo removido por
    /// fora entre o `Exists` e o `ReadAllBytes` cai no `null` seguro, nunca crash.
    public byte[]? LerRubrica()
    {
        try { return File.Exists(_rubricaFile) ? File.ReadAllBytes(_rubricaFile) : null; }
        catch (IOException) { return null; }
    }

    /// Grava (ou substitui) a rubrica. Os bytes já devem ter sido validados pela camada App
    /// (`IPdfEditor.IsSupportedImage`) — aqui é só I/O.
    public void SalvarRubrica(byte[] bytes) => File.WriteAllBytes(_rubricaFile, bytes);

    /// Remove a rubrica salva (idempotente — no-op se não houver).
    public void RemoverRubrica()
    {
        try { if (File.Exists(_rubricaFile)) File.Delete(_rubricaFile); }
        catch (IOException) { /* best-effort — não crash se o arquivo estiver travado */ }
    }
}
