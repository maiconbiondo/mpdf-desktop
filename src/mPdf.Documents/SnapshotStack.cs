...
public sealed class SnapshotStack : IDisposable
{
    ...
    {
        // CONCORRÊNCIA (Plano 19): esta pilha é "UI-thread-only por convenção", mas `Dispose` foge dessa
        // convenção — `DocumentViewModel.Dispose` enfileira `session.Dispose()` na fila SERIAL de
        // `PendingDisposals` (thread de background do TPL), enquanto o DONO da sessão (ex.: um teste com
        // `using var session`, ou qualquer segundo caminho de descarte) pode chamar `session.Dispose()`
        // na sua PRÓPRIA thread. As duas chegam em `Clear()` ao mesmo tempo; sem sincronização, o
        // `foreach (var entry in _entries)` de uma disparava "Collection was modified" quando a outra
        // mutava `_entries` no meio da iteração (flaky histórico de FormFlatten/FormFill). ESTE lock
        // torna TODA mutação/leitura de `_entries` atômica — sem mudar undo/redo/spill/limite, só
        // robustez. `Dispose` continua "melhor esforço, nunca lança" (e agora idempotente). Os helpers
        // privados (`SpillOldestInMemory`/`DiscardOldestSpilled`) NÃO retomam o lock: são chamados APENAS
        // de dentro de `Push`, que já o segura (documentado em cada um).
        ...
        /// CONCORRÊNCIA (Plano 19): chamado SOMENTE de dentro de `Push`, que já segura `_sync` — não
        /// retoma o lock (seria reentrante e inofensivo, mas o contrato é "só sob o lock de Push").
        ...
        /// CONCORRÊNCIA (Plano 19): chamado SOMENTE de dentro de `Push` (sob `_sync`) — ver
        /// `SpillOldestInMemory`. `EntryDiscarded` dispara enquanto o lock é segurado; os handlers reais
        /// (`SnapshotStack.OnEntryDiscarded` -> `DocumentSession`) só marcam flags/disparam eventos e
        /// NUNCA reentram nesta pilha, então não há risco de deadlock nem de reentrância problemática.
        ...
        public bool TryPop(out byte[] value)
        {
            lock (_sync)
            {
                ...
                // Reancora o invariante — ver doc XML da classe: só importa quando o TryPop acima removeu
                // a última entrada ainda em RAM (_firstInMemoryIndex ficaria > Count sem este ajuste).
