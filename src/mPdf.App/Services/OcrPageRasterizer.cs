...
public sealed class OcrPageRasterizer : IOcrPageRasterizer
{
    ...
    /// Task 6 (SDD "Salvar como PDF/A"): tamanho real da página em pontos — delega direto ao renderer
    /// (mesmo dado que `DialogoImpressao`/`ImposedPrintPaginator` já consomem).
    public PdfPageSize GetPageSize(int pageIndex) => _renderer.GetPageSize(pageIndex);
