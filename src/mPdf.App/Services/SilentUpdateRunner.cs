...
public sealed class SilentUpdateRunner
{
    ...
    public SilentUpdateRunner(Func<IUpdateSource>? sourceFactory = null, Action<string>? lancarInstalador = null)
    {
        ...
        // reabrir:false — o update do RMM NÃO relança o mPDF na estação (invisível). Ver SilentUpdateInstaller.
        _lancarInstalador = lancarInstalador ?? (p => Process.Start(SilentUpdateInstaller.BuildStartInfo(p, reabrir: false)));
    }
