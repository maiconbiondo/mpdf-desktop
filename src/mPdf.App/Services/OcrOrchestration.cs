...
public interface IOcrPageRasterizer : IDisposable
{
    ...
    /// Tamanho da página `pageIndex` em PONTOS (72dpi) — para dimensionar a página do PDF/A (Task 6,
    /// SDD "Salvar como PDF/A"). Delegado ao `PdfDocumentRenderer.GetPageSize` (mPdf.Rendering), já
    /// usado pelo mesmo padrão em `ImposedPrintPaginator`/`DialogoImpressao`.
