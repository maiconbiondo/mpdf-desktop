...
/// `UseRubrica` (Plano 21): `true` = a aparência do carimbo posicionado é a RUBRICA salva (imagem), não o
/// selo/texto padrão. Só faz sentido quando `PlaceStamp` também é `true` (a View só permite marcar
/// "Minha rubrica" quando há rubrica salva, e "Minha rubrica" implica posicionar na página); um fake de
/// teste pode montar `UseRubrica=true` com `PlaceStamp=false` de propósito — o VM ignora `UseRubrica`
/// no caminho sem carimbo (nunca monta um `VisibleStampSpec`).
...
public interface ISignDialogService
{
    ...
    /// `hasRubrica` (Plano 21): quando `true`, a View habilita o cartão "Minha rubrica" (usa a imagem
    /// salva como aparência); quando `false`, o cartão aparece DESABILITADO com a dica de defini-la em
    /// Configurações (nunca escondido — o usuário precisa VER que a opção existe). Devolve `null` se o
    /// usuário cancelou.
...
public sealed class SignDialogService : ISignDialogService
{
    public SignDialogResult? PromptForSignature(
        IReadOnlyList<SigningCertificateInfo> certificates, bool allowDocMdp, bool hasRubrica)
    {
        var dialog = new Views.SignDialog(certificates, allowDocMdp, hasRubrica)
        {
