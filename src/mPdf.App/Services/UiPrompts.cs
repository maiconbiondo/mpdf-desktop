...
public static class UiPrompts
{
    ...
    /// <summary>Janela "Configurações" (Task 2, Plano 17 — <see cref="ConfiguracoesDialogService"/>) —
    /// default do parâmetro `configuracoesDialog` de `MainViewModel`. Hospeda Tema/Nitidez/Atualização,
    /// migrados do "Sobre" (ver doc XML de <see cref="mPdf.App.ViewModels.ConfiguracoesViewModel"/>).</summary>
    public static Func<IConfiguracoesDialogService> CreateConfiguracoesDialog { get; set; } = () => new ConfiguracoesDialogService();
    ...
    /// <summary>Diálogo "Salvar como PDF/A" (Task 6, SDD "Salvar como PDF/A" — <see cref="IPdfADialogService"/>,
    /// view ligada na Task 7 — <see cref="PdfADialogService"/>) — default do parâmetro `pdfADialog` de
    /// <see cref="mPdf.App.ViewModels.DocumentViewModel"/>. Mesma classe de risco/forma de
    /// `CreateExportImageDialog`: abre uma `Window` real que um teste headless não pode alcançar sem
    /// travar — por isso entra em `UiPromptsTestGuard`/`UiPromptsGuardTests` como qualquer outro diálogo
    /// desta seam (a partir da Task 7; antes disso o default TEMPORÁRIO já lançava `NotImplementedException`
    /// por construção, sem hang possível, e ficava de fora dos dois).</summary>
    public static Func<IPdfADialogService> CreatePdfADialog { get; set; } = () => new PdfADialogService();
