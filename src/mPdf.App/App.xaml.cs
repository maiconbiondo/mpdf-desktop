...
public partial class App : Application
{
    ...
    // Plano 18 (Task 2): mutex `Global\` de PRESENÇA que o instalador silencioso (`AppMutex=` do
    // `mpdf.iss`) usa para detectar esta instância e esperá-la fechar (`CloseApplications=yes`) antes de
    // trocar os arquivos do .exe em uso. SEGURADO pela vida inteira do processo (mantido neste campo,
    // liberado em `OnExit`) — enquanto qualquer handle ao objeto nomeado existir, o Inno o vê. INDEPENDENTE
    // do `SingleInstanceService` acima (que é por usuário+sessão e cuja lógica de abrir-por-duplo-clique
    // NÃO é tocada): este é só um farol de presença, não participa de nenhuma decisão de instância única.
    // Best-effort (ver `SilentUpdateInstaller.AcquireAppMutex`): uma falha ao criar o objeto Global NUNCA
    // pode impedir o app de abrir.
    private Mutex? _updateAppMutex;

    protected override void OnStartup(StartupEventArgs e)
    {
        // Atualizacao por linha de comando (RMM/mTI): /checkupdate (so verifica) e /update (silencioso).
        // ANTES da instancia unica; processo efemero (Shutdown com exit code). Task.Run OBRIGATORIO:
        // durante OnStartup o WPF ja instalou o DispatcherSynchronizationContext e bloquear a async
        // diretamente travaria (deadlock) — Task.Run escapa o contexto de UI.
        var cmdUpdate = e.Args.Length > 0 ? e.Args[0].ToLowerInvariant() : "";
        if (cmdUpdate is "/checkupdate" or "/update")
        {
            var runner = new Services.SilentUpdateRunner();
            var status = System.Threading.Tasks.Task.Run(() => cmdUpdate == "/update"
                ? runner.AtualizarAsync(System.Threading.CancellationToken.None)
                : runner.VerificarAsync(System.Threading.CancellationToken.None)).GetAwaiter().GetResult();
            var linha = Services.SilentUpdateRunner.FormatarStatus(cmdUpdate, status);
            Services.ConsoleStatus.Escrever(linha);
            try { CrashLog.Append(new Exception("[update-cli] " + linha)); } catch { } // trilha no log de diagnostico
            Shutdown(Services.SilentUpdateRunner.MapearExitCode(status.Resultado));
            return;
        }

        // Impressao pelo menu de contexto (/print, /printadv): processo efemero que imprime e encerra,
        // ANTES da instancia unica (nao vira aba de um editor aberto) e SEM materializar a MainWindow
        // (mesmo mecanismo de Shutdown()-antes-de-base.OnStartup ja usado abaixo pro argPath).
        var ctxImpressao = Services.ImpressaoContextoService.Parse(e.Args);
        if (ctxImpressao.Modo != Services.ModoImpressaoContexto.Nenhum)
        {
            try
            {
                if (ctxImpressao.Modo == Services.ModoImpressaoContexto.Silencioso)
                {
                    Services.ImpressaoContextoService.ImprimirSilencioso(
                        ctxImpressao.Caminho, new Services.ImpressoraPadraoReal(),
                        msg => MessageBox.Show(msg, "mPDF", MessageBoxButton.OK, MessageBoxImage.Warning));
                }
                else // Avancado
                {
                    if (string.IsNullOrWhiteSpace(ctxImpressao.Caminho) || !System.IO.File.Exists(ctxImpressao.Caminho))
                        MessageBox.Show("Arquivo PDF nao encontrado ou invalido.", "mPDF", MessageBoxButton.OK, MessageBoxImage.Warning);
                    else
                    {
                        // Aplica o tema persistido (mesma chamada de baixo, ~linha 153) ANTES de
                        // mostrar o dialogo: esta instancia efemera nunca chega la (Shutdown() logo
                        // abaixo), entao sem isto /printadv sempre abria no tema Escuro default,
                        // ignorando um usuario que escolheu Claro numa sessao anterior. Best-effort
                        // (AppConfig.Load trata JsonException) e ja protegido pelo try/catch externo
                        // deste bloco -> CrashLog -> Shutdown.
                        Services.ThemeService.AplicarNoApp(new mPdf.Documents.AppConfig(mPdf.Documents.AppConfig.DefaultDirectory).ThemeMode);

                        var session = mPdf.Documents.DocumentSession.Open(ctxImpressao.Caminho);
                        new Views.DialogoImpressao(session, System.IO.Path.GetFileNameWithoutExtension(ctxImpressao.Caminho)).ShowDialog();
                    }
                }
            }
            catch (System.Exception ex) { CrashLog.Append(ex); MessageBox.Show($"Nao foi possivel imprimir: {ex.Message}", "mPDF", MessageBoxButton.OK, MessageBoxImage.Warning); }
            Shutdown();
            return;
        }
        ...
        // Plano 18 (Task 2): adquire o mutex `Global\` de presença para o instalador silencioso (ver o
        // campo `_updateAppMutex`). Best-effort — captura QUALQUER exceção (ex.: sem privilégio para criar
        // objeto no namespace `Global\`), registra em CrashLog (chamável mesmo antes dos handlers de crash
        // abaixo) e SEGUE: uma falha aqui só significa que o instalador não vai esperar por este mutex
        // (recai no fechamento por janela do `CloseApplications`), nunca impedir o app de abrir.
        try { _updateAppMutex = SilentUpdateInstaller.AcquireAppMutex(); }
        catch (Exception ex) { CrashLog.Append(ex); }

        // Plano 14 (Task 1): aplica o TEMA persistido (AppConfig.ThemeMode, default Escuro) ANTES da
        // MainWindow do StartupUri ser materializada (o framework só a cria DEPOIS que este OnStartup
        // retorna — ver comentário em argPath acima). App.xaml já mescla Tokens.Escuro por padrão, então
        // isto só troca de fato quando o usuário escolheu Claro numa sessão anterior; nos dois casos a
        // janela nasce já com o tema certo, sem flash. Best-effort: uma leitura de config corrompida cai
        // no default seguro (AppConfig.Load trata JsonException), e AplicarNoApp opera sobre
        // Application.Current.Resources (existe neste ponto).
        Services.ThemeService.AplicarNoApp(new mPdf.Documents.AppConfig(mPdf.Documents.AppConfig.DefaultDirectory).ThemeMode);
    ...
    protected override void OnExit(ExitEventArgs e)
    {
        ...
        // Plano 18 (Task 2): libera o mutex `Global\` de presença — o último handle a fechar destrói o
        // objeto nomeado, sinalizando ao instalador (que espera por ele via `CloseApplications`) que pode
        // prosseguir com a troca dos arquivos.
        _updateAppMutex?.Dispose();
        base.OnExit(e);
    }
