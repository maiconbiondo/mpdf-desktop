...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        // Ctrl+Shift+S = "Salvar como" (salvar uma cópia em outro local) — SaveAsCommand já tem CanExecute
        // (sem documento / edição em curso, o KeyBinding não dispara, igual ao botão desabilitado).
        InputBindings.Add(new KeyBinding(ViewModel.SaveAsCommand, Key.S, ModifierKeys.Control | ModifierKeys.Shift));
        ...
        // Task 4 (SDD desenho-caixa-melhorias): Ctrl+C agora tem 2 caminhos — decisão de produto
        // documentada no brief: ANOTAÇÃO selecionada tem PRIORIDADE sobre seleção de texto (mesmo
        // documento, `SelectedDocument.SelectedAnnotation != null` -> `CopyAnnotationCommand`; senão o
        // copiar-de-TEXTO de sempre, `CopySelectedText`). `CopyOrCopySelectedText` decide qual dos 2 —
        // wiring FINO por construção (o roteamento em si é só uma checagem de campo + `if/else`, toda a
        // LÓGICA de cópia/clone vive no comando testável do VM, ver `DocumentViewModel.CopyAnnotation`).
        InputBindings.Add(new KeyBinding(new RelayCommand(CopyOrCopySelectedText), Key.C, ModifierKeys.Control));
        // Task 4 (SDD desenho-caixa-melhorias): Ctrl+V cola a anotação do clipboard INTERNO (não há
        // "colar texto" nesta versão do app — nenhuma edição de texto de documento existe pra colar em
        // cima —, então V é só pra anotação). `PasteAnnotationCommand` já tem seu PRÓPRIO CanExecute
        // (clipboard vazio/documento assinado/edição em voo -> não dispara, mesmo padrão de
        // Undo/Redo acima) — o wrapper só resolve QUAL documento (aba ATIVA), mesma técnica.
        InputBindings.Add(new KeyBinding(new RelayCommand(PasteAnnotation), Key.V, ModifierKeys.Control));
        ...
        // Delete = excluir a anotação SELECIONADA (Fix v2.12.2). NÃO é um KeyBinding (sem modificador,
        // ele "comeria" o Delete de qualquer TextBox); é um PreviewKeyDown de JANELA — dispara com
        // qualquer foco (o handler do viewer só disparava com o foco DENTRO do PageList, o que raramente
        // acontece ao selecionar anotação com o mouse -> Delete não funcionava sem o botão "Excluir").
        // Guarda: se o foco está numa caixa de texto (busca, edição de FreeText), deixa o Delete apagar
        // texto normalmente; só apaga a anotação fora desse caso.
    ...
    /// Task 4 (SDD desenho-caixa-melhorias): roteamento do Ctrl+C — ver doc XML do KeyBinding acima.
    /// Anotação selecionada (e não-ImageStamp — `CopyAnnotationCommand.CanExecute` já filtra isso)
    /// vence; senão cai pro `CopySelectedText` de sempre. `CanExecute(null)` (não `Execute` direto):
    /// mesmo cuidado de `Undo`/`Redo` acima — sem seleção de anotação válida o comando simplesmente não
    /// dispara (no-op), nunca lança.
    // Fix v2.12.2: Delete apaga a anotação selecionada, com qualquer foco (ver comentário no construtor).
    private void OnWindowPreviewKeyDown(object sender, System.Windows.Input.KeyEventArgs e)
    {
        if (e.Key != System.Windows.Input.Key.Delete) return;
        // Foco numa caixa de texto (busca / edição de FreeText) -> deixa o Delete apagar o texto.
        ...
        if (doc?.SelectedAnnotation is not null && doc.DeleteSelectedAnnotationCommand.CanExecute(null))
        {
    ...
    private void CopyOrCopySelectedText()
    {
        if (ViewModel.SelectedDocument?.SelectedAnnotation is not null
            && ViewModel.SelectedDocument.CopyAnnotationCommand.CanExecute(null))
        {
            ViewModel.SelectedDocument.CopyAnnotationCommand.Execute(null);
            return;
        }
        CopySelectedText();
    }

    /// Task 4 (SDD desenho-caixa-melhorias): roteamento do Ctrl+V — ver doc XML do KeyBinding acima.
    private void PasteAnnotation()
    {
        if (ViewModel.SelectedDocument?.PasteAnnotationCommand is { } cmd && cmd.CanExecute(null))
            cmd.Execute(null);
    }
    ...
    // Plano 20: o ⚙ Configurações saiu do pé do rail para a barra superior (ao lado do Sobre), onde é
    // um Button comum ligado direto a `ConfiguracoesCommand` — não precisa mais deste handler de
    ...
    protected override void OnSourceInitialized(EventArgs e)
    {
        ...
        // Plano 23: em monitores MENORES que o tamanho de projeto (1200x800), o tamanho de RESTAURAÇÃO
        // (Normal) estourava a tela — ao restaurar, a janela ficava mais alta/larga que o monitor e a
        // barra de título (com min/max/fechar) saía POR CIMA, ficando inacessível. Clampa o tamanho Normal
        // ao work area (~92%) e recentraliza, sem tocar no estado inicial MAXIMIZADO. Assim: abre
        // maximizada (botões no topo, sempre acessíveis) E, ao restaurar, cabe na tela com os botões
        // alcançáveis. Só ENCOLHE (nunca aumenta além do projeto). WorkArea em DIPs = mesma unidade de
        // Width/Height. Como WindowState=Maximized, ajustar Width/Height/Left/Top aqui define o RestoreBounds.
        AjustarTamanhoRestauracaoParaTela();
    }

    private void AjustarTamanhoRestauracaoParaTela()
    {
        var wa = SystemParameters.WorkArea; // work area do monitor primário, em DIPs
        if (wa.Width <= 0 || wa.Height <= 0) return;

        double alvoW = System.Math.Min(Width, wa.Width * 0.92);
        double alvoH = System.Math.Min(Height, wa.Height * 0.92);
        bool mudou = false;
        if (alvoW < Width) { Width = alvoW; mudou = true; }
        if (alvoH < Height) { Height = alvoH; mudou = true; }
        if (!mudou) return; // já cabe — nada a fazer

        // recentraliza o tamanho Normal na work area (WindowStartupLocation não recentraliza após o resize)
