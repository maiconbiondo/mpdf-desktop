...
public partial class SignDialog : Window
{
    ...
    public SignDialog(IReadOnlyList<SigningCertificateInfo> certificates, bool allowDocMdp, bool hasRubrica)
    {
        InitializeComponent();
        ...
        // Plano 21: "Minha rubrica" só é selecionável quando há rubrica salva. Sem rubrica -> cartão
        // desabilitado (VISÍVEL, nunca escondido — o usuário precisa saber que a opção existe) + a dica
        // "defina em Configurações". Nunca mexe na seleção default ("Carimbo padrão").
        RubricaStampRadio.IsEnabled = hasRubrica;
        RubricaHintText.Visibility = hasRubrica ? Visibility.Collapsed : Visibility.Visible;
