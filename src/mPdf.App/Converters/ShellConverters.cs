...
/// Revisão 2 (compacta cores da barra de anotação): `uint?` ARGB (`SelectedMarkupColorArgb`/
/// `SelectedFillColorArgb` do DocumentViewModel) -> `SolidColorBrush`, pro quadradinho de "cor atual" dos
/// 2 seletores compactos (Contorno/Preench.) mostrar a cor selecionada sem repetir o parsing de hex em
/// XAML. `null` (opção "Nenhum", 1ª classe desde a Task B de desenho-caixa-revisao) -> `Brushes.Transparent`
/// (já congelado pelo próprio WPF) — o quadrado some visualmente, mas a borda fixa do Border que o hospeda
/// (ver AnnotationToolsView.xaml) continua desenhando o contorno, o mesmo efeito "vazado" do antigo
/// SwatchVazado. Brush sempre congelado (Freeze()) — não muda depois de criado, thread-safe, mais barato
/// pro renderer.
public sealed class ArgbParaBrushConverter : IValueConverter
{
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (value is not uint argb) return Brushes.Transparent;
        var cor = Color.FromArgb((byte)(argb >> 24), (byte)(argb >> 16), (byte)(argb >> 8), (byte)argb);
