...
public sealed partial class OrganizerViewModel : ObservableObject, IDisposable
{
    ...
    /// Plano 14 (Task 5): rótulo mudo "N página(s) selecionada(s)" da barra de ações do organizador —
    /// puramente APRESENTAÇÃO (deriva de `SelectedIndexes`, nada muta). Notificado junto de `HasSelection`
    /// em `NotifySelectionChanged` (SelectedIndexes é recomputado a cada leitura, sem PropertyChanged
    /// próprio — este rótulo precisa da mesma reavaliação que os CanExecute de seleção).
    public string SelectionCountLabel => SelectedIndexes.Count switch
    {
        0 => "Nenhuma página selecionada",
        1 => "1 página selecionada",
        var n => $"{n} páginas selecionadas",
    };
