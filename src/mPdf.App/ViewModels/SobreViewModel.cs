...
public sealed partial class SobreViewModel : ObservableObject
{
    ...
    // Task 2 (Plano 13): AppConfig injetável (mesmo padrão de MainViewModel/DocumentViewModel — testes
    // usam um diretório temporário próprio, nunca tocam %AppData%\mPDF real) e um callback OPCIONAL que
    // o chamador (produção: `MainViewModel.Sobre`) usa pra re-renderizar todos os documentos JÁ ABERTOS
    ...
    // Plano 14 (Task 1): callback que APLICA o tema ao vivo (produção: ThemeService.AplicarNoApp, que
    // troca o dicionário de tokens em Application.Resources). Opcional com no-op seguro — mesma classe de
    // risco de `_applySupersampleFactor` (um teste que não injeta nada nunca toca Application.Current).
    ...
    /// Task 2 (Plano 13): estado do toggle "Nitidez extra do texto" (Sobre dialog) — inicializado da
    /// config PERSISTIDA (`AppConfig.NitidezExtra`) no construtor, então reabrir o diálogo sempre mostra
    /// o estado real, não um default hardcoded. `OnNitidezExtraChanged` abaixo persiste E propaga pro
    /// callback de re-render — mesmo padrão "1 propriedade observável, 1 partial void reage" já usado por
    /// `DocumentViewModel.SupersampleFactor`/`OnSupersampleFactorChanged` (Task 1).
    [ObservableProperty] private bool nitidezExtra;

    /// Plano 14 (Task 1): estado do toggle "Tema escuro" (Sobre) — inicializado da config PERSISTIDA
    /// (`AppConfig.ThemeMode`) no construtor. Marcado = Escuro (o default v2.0); desmarcado = Claro (a
    /// paleta do Plano 12). `OnTemaEscuroChanged` persiste E aplica ao vivo — mesmo padrão de
    /// `NitidezExtra` acima.
        ...
        Action<ThemeMode>? aplicarTema = null)
    {
        ...
        // Task 2 (Plano 13): mesma disciplina — `config` opcional (default aponta pro %AppData% real de
        // produção, mesmo padrão de MainViewModel/DocumentViewModel), `applySupersampleFactor` opcional
        // com no-op seguro (nenhum documento pra atualizar num teste que não injeta callback nenhum —
        // NUNCA lança/toca estado nenhum por omissão, mesma classe de risco de `_logCrash` acima).
        _config = config ?? new AppConfig(AppConfig.DefaultDirectory);
        _applySupersampleFactor = applySupersampleFactor ?? (_ => { });
        // Plano 14 (Task 1): mesma disciplina — no-op seguro por omissão (nenhum teste sem callback toca
        // Application.Current/o dicionário de tokens).
        ...
        // nenhuma -- nenhum documento existe ainda quando este VM é construído por MainViewModel.Sobre).
    }
    ...
    /// Task 2 (Plano 13): toggle "Nitidez extra do texto" — persiste na config JÁ (não espera "Fechar")
    /// e re-renderiza todo documento ABERTO agora, via o callback do chamador. Mesmo padrão exato de
    /// `DocumentViewModel.OnSupersampleFactorChanged` (Task 1): sem este hook, ligar/desligar o toggle
    /// só "faria efeito" na próxima vez que algo mais disparasse um re-render (zoom, DPI) — mesma classe
    /// de bug real, pego lá pelo teste STA ponta a ponta.
    ...
    /// Plano 14 (Task 1): toggle "Tema escuro" — persiste na config JÁ (não espera "Fechar") e aplica o
    /// tema AO VIVO via o callback (ThemeService troca o dicionário de tokens; os {DynamicResource Cor.*}
    /// re-pintam a UI inteira sem reiniciar). Mesmo padrão de `OnNitidezExtraChanged`.
