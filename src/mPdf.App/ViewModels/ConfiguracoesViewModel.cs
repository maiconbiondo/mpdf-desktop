...
public sealed partial class ConfiguracoesViewModel : ObservableObject
{
    ...
    // Plano 23: abre a tela "Aplicativos padrão" do Windows pra o usuário escolher o mPDF como leitor de
    // PDF (o Windows 11 NÃO deixa um app se auto-definir como padrão — a escolha é sempre do usuário). A
    // associação em si (ProgID/OpenWith/Capabilities/RegisteredApplications) já vem do instalador; isto só
    // leva o usuário até o lugar certo. `Action` simples (namespace System, isento da varredura de
    // cobertura de UiPrompts) com default de produção; teste injeta um espião sem abrir nada de verdade.
    private readonly Action _abrirDefaultApps;
        ...
        Action? abrirDefaultApps = null)
    {
        ...
        _abrirDefaultApps = abrirDefaultApps ?? DefaultAbrirDefaultApps;
        // Plano 17 (Task 3): estado inicial lido direto no campo -- NÃO dispara OnMenuAnotacaoNaBarraLateralChanged
        // (mesmo raciocínio de nitidezExtra: é o estado INICIAL, não uma mudança de UI).
    ...
    /// e da tira do rail, sem recriar a janela). Mesmo padrão dos demais toggles.
    ...
    /// Plano 23: "Definir mPDF como leitor padrão de PDF" — abre a tela de Aplicativos padrão do Windows
    /// (o Windows 11 não permite auto-definir; a escolha é do usuário). Ajuda a "abrir PDFs do navegador
    /// no mPDF": com o mPDF como padrão, os PDFs baixados/abertos vão pra ele.
    [RelayCommand]
    private void DefinirLeitorPadrao() => _abrirDefaultApps();

    private static void DefaultAbrirDefaultApps()
    {
        try
        {
        ...
        catch { /* best-effort — não crash se o app de Configurações estiver bloqueado (edição corporativa) */ }
    }
