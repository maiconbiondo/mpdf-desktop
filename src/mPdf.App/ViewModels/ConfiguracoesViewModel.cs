...
public enum ConfiguracoesEstado
{
    ...
    /// e atualizar?" ou cancelou a resolução de documentos sujos. O arquivo baixado é MANTIDO no
...
public sealed partial class ConfiguracoesViewModel : ObservableObject
{
    ...
    // Plano 21: callback que abre o seletor de imagem, VALIDA (PNG/JPG suportado, limite de pixels, não
    // CMYK) e devolve os bytes prontos — ou `null` se o usuário cancelou OU a imagem foi rejeitada (nesse
    // caso a produção JÁ notificou o erro; este VM não re-notifica). Opcional com no-op seguro (default
    // devolve `null`), mesma classe de risco de `_aplicarTema`/`_applySupersampleFactor`: um teste que
    // não injeta nada nunca abre um seletor de arquivo real.
    private readonly Func<byte[]?> _escolherRubricaImagem;
    ...
    /// Plano 21: `true` quando há uma rubrica (PNG/JPG) salva — a View mostra a prévia + "Remover" quando
    /// `true`, ou só "Escolher imagem…" quando `false`. Inicializado do config persistido no construtor.
    [ObservableProperty] private bool temRubrica;

    /// Bytes da rubrica salva (ou `null`) — a View converte pra prévia (BitmapImage). Exposto como DADO
    /// puro (não `ImageSource`) pra o VM continuar testável sem WPF imaging; a conversão vive no
    /// code-behind do diálogo. Lido do disco a cada acesso, e re-notificado nos comandos abaixo.
    public byte[]? RubricaPreviewBytes => _config.LerRubrica();
    ...
    public ConfiguracoesViewModel(
        ...
        Func<byte[]?>? escolherRubricaImagem = null)
    {
        ...
        _escolherRubricaImagem = escolherRubricaImagem ?? (() => null);
        temRubrica = _config.TemRubrica; // Plano 21: estado inicial lido do config (não dispara nada)
    ...
    /// Plano 21: "Escolher imagem…" da rubrica — abre o seletor (produção valida PNG/JPG e notifica erro),
    /// salva no config e atualiza o estado/prévia. Cancelar OU imagem rejeitada = `null` do callback =
    /// no-op silencioso (a produção já avisou). Persiste JÁ (mesma disciplina dos toggles acima).
    [RelayCommand]
    private void EscolherRubrica()
    {
        if (_escolherRubricaImagem() is not { } bytes) return;
        _config.SalvarRubrica(bytes);
        TemRubrica = true;
        OnPropertyChanged(nameof(RubricaPreviewBytes));
    }

    /// Plano 21: "Remover" a rubrica salva — apaga o arquivo e volta ao estado sem rubrica (a assinatura
    /// volta a oferecer só o carimbo padrão). Idempotente.
    [RelayCommand]
    private void RemoverRubrica()
    {
        _config.RemoverRubrica();
        TemRubrica = false;
        OnPropertyChanged(nameof(RubricaPreviewBytes));
    }
    ...
    private async Task VerificarAtualizacao()
    {
        ...
        switch (resultado.Status)
        {
            case UpdateCheckStatus.Disponivel:
                ...
                // T3 (Plano 18) — fluxo enxuto: em vez de deixar só a tela (com o botão "Baixar e
                // instalar") esperando um clique, pergunta de cara com um prompt CURTO. Aprovado -> baixa
                // AUTOMATICAMENTE (mesmo caminho de código do botão -- `BaixarEInstalar` abaixo, chamado
                // direto como método comum, não via `ICommand`). Recusado -> Estado permanece Disponivel,
                // e o botão "Baixar e instalar" na tela continua servindo de fallback manual (usuário pode
                // mudar de ideia depois sem precisar clicar "Verificar atualização" de novo).
                if (_confirmInstall.Confirm($"Nova versão {resultado.Info!.TagVersao} disponível — Atualizar agora?"))
                {
                    await BaixarEInstalar();
                }
                break;
    ...
    {
        // T3 (Plano 18): mensagem enxuta, mas com a nota discreta do UAC embutida -- o usuário precisa
        // saber que o 1 clique de permissão do Windows que vai aparecer em seguida é ESPERADO (instalador
        // silencioso da T2 ainda passa pelo UAC -- não há bypass de elevação).
        if (!_confirmInstall.Confirm("Fechar e atualizar? Vai aparecer uma permissão do Windows e o mPDF reabre atualizado."))
        {
        ...
        catch (Exception ex)
        {
            _logCrash(ex);
            CaminhoArquivoBaixado = arquivo.CaminhoArquivo;
            MensagemErro = $"Não foi possível iniciar a atualização. O arquivo baixado está em: {arquivo.CaminhoArquivo}";
            Estado = ConfiguracoesEstado.Erro;
            return Task.CompletedTask;
        }
