...
/// Plano 21: PROPÓSITO da caixa ajustável — a MESMA máquina de estados (desenhar/ajustar/mover/
/// redimensionar) serve os casos: `Assinatura` (carimbo padrão OU rubrica — confirmar dispara o motor
/// de assinatura), `Imagem` (colocar uma imagem nova como anotação — confirmar adiciona a anotação),
/// `EdicaoImagem` (redimensionar/mover uma imagem JÁ colocada — confirmar faz lift Remove+Add) e, Task 3
/// (SDD desenho-caixa-melhorias), `EdicaoForma` (redimensionar/mover uma `Rectangle` JÁ colocada — MESMO
/// lift Remove+Add de `EdicaoImagem`, só que sem bytes/cache: a forma é totalmente reconstruível a
/// partir de `ReadAnnotations`, ver `BeginShapeEditBox`/`ConfirmShapeEditBoxAsync`). O propósito decide
/// só o que o CONFIRMAR faz (`ConfirmStampBoxAsync`) e o rótulo do botão; toda a geometria é idêntica.
public enum StampBoxPurpose
{
    Assinatura,
    Imagem,
    EdicaoImagem,
...
public sealed partial class DocumentViewModel : ObservableObject, IDisposable
{
    ...
    // Task B (SDD desenho-caixa-revisao): paleta ampliada de 3 pra 7 cores — mesmas ARGB opacas (alfa
    // 0xFF) dos 3 originais acima, sem nenhuma noção de translucência nova (ver comentário acima sobre
    // /Highlight herdar a transparência do PRÓPRIO subtype, não de um /CA fracionário deste módulo).
    public const uint ColorPreto = 0xFF000000;
    public const uint ColorBranco = 0xFFFFFFFF;
    public const uint ColorLaranja = 0xFFFF8800;
    public const uint ColorAzul = 0xFF1E6FE0;

    private readonly IPdfEditor _editor;
    private readonly AppConfig _config;
    private readonly RubricaGallery _rubricaGallery; // Plano 22: galeria de rubricas (diálogo de assinar)
    ...
    // ---- Task 6 (SDD "Salvar como PDF/A"): orquestração "Salvar como PDF/A" -------------------------
    /// Diálogo "Salvar como PDF/A" (nível + manter pesquisável) — INJETADO cru e resolvido no comando
    /// `SalvarComoPdfA()`, não aqui no campo/ctor. O default de `UiPrompts.CreatePdfADialog` é
    /// `() => new PdfADialogService()` (a view real da Task 7; NÃO lança) — então resolvê-lo no ctor como
    /// `_exportImageDialog` funcionaria e seria uma escolha válida. A resolução lazy é um resíduo do
    /// estado interino da Task 6 (quando o default era um stub que lançava `NotImplementedException` já na
    /// FÁBRICA), mantido por ser inofensivo: `SalvarComoPdfACoreAsync` (usado pelos testes desta task)
    /// nunca toca este campo, e o comando resolve o default no instante em que o diálogo é preciso.
    private readonly IPdfADialogService? _pdfADialog;
    /// Diálogo "Salvar como…" para o arquivo NOVO do PDF/A. Seam PRÓPRIO (não `_dialogs`/`IFileDialogService`
    /// acima) — exemplar `DialogoImpressao.SalvarPdf_Click` (`new FileDialogService().PickPdfToSave(...)`
    /// direto, fora da seam UiPrompts): mesma forma aqui, só que injetável, pra testes de orquestração
    /// substituírem sem abrir um `SaveFileDialog` real. `Func<string,string?>` (namespace System, fora da
    /// varredura de UiPromptsCoverageTests — mesma isenção estrutural de `rasterizerFactory`).
    private readonly Func<string, string?> _pickPdfToSave;
    ...
    /// Task B (SDD desenho-caixa-revisao): virou `uint?` (era `uint` fixo) — `null` agora É uma opção de
    /// 1ª classe (`SelectColorNenhum` abaixo), mesmo espírito de `SelectedFillColorArgb` logo abaixo,
    /// porque a REGRA da Task A (`AnnotationData.ColorArgb == null` -> `PdfEditor` grava largura de
    /// borda 0, ver `Contract.cs`) só se aplica a FORMAS (Rectangle/Line/Arrow/Ink); em Highlight/
    /// Underline/Strikeout (`ApplyMarkup`) e StickyNote (`PlaceAnnotationAtAsync`) este valor nunca é
    /// `null` na prática porque nenhum caminho de UI de MARCAÇÃO oferece o botão "Nenhum" — só o
    /// contexto de forma/edição (`SelectColor*` com uma forma selecionada, Step 4) o expõe. Default
    /// continua `ColorAmarelo` (cor VISÍVEL) — preserva o comportamento de todo highlight/nota criado
    /// antes desta task.
    [ObservableProperty] private uint? selectedMarkupColorArgb = ColorAmarelo;

    /// Task 2 (SDD desenho-caixa-melhorias): cor de PREENCHIMENTO selecionada na toolbar — `null` (default)
    /// = sem preenchimento, preserva o comportamento de toda caixa desenhada antes desta task (ver
    /// `AnnotationData.FillArgb`, Task 1, e `CommitDrawingAsync` abaixo, que só carrega `FillArgb` pra
    /// `AnnotationKind.Rectangle`). Mesmo espírito de `SelectedMarkupColorArgb` acima (desde a Task B
    /// também `uint?`, com "nenhuma" como opção de 1ª classe): "sem preenchimento" É uma opção de 1ª
    /// classe da UI (botão "Nenhum" — ver `SelectFillNenhum` abaixo), não apenas a ausência de escolha.
    [ObservableProperty] private uint? selectedFillColorArgb;

    /// Task B (SDD desenho-caixa-revisao): espessura de borda SELECIONADA na toolbar pras 4 formas
    /// (Rectangle/Line/Arrow/Ink) — `null` (default) = espessura padrão do motor (`PdfEditor.
    /// StrokeWidthPt` const, 2pt — mesmo contrato de `AnnotationData.StrokeWidthPt`, Task A: `null` ali
    /// significa "usa o default do motor", não "zero"). Mesmo padrão de `SelectedFillColorArgb`/
    /// `SelectedMarkupColorArgb` acima: só um valor-PADRÃO pra próxima forma desenhada (`CommitDrawingAsync`
    /// abaixo) — aplicar a uma forma JÁ selecionada é o funil `SelectEspessuraAsync` (Step 4, mesmo
    /// espírito de `SelectColorAsync`/`SelectFillAsync`).
    [ObservableProperty] private double? selectedStrokeWidthPt = null;

    /// Constantes de espessura da toolbar (brief: "fina/média/grossa") — `EspessuraMedia` é
    /// DELIBERADAMENTE igual ao default do motor (`PdfEditor.StrokeWidthPt` const = 2f), mas os dois
    /// nunca precisam ficar em sincronia automática: escolher "Média" explicitamente seta
    /// `SelectedStrokeWidthPt = 2.0` (um valor NÃO-null que o motor grava explicitamente), diferente do
    /// default `null` (que delega ao motor) — resultado visual idêntico, proveniência diferente.
    public const double EspessuraFina = 1.0;
    public const double EspessuraMedia = 2.0;
    public const double EspessuraGrossa = 4.0;
    ...
    /// Task 4 (SDD desenho-caixa-melhorias): clipboard de anotação INTERNO ao app — NUNCA toca a área
    /// de transferência do Windows (`Clipboard.SetText`/`GetText`), é só um campo deste VM. Populado por
    /// `CopyAnnotation` (clone de `SelectedAnnotation` SEM Id, `with { Id = null }`), consumido por
    /// `PasteAnnotation` (que gera um Id NOVO ao colar — ver doc XML lá). Sobrevive a `Session.Applied`
    /// de propósito (diferente de `SelectedAnnotation`, que `OnSessionApplied` limpa a cada edição): colar
    /// depois de outra edição qualquer (ex.: mover uma anotação diferente) continua colando o MESMO
    /// conteúdo copiado — mesmo espírito de "clipboard sobrevive a outras ações" de qualquer app real.
    private AnnotationData? _annotationClipboard;
    ...
    partial void OnSelectedAnnotationChanged(AnnotationData? oldValue, AnnotationData? newValue)
    {
        ...
        // Task 4 (SDD desenho-caixa-melhorias): Copy/Replicar compõem SelectedAnnotation (não-ImageStamp)
        // — mesmo disparo dos 2 comandos acima, mesmo motivo (CommunityToolkit não reavalia CanExecute
        // sozinho quando a fonte é uma PROPRIEDADE, não um [RelayCommand] parametrizado).
        CopyAnnotationCommand.NotifyCanExecuteChanged();
        ReplicarProximaPaginaCommand.NotifyCanExecuteChanged();
    ...
    partial void OnIsSignedDocumentChanged(bool value)
    {
        ...
        DeleteSelectedAnnotationCommand.NotifyCanExecuteChanged();
        EditSelectedAnnotationCommand.NotifyCanExecuteChanged();
        // Task 4 (SDD desenho-caixa-melhorias): Paste/Replicar também compõem CanEdit (Copy não — ver
        // doc XML de CanCopySelectedAnnotation, é uma leitura pura).
        PasteAnnotationCommand.NotifyCanExecuteChanged();
        ReplicarProximaPaginaCommand.NotifyCanExecuteChanged();
    ...
    partial void OnIsXfaFormChanged(bool value)
    {
        ...
        DeleteSelectedAnnotationCommand.NotifyCanExecuteChanged();
        EditSelectedAnnotationCommand.NotifyCanExecuteChanged();
        // Task 4 (SDD desenho-caixa-melhorias): mesmo espelho de OnIsSignedDocumentChanged acima.
        PasteAnnotationCommand.NotifyCanExecuteChanged();
        ReplicarProximaPaginaCommand.NotifyCanExecuteChanged();
    ...
    public DocumentViewModel(
        ...
        // Plano 22: galeria de rubricas (imagens usadas como aparência do carimbo, escolhidas NO diálogo
        // de assinar). Default -> `RubricaGallery.DefaultDirectory` (isolado por MPDF_CONFIG_DIR em teste);
        // testes de assinatura injetam um diretório temporário próprio.
        RubricaGallery? rubricas = null,
        // Task 6 (SDD "Salvar como PDF/A"): seams da orquestração "Salvar como PDF/A" — mesmo padrão de
        ...
        Func<string, Task>? openSavedDocument = null)
    {
        ...
        _rubricaGallery = rubricas ?? new RubricaGallery(RubricaGallery.DefaultDirectory);
        ...
        // Task 6 (SDD "Salvar como PDF/A"): `_pdfADialog` é guardado CRU e resolvido no comando (não aqui);
        // ver doc XML do campo acima — resíduo inofensivo do estado interino da Task 6 (o default de
        // `UiPrompts.CreatePdfADialog` HOJE é `new PdfADialogService()`, que não lança).
        // `_pickPdfToSave`/`_writeAllBytes` NÃO passam pela seam UiPrompts (mesmo exemplar de
        // `DialogoImpressao.SalvarPdf_Click`, que também usa `new FileDialogService().PickPdfToSave(...)`/
        // `File.WriteAllBytes` direto) — só injetáveis pra teste, produção idêntica ao exemplar.
    ...
    private void OnSessionEditInFlightChanged(object? sender, EventArgs e)
    {
        ...
        DeleteSelectedAnnotationCommand.NotifyCanExecuteChanged();
        EditSelectedAnnotationCommand.NotifyCanExecuteChanged();
        // Task 4 (SDD desenho-caixa-melhorias): Paste/Replicar também compõem !Session.IsEditInFlight.
        PasteAnnotationCommand.NotifyCanExecuteChanged();
        ReplicarProximaPaginaCommand.NotifyCanExecuteChanged();
        ApplyFormValuesCommand.NotifyCanExecuteChanged();
    ...
    // Task 6 (Plano 3a): botões toggle-less da toolbar (🖍 Marca-texto, sublinhar, riscar; e a partir da
    // Task B, o contorno de forma) trocam a cor SELECIONADA — cada botão de cor é um comando PARAMETERLESS
    ...
    // InvalidCastException tentando um cast direto. Comandos nomeados são a via mais simples e mais
    ...
    //
    // Task B (SDD desenho-caixa-revisao): paleta ampliada de 3 pra 7 cores + "Nenhum" (`SelectColorNenhum`
    // -> `null`, só faz sentido no contexto de FORMA — ver doc XML de `SelectedMarkupColorArgb`), e os 8
    // comandos passaram de `void` pra `Task` — mesmo funil `SelectColorAsync` de `SelectFillAsync`
    // abaixo: além de setar o PADRÃO (`SelectedMarkupColorArgb`, sempre, mesmo sem seleção/CanEdit —
    // escolher a cor não é uma edição em si), se houver uma FORMA (Rectangle/Line/Arrow/Ink — as 4 únicas
    // com noção de contorno) já SELECIONADA e `CanEdit`, repinta ELA na hora via lift (mesma decisão de
    // produto do brief de `SelectFillAsync`: "sem seleção -> define o padrão; com forma selecionada ->
    // pinta aquela"). Diferente de `SelectFillAsync` (só Rectangle tem `FillArgb`), aqui o gate é um
    // ALLOWLIST de 4 kinds — todas têm `ColorArgb`/contorno.
    [RelayCommand] private Task SelectColorPreto() => SelectColorAsync(ColorPreto);
    [RelayCommand] private Task SelectColorBranco() => SelectColorAsync(ColorBranco);
    [RelayCommand] private Task SelectColorVermelho() => SelectColorAsync(ColorVermelho);
    [RelayCommand] private Task SelectColorLaranja() => SelectColorAsync(ColorLaranja);
    [RelayCommand] private Task SelectColorAmarelo() => SelectColorAsync(ColorAmarelo);
    [RelayCommand] private Task SelectColorVerde() => SelectColorAsync(ColorVerde);
    [RelayCommand] private Task SelectColorAzul() => SelectColorAsync(ColorAzul);
    [RelayCommand] private Task SelectColorNenhum() => SelectColorAsync(null);

    /// Funil comum dos 8 comandos `SelectColor*` acima — mesmo formato de `SelectFillAsync` abaixo, só
    /// que o ALLOWLIST de kinds liftáveis é as 4 formas (Rectangle/Line/Arrow/Ink), não só Rectangle
    /// (`FillArgb` é exclusivo de Rectangle; `ColorArgb`/contorno existe nas 4). Mesmo gate silencioso
    /// (sem `_notifyError`) fora de CanEdit/seleção/rotação — ver doc XML de `SelectFillAsync`.
    private async Task SelectColorAsync(uint? colorArgb)
    {
        SelectedMarkupColorArgb = colorArgb;
        if (!CanEdit) return;
        if (SelectedAnnotation is not
            { Id: not null, Kind: AnnotationKind.Rectangle or AnnotationKind.Line or AnnotationKind.Arrow or AnnotationKind.Ink } sel)
            return;
        if (IsPageRotated(sel.PageIndex)) return; // mesmo gate silencioso de SelectFillAsync
        await LiftSelectedAnnotationAsync(sel with { ColorArgb = colorArgb });
    }

    /// Task B (SDD desenho-caixa-revisao): 3 comandos parameterless de espessura de borda (brief:
    /// "fina/média/grossa") — mesmo funil `SelectEspessuraAsync` de `SelectColorAsync`/`SelectFillAsync`
    /// acima (padrão + lift na forma selecionada, mesmo ALLOWLIST de 4 kinds de `SelectColorAsync`: as
    /// 4 formas com contorno também são as 4 com espessura).
    [RelayCommand] private Task SelectEspessuraFina() => SelectEspessuraAsync(EspessuraFina);
    [RelayCommand] private Task SelectEspessuraMedia() => SelectEspessuraAsync(EspessuraMedia);
    [RelayCommand] private Task SelectEspessuraGrossa() => SelectEspessuraAsync(EspessuraGrossa);

    /// Funil comum dos 3 comandos `SelectEspessura*` acima — mesmo formato de `SelectColorAsync`.
    private async Task SelectEspessuraAsync(double strokeWidthPt)
    {
        SelectedStrokeWidthPt = strokeWidthPt;
        if (!CanEdit) return;
        if (SelectedAnnotation is not
            { Id: not null, Kind: AnnotationKind.Rectangle or AnnotationKind.Line or AnnotationKind.Arrow or AnnotationKind.Ink } sel)
            return;
        if (IsPageRotated(sel.PageIndex)) return; // mesmo gate silencioso de SelectFillAsync/SelectColorAsync
        await LiftSelectedAnnotationAsync(sel with { StrokeWidthPt = strokeWidthPt });
    }

    /// Task 2 (SDD desenho-caixa-melhorias): 4 comandos parameterless espelhando `SelectColorAmarelo/
    /// Verde/Vermelho` acima (mesmo motivo: `CommandParameter` chegaria como string crua em XAML) — só
    /// que estes trocam `SelectedFillColorArgb` (o PADRÃO pra próxima caixa desenhada, ver
    /// `CommitDrawingAsync`) E, se houver uma `Rectangle` já SELECIONADA, também repintam ELA na hora
    /// (decisão de produto do brief: "sem seleção -> define o padrão; com caixa selecionada -> pinta
    /// aquela"). `SelectFillNenhum` é o 4º botão ("Nenhum"/□) — `Rectangle` é o único kind com noção de
    /// preenchimento (Task 1), então os outros 3 kinds nunca aparecem aqui por construção (`SelectedAnnotation`
    /// já viria com `FillArgb` sempre `null` se não fosse Rectangle).
    [RelayCommand] private Task SelectFillNenhum() => SelectFillAsync(null);
    [RelayCommand] private Task SelectFillPreto() => SelectFillAsync(ColorPreto);
    [RelayCommand] private Task SelectFillBranco() => SelectFillAsync(ColorBranco);
    [RelayCommand] private Task SelectFillAmarelo() => SelectFillAsync(ColorAmarelo);
    [RelayCommand] private Task SelectFillVerde() => SelectFillAsync(ColorVerde);
    [RelayCommand] private Task SelectFillVermelho() => SelectFillAsync(ColorVermelho);
    [RelayCommand] private Task SelectFillLaranja() => SelectFillAsync(ColorLaranja);
    [RelayCommand] private Task SelectFillAzul() => SelectFillAsync(ColorAzul);

    /// Funil comum dos 4 comandos `SelectFill*` acima. `SelectedFillColorArgb` é setado SEMPRE (mesmo
    /// sem seleção, mesmo se `CanEdit` for falso/página girada — escolher a cor-padrão não é uma edição
    /// do documento em si, mesmo espírito de `SelectColorAmarelo/Verde/Vermelho`, que também não gateiam
    /// em `CanEdit`). O lift na caixa selecionada é que É uma edição de verdade — gateado exatamente como
    /// os outros pontos de escrita deste VM (`CanEdit`, `IsPageRotated`, mesmo padrão de
    /// `MoveSelectedAnnotationAsync`): no-op silencioso (sem `_notifyError`) fora desses casos, porque o
    /// comando SEMPRE teve efeito (mudou o padrão) — não é uma falha, só não havia caixa válida pra repintar.
    private async Task SelectFillAsync(uint? fillArgb)
    {
        SelectedFillColorArgb = fillArgb;
        if (!CanEdit) return;
        if (SelectedAnnotation is not { Id: not null, Kind: AnnotationKind.Rectangle } sel) return;
        if (IsPageRotated(sel.PageIndex)) return; // mesmo gate silencioso de MoveSelectedAnnotationAsync
        await LiftSelectedAnnotationAsync(sel with { FillArgb = fillArgb });
    }
    ...
    private async Task ApplyMarkup(AnnotationKind kind)
    {
        ...
        try
        {
            ...
            var data = new AnnotationData
            {
                ...
                Quads = quads,
                // Marcação (Highlight/Underline/Strikeout) SEMPRE recebe cor visível: o swatch "Contorno:
                // nenhum" (SelectedMarkupColorArgb==null) existe pra ZERAR a BORDA de uma FORMA — não faz
                // sentido num realce (ficaria invisível). Se o padrão estiver em "nenhum", cai no amarelo.
                ColorArgb = SelectedMarkupColorArgb ?? ColorAmarelo,
    ...
    public void ToggleStampTool(byte[] imageBytes)
    {
        ...
        // Plano 21 (Task 5): a GALERIA continua com clique único (tamanho natural) — carimbos prontos,
        // repetidos, onde arrastar seria fricção. O ajuste de tamanho/posição fica pra DEPOIS (clicar no
        // carimbo abre a caixa ajustável — os bytes são cacheados no PlaceStampAtAsync).
        PendingImageUsesBox = false;
        ActiveTool = AnnotationTool.ImageStamp;
    }

    /// Plano 21 (Task 5): a View lê isto no mouse-down com `ActiveTool == ImageStamp` pra decidir entre
    /// a caixa ajustável (`true` — "Inserir imagem", desenha tamanho+posição) e o clique único de tamanho
    /// natural (`false` — galeria de carimbos). Setado por `ToggleImageTool`/`ToggleStampTool`.
    public bool PendingImageUsesBox { get; private set; }
    ...
    public async Task PlaceStampAtAsync(int pageIndex, double xPt, double yPt)
    {
        ...
        try
        {
            ...
            var prevIds = CurrentImageStampIds(); // Plano 21: base do set-diff pra cachear os bytes
            var data = new AnnotationData
            {
            ...
            ActiveTool = AnnotationTool.None;
            _pendingStampBytes = null;
            // Plano 21 (Task 5): cacheia os bytes sob o Id novo — permite mover/redimensionar depois
            // (clicar no carimbo abre a caixa de edição), mesmo pros carimbos de galeria.
            await CacheNewImageStampBytesAsync(prevIds, bytes);
        }
        finally { Session.EndEdit(); }
    }
    ...
    private void ToggleImageTool()
    {
        ...
        // Plano 21 (Task 5): "Inserir imagem" usa a CAIXA AJUSTÁVEL (desenhar tamanho + reposicionar) —
        // é justamente o pedido do usuário de editar tamanho/posição ao adicionar.
        PendingImageUsesBox = true;
        ActiveTool = AnnotationTool.ImageStamp;
    }
    ...
    // Plano 25: abre o diálogo de imposição (N-up / livreto / escala, com prévia) — "Salvar como PDF"
    // aplica a imposição ao documento e grava um PDF novo; "Imprimir" manda pra impressora. Reusa o
    // mesmo diálogo do comando Imprimir (Ctrl+P); aqui é o ponto de entrada a partir do Organizador.
    [RelayCommand]
    private void ImporEmFolhas() => mPdf.App.Services.PrintService.Print(this);
    ...
    public async Task CommitDrawingAsync(int pageIndex, IReadOnlyList<PdfPoint> pathPt)
    {
        ...
        try
        {
            ...
            var data = new AnnotationData
            {
                ...
                // Task 2 (SDD desenho-caixa-melhorias): só Rectangle carrega o preenchimento-padrão da
                // toolbar (`SelectedFillColorArgb`) — Ink/Line/Arrow sempre `null` (Task 1: `FillArgb`
                // só tem efeito em `PdfEditor.BuildAnnotation` pra `AnnotationKind.Rectangle` mesmo, mas
                // o contrato aqui é explícito por construção, não por confiar no editor ignorar o campo).
                FillArgb = kind == AnnotationKind.Rectangle ? SelectedFillColorArgb : null,
                // Task B (SDD desenho-caixa-revisao): as 4 formas (`kind` só chega aqui como Ink/
                // Rectangle/Line/Arrow — ver switch em `ActiveTool` acima) carregam a espessura-padrão da
                // toolbar; `null` delega ao default do motor (`PdfEditor.StrokeWidthPt` const, 2pt — ver
                // doc XML de `SelectedStrokeWidthPt`).
                StrokeWidthPt = SelectedStrokeWidthPt,
                Author = _config.Autor,
            };
    ...
    // ==== Task 4 (SDD desenho-caixa-melhorias): copiar/colar anotação + replicar na próxima página =====

    /// ImageStamp fora (v1, brief): `ReadAnnotations` NUNCA devolve `ImageBytes` de volta (sentinela
    /// documentada em `AnnotationData.ImageBytes` — só a leitura fica sem os bytes, mesmo pra um carimbo
    /// recém-criado por este módulo), então um clone/réplica sem os bytes originais não tem como
    /// reconstruir a appearance da imagem — mesma razão que já torna ImageStamp NÃO-liftável em
    /// `MoveSelectedAnnotationAsync`. Não compõe `CanEdit`: copiar é uma leitura pura (não muta o PDF),
    /// mesmo espírito de `SelectFillNenhum`/seletores de cor acima (que também não gateiam em `CanEdit`)
    /// — só colar/replicar (que ESCREVEM) compõem `CanEdit`.
    private bool CanCopySelectedAnnotation() =>
        SelectedAnnotation is { Kind: not AnnotationKind.ImageStamp };

    /// Clona `SelectedAnnotation` pro clipboard interno (`with { Id = null }` — o `record` copia TODOS
    /// os demais campos: Kind, retângulo, ColorArgb/FillArgb, Content, Quads, InkStrokes, LineStart/
    /// EndPt, Author, formatação de FreeText). `PasteAnnotationCommand` pode ter ficado com `CanExecute`
    /// falso até agora (clipboard vazio) — reavalia explicitamente aqui, já que `_annotationClipboard`
    /// não é um `[ObservableProperty]` (é um campo puro, sem notificação automática).
    [RelayCommand(CanExecute = nameof(CanCopySelectedAnnotation))]
    private void CopyAnnotation()
    {
        if (SelectedAnnotation is not { Kind: not AnnotationKind.ImageStamp } sel) return;
        _annotationClipboard = sel with { Id = null };
        PasteAnnotationCommand.NotifyCanExecuteChanged();
    }

    private bool CanPasteAnnotation() =>
        _annotationClipboard is not null && CanEdit && !Session.IsEditInFlight;

    /// Cola o clipboard na MESMA página de origem (`clip.PageIndex`), deslocado +`PasteOffsetPt` em X/Y
    /// (clampado aos limites da página, `ClampToPage` — mesmo helper de todo outro ponto de escrita deste
    /// VM) — Line/Arrow (`LineStartPt`/`LineEndPt`) e Ink (`InkStrokes`) têm TODOS os pontos deslocados
    /// pelo MESMO delta resultante do clamp (`Translate`/`TranslateStrokes`, mesma família usada por
    /// `MoveSelectedAnnotationAsync`), não só o bbox — mesma razão documentada lá (PDFium sintetiza a
    /// aparência a partir dos pontos, não do `/Rect`). Colar de novo cola outra cópia a partir do MESMO
    /// clipboard (sempre +12pt do ORIGINAL copiado, não acumulado sobre a última colada) — aceito
    /// (brief: "colar de novo cola outra cópia, ok").
    ///
    /// Id NOVO gerado AQUI (não deixado `null` pro editor gerar um GUID internamente): decisão de
    /// implementação — assim o VM já sabe o Id da anotação recém-colada e pode SELECIONÁ-LA sem precisar
    /// reler o PDF de volta. `AddAnnotation` aceita um Id explícito e valida unicidade dentro do MESMO
    /// documento (ver doc XML de `AnnotationData.Id`) — um GUID novo nunca colide. A atribuição a
    /// `SelectedAnnotation` abaixo, DEPOIS de `TryApplyEdit` ter sucesso, é quem vale: `Session.ApplyEdit`
    /// já disparou `Session.Applied` SÍNCRONO (`Apply -> Applied?.Invoke`) e `OnSessionApplied` já limpou
    /// a seleção antiga (mesmo ACHADO documentado em `ApplyMarkup`) ANTES desta linha rodar.
    [RelayCommand(CanExecute = nameof(CanPasteAnnotation))]
    private async Task PasteAnnotation()
    {
        if (_annotationClipboard is not { } clip) return;
        if (!Session.TryBeginEdit()) return;
        try
        {
            await EnsureRotationCacheFreshAsync();
            int pageIndex = clip.PageIndex;
            if (pageIndex < 0 || pageIndex >= Pages.Count) return; // página do clipboard não existe mais
            if (IsPageRotated(pageIndex)) { _notifyError(RotatedPageNotice); return; }

            var page = Pages[pageIndex];
            double w = clip.RightPt - clip.LeftPt, h = clip.TopPt - clip.BottomPt;
            var (left, bottom, right, top) = ClampToPage(
                clip.LeftPt + PasteOffsetPt, clip.BottomPt + PasteOffsetPt, w, h, page.WidthPt, page.HeightPt);
            double dx = left - clip.LeftPt, dy = bottom - clip.BottomPt;
            ...
            var data = clip with
            {
                ...
                PageIndex = pageIndex,
                LeftPt = left, BottomPt = bottom, RightPt = right, TopPt = top,
                Quads = TranslateQuads(clip.Quads, dx, dy),
                InkStrokes = TranslateStrokes(clip.InkStrokes, dx, dy),
                LineStartPt = Translate(clip.LineStartPt, dx, dy),
                LineEndPt = Translate(clip.LineEndPt, dx, dy),
            };

            byte[]? pdfDepois = await TryAddAnnotationAsync(data);
            if (pdfDepois is null) return; // falha tipada, já notificada dentro do helper
            if (!TryApplyEdit(pdfDepois)) return;
            SelectedAnnotation = data; // seleciona a colada — ver doc XML acima (pós-Apply é seguro)
        }
        finally { Session.EndEdit(); }
    }

    /// Deslocamento (pt) aplicado pelo colar — brief: "~12pt em X e Y".
    private const double PasteOffsetPt = 12.0;

    private bool CanReplicarProximaPagina() =>
        SelectedAnnotation is { Kind: not AnnotationKind.ImageStamp } && CanEdit && !Session.IsEditInFlight;

    /// pt-BR (brief) — avisos específicos de `ReplicarProximaPagina` (distintos de `RotatedPageNotice`,
    /// que fala da página de ORIGEM; aqui a checada é sempre a página de DESTINO).
    private const string NoNextPageNotice = "Não há próxima página.";
    private const string RotatedNextPageNotice = "A próxima página está girada; não é possível replicar.";

    /// Cria uma cópia de `SelectedAnnotation` na página seguinte (`PageIndex + 1`), com o MESMO
    /// retângulo/pontos (nenhum deslocamento — diferente de `PasteAnnotation` — só clampado ao tamanho
    /// da página DESTINO, que pode ser diferente da origem). Recusa (aviso pt-BR, no-op) se não houver
    /// próxima página, ou se a destino estiver girada (`IsPageRotated`, MESMO gate/MESMA costura de
    /// rotação dos outros pontos de escrita — checa EXISTÊNCIA antes de checar ROTAÇÃO, ordem que o
    /// brief pede). Mesmo truque de Id novo gerado aqui + seleção pós-Apply de `PasteAnnotation` (ver doc
    /// XML lá) — aqui a réplica também dispara `ScrollToPageRequested` pra a página destino ficar visível
    /// (opcional no brief; feito porque o usuário provavelmente quer CONFERIR a réplica).
    [RelayCommand(CanExecute = nameof(CanReplicarProximaPagina))]
    private async Task ReplicarProximaPagina()
    {
        if (SelectedAnnotation is not { Kind: not AnnotationKind.ImageStamp } sel) return;
        if (!Session.TryBeginEdit()) return;
        try
        {
            await EnsureRotationCacheFreshAsync();
            int destIndex = sel.PageIndex + 1;
            if (destIndex < 0 || destIndex >= Pages.Count) { _notifyError(NoNextPageNotice); return; }
            if (IsPageRotated(destIndex)) { _notifyError(RotatedNextPageNotice); return; }

            var destPage = Pages[destIndex];
            double w = sel.RightPt - sel.LeftPt, h = sel.TopPt - sel.BottomPt;
            var (left, bottom, right, top) = ClampToPage(sel.LeftPt, sel.BottomPt, w, h, destPage.WidthPt, destPage.HeightPt);
            double dx = left - sel.LeftPt, dy = bottom - sel.BottomPt;
            ...
            var data = sel with
            {
                ...
                PageIndex = destIndex,
                LeftPt = left, BottomPt = bottom, RightPt = right, TopPt = top,
                Quads = TranslateQuads(sel.Quads, dx, dy),
                InkStrokes = TranslateStrokes(sel.InkStrokes, dx, dy),
                LineStartPt = Translate(sel.LineStartPt, dx, dy),
                LineEndPt = Translate(sel.LineEndPt, dx, dy),
            };

            byte[]? pdfDepois = await TryAddAnnotationAsync(data);
            if (pdfDepois is null) return; // falha tipada, já notificada dentro do helper
            if (!TryApplyEdit(pdfDepois)) return;
            SelectedAnnotation = data; // seleciona a réplica — ver doc XML de PasteAnnotation
            ScrollToPageRequested?.Invoke(destIndex);
        }
        finally { Session.EndEdit(); }
    }
    ...
    // ---- Task 6 (SDD "Salvar como PDF/A"): orquestração "Salvar como PDF/A" -------------------------
    //
    // LEITURA PURA + arquivo NOVO (mesma classe de ExportImage/ExportDocumentCoreAsync acima) — SEM
    // TryBeginEdit/ApplyEdit: a conversão nunca toca o documento aberto, só lê o snapshot e escreve um
    // PDF/A separado. Por isso funciona em documento ASSINADO também (o resultado é avisado como
    // NÃO-assinado — assinar de novo é responsabilidade de um passo posterior do usuário).
    //
    // Fluxo: pergunta nível+pesquisável (diálogo, Task 7) -> rasteriza cada página (T2, mesmo rasterizer
    // de OCR) -> BGRA->JPEG (T5) -> se `manterPesquisavel` e a página NÃO tem texto, roda OCR (T1) e
    // monta as caixas invisíveis (mesmo mapeamento de `RunOcr` acima) -> `_editor.CriarPdfADeImagens`
    // (T2-4) -> Salvar como... -> grava. Progresso/cancelamento: mesma seam `_ocrProgress` do OCR.
    //
    // FALHA DE RENDER (uma página) ABORTA a conversão inteira (propaga) — um PDF/A com página faltando
    // seria inválido. FALHA DE OCR numa página NÃO aborta — a página entra só como imagem (mesmo
    // espírito de `RunOcr`, que conta falhas e segue, só que aqui nem conta: é best-effort silencioso).
    [RelayCommand]
    private async Task SalvarComoPdfA()
    {
        // Resolução do default aqui (não no ctor/campo, por ora) — ver doc XML de `_pdfADialog`.
        var dialog = _pdfADialog ?? UiPrompts.CreatePdfADialog();
        var escolha = dialog.Perguntar(); // null = usuário cancelou
        if (escolha is not { } e) return;
        await SalvarComoPdfACoreAsync(e.Nivel, e.ManterPesquisavel);
    }

    /// `internal` para os testes exercitarem a orquestração direto (mesmo precedente de
    /// `RecognizeTextCoreAsync`/`ExportDocumentCoreAsync`), bypassando `_pdfADialog`.
    internal async Task SalvarComoPdfACoreAsync(PdfANivel nivel, bool manterPesquisavel)
    {
        byte[] pdf = Session.Snapshot; // capturado ANTES do 1º await (coerência de instantâneo)
        string baseName = Path.GetFileNameWithoutExtension(Session.FileName);
        try
        {
            using (var progresso = _ocrProgress.Start())
            {
                var token = progresso.Token;
                var progress = progresso.Progress;
                byte[]? fonte = manterPesquisavel ? PdfAConversao.FonteInvisivelTtf() : null;
                var paginas = await Task.Run(() => RasterizarParaPdfA(pdf, manterPesquisavel, progress, token));
                if (token.IsCancellationRequested) return; // cancelado — nada gravado

                byte[] saida = await Task.Run(() => _editor.CriarPdfADeImagens(
                    paginas, nivel, new PdfAMetadados(baseName, null), fonte));

                var destino = _pickPdfToSave($"{baseName} (PDF-A).pdf");
                if (destino is null) return; // usuário cancelou o "Salvar como…"
                _writeAllBytes(destino, saida);

                string msg = "PDF/A gerado.";
                if (IsSignedDocument)
                    msg += " Atenção: o arquivo gerado NÃO está assinado (para PDF/A assinado, gere e depois assine).";
                _notifyInfo(msg);
            }
        }
        catch (Exception ex) { _notifyError($"Não foi possível gerar o PDF/A: {ex.Message}"); }
    }

    /// Rasteriza cada página do documento (T2) para compor o PDF/A: BGRA->JPEG (T5) + tamanho real em pt
    /// (`GetPageSize`, Task 6) + opcionalmente OCR (T1) nas páginas SEM texto extraível. Fora da UI
    /// thread (`Task.Run` do chamador) — mesmo padrão de `RunOcr`/`DetermineOcrTargetPages` acima.
    private List<PdfAPaginaImagem> RasterizarParaPdfA(
        byte[] pdf, bool manterPesquisavel, IProgress<OcrProgress> progress, CancellationToken token)
    {
        var lista = new List<PdfAPaginaImagem>();
        using var raster = _rasterizerFactory(pdf);
        IOcrEngine? engine = manterPesquisavel ? GetOcrEngine() : null;
        for (int i = 0; i < raster.PageCount; i++)
        {
            if (token.IsCancellationRequested) break;
            progress.Report(new OcrProgress(i + 1, raster.PageCount));

            var pg = raster.RasterizeForOcr(i); // falha aqui PROPAGA e aborta (página faltando = PDF/A inválido)
            var tamanho = raster.GetPageSize(i);
            var jpeg = PdfAConversao.BgraParaJpegRgb(pg.Bgra, pg.WidthPx, pg.HeightPx);

            IReadOnlyList<OcrTextBox>? caixas = null;
            if (engine is not null && !raster.PaginaTemTexto(i))
            {
                // OCR de UMA página que falha NÃO aborta a conversão — a página entra só como imagem
                // (mesmo espírito de RunOcr, que conta falhas e segue; aqui nem conta, é best-effort).
                try
                {
                    var res = engine.Recognize(pg.Bgra, pg.WidthPx, pg.HeightPx, OcrLanguages);
                    var boxes = new List<OcrTextBox>();
                    foreach (var w in res.Words)
                        if (!string.IsNullOrWhiteSpace(w.Text))
                            boxes.Add(new OcrTextBox(w.Text, w.LeftPx, w.TopPx, w.WidthPx, w.HeightPx));
                    if (boxes.Count > 0) caixas = boxes;
                }
                catch { /* página sem camada de texto — segue só como imagem */ }
            }

            lista.Add(new PdfAPaginaImagem(jpeg, tamanho.WidthPt, tamanho.HeightPt, pg.WidthPx, pg.HeightPx, caixas));
        }
        return lista;
    }
    ...
    // Plano 21: `RubricaBytes` != null quando o usuário escolheu "Minha rubrica" no diálogo — a
    ...
    partial void OnActiveToolChanged(AnnotationTool value)
    {
        ...
        // Fix v2.12.2 (pedido do usuário): a caixa (Retângulo) "vem com padrão SEM borda + preenchimento
        // BRANCO". Como a ferramenta é one-shot (desativa após desenhar, ver CommitDrawingAsync), ativar o
        // Retângulo re-aplica esse padrão a cada caixa — se o usuário quiser borda/outra cor, escolhe nos
        // seletores depois de ativar a ferramenta e antes de desenhar.
        if (value == AnnotationTool.Rectangle)
        {
            SelectedMarkupColorArgb = null;       // sem borda (ColorArgb null -> /Border largura 0, ver PdfEditor)
            SelectedFillColorArgb = ColorBranco;  // preenchimento branco
        }
        // Linha/Seta/Ink PRECISAM de traço visível — se o padrão ficou "sem cor" (ex.: vindo do preset da
        // caixa acima), restaura uma cor visível pra não desenhar um traço invisível.
        else if (value is AnnotationTool.Line or AnnotationTool.Arrow or AnnotationTool.Ink
                 && SelectedMarkupColorArgb is null)
        {
            SelectedMarkupColorArgb = ColorVermelho;
        }
    }
    ...
    private async Task Sign()
    {
        ...
        // Plano 22: "Minha rubrica" abre a galeria NO diálogo (escolher/adicionar/remover) — o diálogo
        // devolve os bytes da rubrica escolhida em `result.RubricaBytes`.
        var result = _signDialog.PromptForSignature(
    ...
    /// Plano 22: abre o seletor de imagem e valida como rubrica (mesmos critérios de `ToggleImageTool`:
    /// PNG/JPG, ≤50MP, não CMYK), notificando o erro em pt-BR e devolvendo os bytes prontos (ou `null` se
    /// cancelado/rejeitado). Passado ao diálogo de assinar como o callback do "+" da galeria — o diálogo
    /// adiciona o resultado à `RubricaGallery`.
    private byte[]? PickAndValidateRubrica()
    {
        if (_dialogs.PickImageToImport() is not { } path) return null; // cancelado
        byte[] bytes;
        try { bytes = File.ReadAllBytes(path); }
        catch (IOException ex) { _notifyError(ex.Message); return null; }
    ...
    /// mouse-down quando `ActiveTool == ImageStamp` (substitui o clique único `PlaceStampAtAsync`). Mesmo
    /// gate de rotação de `BeginStampBoxPlacementAsync`; a prévia da caixa é a PRÓPRIA imagem, o rótulo do
    /// confirmar vira "Inserir aqui". O commit real (adicionar a anotação) só acontece no Confirmar
    /// (`ConfirmStampBoxAsync` -> `ConfirmImageBoxAsync`).
    public async Task BeginImageBoxPlacementAsync(int pageIndex, PdfPoint startPt)
    {
        if (ActiveTool != AnnotationTool.ImageStamp) return;
        if (_pendingStampBytes is not { Length: > 0 } bytes) return;
        if (pageIndex < 0 || pageIndex >= Pages.Count) return;
        ...
        _stampBoxImageBytes = bytes;
        _editingAnnotationId = null;
        ImageSource? preview = TryDecodeFrozenImage(bytes);
        BeginStampBoxPlacement(pageIndex, startPt, certificateCn: "", preview,
            confirmLabel: "Inserir aqui", purpose: StampBoxPurpose.Imagem);
    }

    /// Plano 21 (Task 5): confirmar UNIFICADO do cartão flutuante — a View chama SEMPRE este método; ele
    /// roteia pro desfecho certo conforme o propósito da caixa (assinatura/rubrica -> motor; imagem nova
    /// -> adiciona anotação; edição de imagem -> lift Remove+Add). Mantém `ConfirmSignatureStampAsync`
    /// intacto (o caminho de assinatura), só encapsula o roteamento.
    public Task ConfirmStampBoxAsync() => _stampBoxPurpose switch
    {
        StampBoxPurpose.Imagem => ConfirmImageBoxAsync(),
        StampBoxPurpose.EdicaoImagem => ConfirmImageEditBoxAsync(),
        StampBoxPurpose.EdicaoForma => ConfirmShapeEditBoxAsync(),
        _ => ConfirmSignatureStampAsync(),
    };
    ...
    // ==== Plano 21 (Task 5): imagem pela caixa ajustável (colocar + redimensionar/mover depois) =======

    /// Confirmar de uma IMAGEM NOVA (propósito Imagem) — adiciona a anotação `ImageStamp` no rect FINAL
    /// da caixa (aspect-fit, sem distorção). Captura os bytes ANTES de `ConfirmStampBox()` (que reseta o
    /// estado da caixa).
    private async Task ConfirmImageBoxAsync()
    {
        byte[]? bytes = _stampBoxImageBytes;
        if (ConfirmStampBox() is not { } placement) return; // fora de Adjusting -- nada a confirmar
        if (bytes is not { Length: > 0 }) return;
        await PlaceImageStampFromBoxAsync(placement, bytes);
    }

    /// Adiciona a imagem como anotação `ImageStamp` no rect final (aspect-fit dentro da caixa desenhada,
    /// centralizado — mesma proporção que a prévia `Stretch=Uniform` mostrou). Mesmo funil/gate de
    /// `PlaceStampAtAsync`; ao ter sucesso, CACHEIA os bytes sob o Id da anotação recém-criada (set-diff),
    /// pra permitir mover/redimensionar depois (ver `MoveSelectedAnnotationAsync`/`BeginImageEditBox`).
    private async Task PlaceImageStampFromBoxAsync(StampBoxPlacement placement, byte[] bytes)
    {
        if (placement.PageIndex < 0 || placement.PageIndex >= Pages.Count) return;
        if (!Session.TryBeginEdit()) return;
        try
        {
            if (AspectFitWithinBox(bytes, placement.Rect) is not { } rect)
            { _notifyError("Não foi possível ler esta imagem."); return; }

            var prevIds = CurrentImageStampIds();
            var data = new AnnotationData
            {
                Kind = AnnotationKind.ImageStamp, PageIndex = placement.PageIndex,
                LeftPt = rect.LeftPt, BottomPt = rect.BottomPt, RightPt = rect.RightPt, TopPt = rect.TopPt,
                ImageBytes = bytes, Author = _config.Autor,
            };
            ...
            ActiveTool = AnnotationTool.None;
            _pendingStampBytes = null;
            _stampBoxImageBytes = null;
            await CacheNewImageStampBytesAsync(prevIds, bytes);
        }
        finally { Session.EndEdit(); }
    }

    /// Plano 21 (Task 5): abre a caixa ajustável SOBRE uma imagem JÁ colocada (mover/redimensionar) —
    /// chamada pela View quando o usuário clica numa anotação `ImageStamp`. Entra DIRETO em Adjusting
    /// (não Drawing) no rect atual da anotação, com a imagem cacheada como prévia. Sem os bytes no cache
    /// (documento reaberto) avisa e não abre — a imagem continua no PDF, só não editável nesta sessão.
    public void BeginImageEditBox(AnnotationData target)
    {
        if (target.Kind != AnnotationKind.ImageStamp || target.Id is not { } id) return;
        if (target.PageIndex < 0 || target.PageIndex >= Pages.Count) return;
        if (!_imageStampBytes.TryGetValue(id, out var bytes))
        {
            _notifyError("Esta imagem foi inserida em outra sessão — recoloque-a para redimensionar ou mover.");
            return;
        }
        if (IsPageRotated(target.PageIndex)) { _notifyError(RotatedPageNotice); return; }

        StampBoxPageIndex = target.PageIndex;
        StampBoxCertificateCn = null;
        StampBoxDateLabel = null;
        _stampBoxPreviewImage = TryDecodeFrozenImage(bytes);
        _stampBoxConfirmLabel = "Salvar";
        _stampBoxPurpose = StampBoxPurpose.EdicaoImagem;
        _stampBoxImageBytes = bytes;
        _editingAnnotationId = id;
        _boxXa = target.LeftPt; _boxXb = target.RightPt; _boxYa = target.BottomPt; _boxYb = target.TopPt;
        StampBoxNotice = null;
        RecomputeStampBoxRect();
        StampPlacementPhase = StampPlacementPhase.Adjusting;
    }

    /// Confirmar de uma EDIÇÃO de imagem (propósito EdicaoImagem) — faz lift (Remove+Add) da anotação
    /// selecionada pro novo rect (aspect-fit), reusando `LiftSelectedAnnotationAsync` (mesma mecânica de
    /// mover/editar qualquer anotação), com os bytes do cache. Re-mapeia o cache pro Id NOVO (o lift cria
    /// uma anotação nova). Captura Id/bytes ANTES de `ConfirmStampBox()`.
    private async Task ConfirmImageEditBoxAsync()
    {
        string? editId = _editingAnnotationId;
        byte[]? bytes = _stampBoxImageBytes;
        if (ConfirmStampBox() is not { } placement) return;
        if (editId is null || bytes is not { Length: > 0 }) return;
        if (SelectedAnnotation is not { Id: not null } sel || sel.Id != editId) return; // seleção mudou

        if (AspectFitWithinBox(bytes, placement.Rect) is not { } rect)
        { _notifyError("Não foi possível ler esta imagem."); return; }

        var prevIds = CurrentImageStampIds();
        bool applied = await LiftSelectedAnnotationAsync(sel with
        {
            LeftPt = rect.LeftPt, BottomPt = rect.BottomPt, RightPt = rect.RightPt, TopPt = rect.TopPt,
            ImageBytes = bytes,
        });
        if (applied) await RecacheAfterImageEditAsync(prevIds, editId, bytes);
    }

    // ==== Task 3 (SDD desenho-caixa-melhorias): redimensionar/mover uma FORMA (Rectangle) já colocada ==

    /// Abre a caixa ajustável SOBRE uma `Rectangle` JÁ colocada (mover/redimensionar) — chamada pela View
    /// quando o usuário dá duplo-clique numa anotação `Kind==Rectangle` (mesmo gatilho/hit-test de
    /// `BeginImageEditBox`, ver `PdfViewerControl.xaml.cs`). Entra DIRETO em Adjusting (não Drawing) no
    /// rect atual da anotação. DIFERENTE de `BeginImageEditBox`: sem bytes/cache — uma `Rectangle` é
    /// TOTALMENTE reconstruível a partir de `ReadAnnotations` (Left/Bottom/Right/Top + ColorArgb/FillArgb/
    /// Author/Id), então o único estado próprio que este modo precisa guardar é o Id sendo editado
    /// (`_editingAnnotationId` — mesmo campo de `BeginImageEditBox`, RENOMEADO de `_editingImageAnnotationId`
    /// pra cobrir os 2 modos; nunca os dois ativos ao mesmo tempo, `_stampBoxPurpose` decide qual). Gate
    /// `CanEdit` (silencioso — mesmo espírito do gate de `SelectFillAsync`/`MoveSelectedAnnotationAsync`:
    /// documento assinado não abre a caixa) + `IsPageRotated` (avisa, mesmo padrão de `BeginImageEditBox`).
    public void BeginShapeEditBox(AnnotationData target)
    {
        if (target.Kind != AnnotationKind.Rectangle || target.Id is not { } id) return;
        if (target.PageIndex < 0 || target.PageIndex >= Pages.Count) return;
        if (!CanEdit) return;
        if (IsPageRotated(target.PageIndex)) { _notifyError(RotatedPageNotice); return; }
        ...
        StampBoxCertificateCn = null;
        StampBoxDateLabel = null;
        _stampBoxPreviewImage = null;
        _stampBoxConfirmLabel = "Aplicar";
        _stampBoxPurpose = StampBoxPurpose.EdicaoForma;
        _stampBoxImageBytes = null;
        _editingAnnotationId = id;
        _boxXa = target.LeftPt; _boxXb = target.RightPt; _boxYa = target.BottomPt; _boxYb = target.TopPt;
        StampBoxNotice = null;
        // Task D (SDD desenho-caixa-revisao): captura o estilo CORRENTE da caixa nas 3 propriedades de
        // EDIÇÃO (contorno/preenchimento/espessura) — a barrinha Aplicar/Cancelar (XAML) liga os swatches
        // ampliados (Task B) a ESTAS propriedades, não às `SelectedMarkupColorArgb`/`SelectedFillColorArgb`/
        // `SelectedStrokeWidthPt` de cima (que são o PADRÃO pra próxima forma desenhada, contexto
        // diferente). `EmEdicaoDeForma` (abaixo) vira `true` já nesta chamada via `OnStampPlacementPhaseChanged`
        // (dispara no `StampPlacementPhase = Adjusting` no fim deste método, DEPOIS de `_stampBoxPurpose`
        // já estar em `EdicaoForma` — ordem importa pro cálculo).
        EdicaoCorArgb = target.ColorArgb;
        EdicaoFillArgb = target.FillArgb;
        EdicaoEspessuraPt = target.StrokeWidthPt;
        RecomputeStampBoxRect();
        StampPlacementPhase = StampPlacementPhase.Adjusting;
    }

    // Task D (SDD desenho-caixa-revisao): 8 comandos parameterless de CONTORNO da barrinha de edição de
    // forma — mesmo motivo dos `SelectColor*` (Step 4 acima) de não usar `CommandParameter` (chegaria
    // como string crua em XAML). DIFERENTES de `SelectColor*`: não têm efeito nenhum fora de
    // `EmEdicaoDeForma` (a barrinha só aparece nesse modo, mesma disciplina de
    // `StampBoxConfirm_Click`/`StampBoxCancel_Click` — "a visibilidade já é o gate") e NÃO fazem lift —
    // só setam `EdicaoCorArgb`, que só é gravado na anotação no Aplicar (`ConfirmShapeEditBoxAsync`
    // abaixo). "Nenhum" (`EdicaoContornoNenhum`) grava `ColorArgb = null` (mesma regra da Task A: sem
    // cor de contorno = sem borda no motor).
    [RelayCommand] private void EdicaoContornoPreto() => EdicaoCorArgb = ColorPreto;
    [RelayCommand] private void EdicaoContornoBranco() => EdicaoCorArgb = ColorBranco;
    [RelayCommand] private void EdicaoContornoVermelho() => EdicaoCorArgb = ColorVermelho;
    [RelayCommand] private void EdicaoContornoLaranja() => EdicaoCorArgb = ColorLaranja;
    [RelayCommand] private void EdicaoContornoAmarelo() => EdicaoCorArgb = ColorAmarelo;
    [RelayCommand] private void EdicaoContornoVerde() => EdicaoCorArgb = ColorVerde;
    [RelayCommand] private void EdicaoContornoAzul() => EdicaoCorArgb = ColorAzul;
    [RelayCommand] private void EdicaoContornoNenhum() => EdicaoCorArgb = null;

    // Task D: mesmo formato acima, pro PREENCHIMENTO (`EdicaoFillArgb`) — "Nenhum" primeiro (mesma ordem
    // visual de `SelectFillNenhum` acima), depois as 7 cores.
    [RelayCommand] private void EdicaoPreenchNenhum() => EdicaoFillArgb = null;
    [RelayCommand] private void EdicaoPreenchPreto() => EdicaoFillArgb = ColorPreto;
    [RelayCommand] private void EdicaoPreenchBranco() => EdicaoFillArgb = ColorBranco;
    [RelayCommand] private void EdicaoPreenchVermelho() => EdicaoFillArgb = ColorVermelho;
    [RelayCommand] private void EdicaoPreenchLaranja() => EdicaoFillArgb = ColorLaranja;
    [RelayCommand] private void EdicaoPreenchAmarelo() => EdicaoFillArgb = ColorAmarelo;
    [RelayCommand] private void EdicaoPreenchVerde() => EdicaoFillArgb = ColorVerde;
    [RelayCommand] private void EdicaoPreenchAzul() => EdicaoFillArgb = ColorAzul;

    // Task D: mesmo formato acima, pra ESPESSURA (`EdicaoEspessuraPt`) — reusa as 3 constantes
    // `Espessura{Fina,Media,Grossa}` já definidas pra toolbar (Task B).
    [RelayCommand] private void EdicaoEspessuraFina() => EdicaoEspessuraPt = EspessuraFina;
    [RelayCommand] private void EdicaoEspessuraMedia() => EdicaoEspessuraPt = EspessuraMedia;
    [RelayCommand] private void EdicaoEspessuraGrossa() => EdicaoEspessuraPt = EspessuraGrossa;

    /// Confirmar de uma EDIÇÃO de forma (propósito EdicaoForma) — faz lift (Remove+Add) da `Rectangle`
    /// sendo editada pro rect FINAL da caixa, reusando `LiftSelectedAnnotationAsync` (mesmo pipeline de
    /// `ConfirmImageEditBoxAsync`/`MoveSelectedAnnotationAsync`/`SelectFillAsync`). SEM bytes/aspect-fit/
    /// cache — `sel with { ...4 escalares... }` já preserva ColorArgb/FillArgb/Author/Id por construção
    /// do record (só a geometria muda). Captura o Id ANTES de `ConfirmStampBox()` (que reseta
    /// `_stampBoxPurpose` de volta, mesma disciplina de `ConfirmImageEditBoxAsync`); `SelectedAnnotation`
    /// mudou de Id nesse meio-tempo (outra seleção) -> no-op, mesmo guard de `ConfirmImageEditBoxAsync`.
    private async Task ConfirmShapeEditBoxAsync()
    {
        string? editId = _editingAnnotationId;
        // Task D: captura o estilo de EDIÇÃO ANTES de `ConfirmStampBox()` (que reseta `StampPlacementPhase`
        // pra `None` -> `EmEdicaoDeForma` vira `false` já nesta chamada, mesma disciplina de `editId`
        // acima e de `_stampBoxImageBytes` em `ConfirmImageEditBoxAsync`).
        uint? corArgb = EdicaoCorArgb;
        uint? fillArgb = EdicaoFillArgb;
        double? espessuraPt = EdicaoEspessuraPt;
        if (ConfirmStampBox() is not { } placement) return;
        if (editId is null) return;
        if (SelectedAnnotation is not { Id: not null, Kind: AnnotationKind.Rectangle } sel || sel.Id != editId) return;

        // Task D: o lift agora grava o NOVO tamanho E o NOVO estilo de uma vez só (brief: "aplicadas no
        // Aplicar, junto com o novo tamanho") — `ColorArgb`/`FillArgb`/`StrokeWidthPt` vêm das
        // propriedades de edição acima, não mais preservados por construção do `with` (que os teria
        // herdado de `sel` se omitidos aqui).
        await LiftSelectedAnnotationAsync(sel with
        {
            LeftPt = placement.Rect.LeftPt, BottomPt = placement.Rect.BottomPt,
            RightPt = placement.Rect.RightPt, TopPt = placement.Rect.TopPt,
            ColorArgb = corArgb, FillArgb = fillArgb, StrokeWidthPt = espessuraPt,
        });
    }

    /// Aspect-fit da imagem DENTRO da caixa (sem distorção), centralizado — devolve o rect final em
    /// pontos de página, ou `null` se a caixa é degenerada ou a imagem não decodifica.
    private static PdfQuad? AspectFitWithinBox(byte[] imageBytes, PdfQuad box)
    {
        double boxW = box.RightPt - box.LeftPt, boxH = box.TopPt - box.BottomPt;
        ...
        double scale = Math.Min(boxW / iw, boxH / ih);
        double w = iw * scale, h = ih * scale;
        double cx = (box.LeftPt + box.RightPt) / 2, cy = (box.BottomPt + box.TopPt) / 2;
        return new PdfQuad(cx - w / 2, cy - h / 2, cx + w / 2, cy + h / 2);
    }

    /// Ids de todas as anotações `ImageStamp` no cache corrente (`AnnotationsByPage`) — base do set-diff
    /// que descobre qual Id é o RECÉM-criado após um add/lift (ReadAnnotations nunca devolve os bytes,
    /// mas devolve o Id).
    private HashSet<string> CurrentImageStampIds()
    {
        var ids = new HashSet<string>();
        foreach (var pageAnns in AnnotationsByPage)
            foreach (var a in pageAnns)
                if (a.Kind == AnnotationKind.ImageStamp && a.Id is { } id) ids.Add(id);
        return ids;
    }

    /// Após COLOCAR uma imagem nova: espera o cache de anotações refletir o add e mapeia o Id novo
    /// (o que não estava em `prevIds`) -> bytes.
    private async Task CacheNewImageStampBytesAsync(HashSet<string> prevIds, byte[] bytes)
    {
        await EnsureRotationCacheFreshAsync(); // renova AnnotationsByPage (anotações + rotações) do snapshot novo
        foreach (var pageAnns in AnnotationsByPage)
            foreach (var a in pageAnns)
    ...
    /// Após EDITAR (lift) uma imagem: o Id muda (Remove+Add). Remove o Id antigo do cache e mapeia o Id
    /// novo (o que não estava em `prevIds`) -> os MESMOS bytes.
    ...
    private async Task<bool> SignAndSaveAsAsync(SignDialogResult result, VisibleStampSpec? stamp, byte[] snapshotAtDialogOk)
    {
        ...
        byte[] signed;
        // StaTask (NÃO Task.Run): a assinatura toca a chave privada do token/A3, e o middleware do
    ...
    // Plano 21: prévia VISUAL da caixa (imagem da rubrica/imagem sendo posicionada) + rótulo do botão de
    // confirmar. `null`/"Assinar aqui" = carimbo padrão (prévia de texto). Setados quando a colocação
    // começa (BeginStampBoxPlacement), lidos por RefreshStampBoxOverlay pra empurrar pro PageViewModel.
    private ImageSource? _stampBoxPreviewImage;
    private string _stampBoxConfirmLabel = "Assinar aqui";
    // Plano 21 (Task 5): propósito da caixa corrente + estado da EDIÇÃO de imagem existente.
    private StampBoxPurpose _stampBoxPurpose = StampBoxPurpose.Assinatura;
    private byte[]? _stampBoxImageBytes;          // bytes da imagem sendo colocada/editada (não-null nos modos Imagem/EdicaoImagem)
    // Id da anotação sendo redimensionada/movida pela caixa de ajuste — GENÉRICO (Task 3, SDD
    // desenho-caixa-melhorias, renomeado de `_editingImageAnnotationId`): cobre tanto `EdicaoImagem`
    // (ImageStamp, precisa também de `_stampBoxImageBytes`) quanto `EdicaoForma` (Rectangle, reconstruível
    // só com este Id + `SelectedAnnotation`, sem bytes) — os 2 modos nunca coexistem (`_stampBoxPurpose`
    // decide qual está ativo), então 1 campo basta pros 2.
    private string? _editingAnnotationId;
    // Cache app-side dos bytes de imagem por Id de anotação — `ReadAnnotations` sempre devolve
    // ImageBytes null (ver AnnotationData.ImageBytes), então mover/redimensionar uma imagem JÁ colocada
    // (lift = Remove+Add) precisa dos bytes de algum lugar. Preenchido ao COLOCAR (PlaceImageStampFromBoxAsync)
    // e re-mapeado após cada lift. Vive só nesta sessão do documento — reabrir o PDF perde o cache (a
    // imagem continua no PDF, mas não editável até ser recolocada — aviso honesto na UI).
    private readonly Dictionary<string, byte[]> _imageStampBytes = new();

    /// Decodifica bytes de imagem numa `BitmapImage` congelada (thread-safe, sem cache de arquivo) pra
    /// prévia da caixa. `null` se os bytes não decodificam (nunca lança) — a caixa cai na prévia de texto.
    private static ImageSource? TryDecodeFrozenImage(byte[]? bytes)
    {
    ...
    // Task D (SDD desenho-caixa-revisao): estado de EDIÇÃO da caixa (contorno/preenchimento/espessura) —
    // inicializado em `BeginShapeEditBox` a partir do estilo CORRENTE da anotação, mudado pelos comandos
    // `Edicao{Contorno,Preench,Espessura}*` acima (SEM lift), e só gravado na anotação no Aplicar
    // (`ConfirmShapeEditBoxAsync`). `null` é uma opção de 1ª classe nos 2 primeiros (mesmo espírito de
    // `SelectedMarkupColorArgb`/`SelectedFillColorArgb`): "Sem cor" no contorno -> `ColorArgb = null` no
    // lift (Task A: sem cor de contorno = sem borda no motor).
    [ObservableProperty] private uint? edicaoCorArgb;
    [ObservableProperty] private uint? edicaoFillArgb;
    [ObservableProperty] private double? edicaoEspessuraPt;

    /// `true` só quando a caixa ajustável está em Adjusting E o propósito é `EdicaoForma` (não
    /// Assinatura/Imagem/EdicaoImagem) — a XAML (barrinha Aplicar/Cancelar) usa isto pra mostrar o grupo
    /// de contorno/preenchimento/espessura SÓ na edição de uma `Rectangle` já colocada. Computada (não
    /// `[ObservableProperty]`, `_stampBoxPurpose` é um campo privado comum, não um `partial`) —
    /// `OnStampPlacementPhaseChanged` abaixo notifica a UI toda vez que a FASE muda (Begin*/Cancel/Confirm
    /// sempre mudam a fase junto com `_stampBoxPurpose`, mesma garantia estrutural do comentário de
    /// `BeginShapeEditBox` acima sobre a ordem dos 2).
    public bool EmEdicaoDeForma => _stampBoxPurpose == StampBoxPurpose.EdicaoForma && StampPlacementPhase == StampPlacementPhase.Adjusting;
    ...
    partial void OnStampPlacementPhaseChanged(StampPlacementPhase value)
    {
        RefreshStampBoxOverlay();
        // Task D: único ponto de notificação de `EmEdicaoDeForma` — toda transição de fase (Begin*/
        // Cancel/Confirm) passa por aqui, e é sempre acompanhada de um `_stampBoxPurpose` já atualizado
        // (ver doc XML da propriedade).
        OnPropertyChanged(nameof(EmEdicaoDeForma));
    }
    ...
    public void BeginStampBoxPlacement(int pageIndex, PdfPoint startPt, string certificateCn,
        ImageSource? previewImage = null, string confirmLabel = "Assinar aqui",
        StampBoxPurpose purpose = StampBoxPurpose.Assinatura)
    {
        // Plano 21: aceita tanto assinatura (SignatureStamp) quanto imagem (ImageStamp) — a máquina de
        // desenho/ajuste é a mesma; só o CONFIRMAR difere (ver ConfirmStampBoxAsync).
        if (ActiveTool != AnnotationTool.SignatureStamp && ActiveTool != AnnotationTool.ImageStamp) return;
        if (pageIndex < 0 || pageIndex >= Pages.Count) return;
        ...
        // Plano 21: prévia visual (rubrica/imagem) + rótulo + propósito.
        _stampBoxPreviewImage = previewImage;
        _stampBoxConfirmLabel = confirmLabel;
        _stampBoxPurpose = purpose;
        _boxXa = _boxXb = x;
        _boxYa = _boxYb = y;
        StampBoxNotice = null;
        RecomputeStampBoxRect();
    ...
    private void SetDefaultBoxRawAtAnchor()
    {
        if (StampBoxPageIndex < 0 || StampBoxPageIndex >= Pages.Count) return;
        var page = Pages[StampBoxPageIndex];
        double w = Math.Min(DefaultStampWidthPt, page.WidthPt);
    ...
    public void CancelStampBox(bool dueToDocumentMutation = false)
    {
        ...
        StampBoxDateLabel = null;
        StampBoxNotice = null;
        // Plano 21: reseta prévia/rótulo/propósito/estado de imagem da caixa.
        _stampBoxPreviewImage = null; _stampBoxConfirmLabel = "Assinar aqui";
        _stampBoxPurpose = StampBoxPurpose.Assinatura; _stampBoxImageBytes = null; _editingAnnotationId = null;
        _boxXa = _boxXb = _boxYa = _boxYb = 0;
        StampBoxRect = default;
        // Task D: descarta o estilo de EDIÇÃO junto com a geometria (brief: "Cancelar descarta tudo") —
        // inócuo pros outros 3 propósitos (Assinatura/Imagem/EdicaoImagem nunca leem estas 3 propriedades).
        EdicaoCorArgb = null; EdicaoFillArgb = null; EdicaoEspessuraPt = null;
        ...
        // Plano 21: cancelar a colocação de uma imagem também sai da ferramenta e limpa os bytes pendentes
        // (mesma disciplina do carimbo acima). Edição de imagem (EdicaoImagem) roda com ActiveTool == None
        // — nada de ferramenta pra desativar, só a caixa foi limpa acima.
        if (wasActive && ActiveTool == AnnotationTool.ImageStamp)
        {
            ActiveTool = AnnotationTool.None;
            _pendingStampBytes = null;
        }
    ...
    public StampBoxPlacement? ConfirmStampBox()
    {
        ...
        StampBoxDateLabel = null;
        StampBoxNotice = null;
        // Plano 21 (+ Task 3, SDD desenho-caixa-melhorias): reseta prévia/rótulo/propósito. NÃO limpa
        // `_stampBoxImageBytes`/`_editingAnnotationId` aqui: os confirmadores de imagem/forma
        // (`ConfirmImageBoxAsync`/`ConfirmImageEditBoxAsync`/`ConfirmShapeEditBoxAsync`) CAPTURAM esses
        // valores ANTES de chamar `ConfirmStampBox()` e ainda precisam deles logo em seguida — quem
        // reseta o resto do estado é o CancelStampBox (o caminho de sucesso não passa por lá, mas os
        // campos são re-setados no próximo Begin*).
        _stampBoxPreviewImage = null; _stampBoxConfirmLabel = "Assinar aqui";
        _stampBoxPurpose = StampBoxPurpose.Assinatura;
        _boxXa = _boxXb = _boxYa = _boxYb = 0;
        StampBoxRect = default;
        return result;
    }
    ...
    private void RefreshStampBoxOverlay()
    {
        ...
        // Plano 21: prévia de IMAGEM (rubrica/imagem) tem prioridade; senão, prévia de TEXTO (carimbo
        // padrão). Os 2 bools são exclusivos — a View mostra um OU outro.
        page.StampBoxPreviewImage = _stampBoxPreviewImage;
        page.HasStampBoxPreviewImage = _stampBoxPreviewImage is not null;
        page.HasStampBoxPreviewText = _stampBoxPreviewImage is null;
        page.StampBoxConfirmLabel = _stampBoxConfirmLabel;
