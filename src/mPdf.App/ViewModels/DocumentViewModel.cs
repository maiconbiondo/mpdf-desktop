...
public sealed partial class DocumentViewModel : ObservableObject, IDisposable
{
    ...
    /// `AnnotationKind.Rectangle`). Diferente de `SelectedMarkupColorArgb` acima (sempre um `uint` — 3
    /// cores fixas, nenhuma noção de "nenhuma"), este é `uint?` porque "sem preenchimento" É uma opção de
    /// 1ª classe da UI (botão "Nenhum" — ver `SelectFillNenhum` abaixo), não apenas a ausência de escolha.
