...
/// redimensionar) serve 3 casos: `Assinatura` (carimbo padrão OU rubrica — confirmar dispara o motor de
/// assinatura), `Imagem` (colocar uma imagem nova como anotação — confirmar adiciona a anotação) e
/// `EdicaoImagem` (redimensionar/mover uma imagem JÁ colocada — confirmar faz lift Remove+Add). O
/// propósito decide só o que o CONFIRMAR faz (`ConfirmStampBoxAsync`) e o rótulo do botão; toda a
/// geometria é idêntica.
...
public sealed partial class DocumentViewModel : ObservableObject, IDisposable
{
    ...
    public async Task BeginImageBoxPlacementAsync(int pageIndex, PdfPoint startPt)
    {
        ...
        _editingImageAnnotationId = null;
    ...
    {
        string? editId = _editingImageAnnotationId;
        byte[]? bytes = _stampBoxImageBytes;
    ...
    private string? _editingImageAnnotationId;    // Id da anotação sendo redimensionada (só no modo EdicaoImagem)
    ...
    public void CancelStampBox(bool dueToDocumentMutation = false)
    {
        ...
        _stampBoxPurpose = StampBoxPurpose.Assinatura; _stampBoxImageBytes = null; _editingImageAnnotationId = null;
    ...
    public StampBoxPlacement? ConfirmStampBox()
    {
        ...
        StampBoxDateLabel = null;
        StampBoxNotice = null;
        // Plano 21: reseta prévia/rótulo/propósito/estado de imagem. NÃO limpa `_stampBoxImageBytes`/
        // `_editingImageAnnotationId` aqui: os confirmadores de imagem (`ConfirmImageBoxAsync`/
        // `ConfirmImageEditBoxAsync`) CAPTURAM esses valores ANTES de chamar `ConfirmStampBox()` e ainda
        // precisam deles logo em seguida — quem reseta o resto do estado de imagem é o CancelStampBox (o
        // caminho de sucesso não passa por lá, mas os campos são re-setados no próximo Begin*).
        _stampBoxPreviewImage = null; _stampBoxConfirmLabel = "Assinar aqui";
