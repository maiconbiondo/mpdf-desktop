...
public sealed partial class DocumentViewModel : ObservableObject, IDisposable
{
    ...
    private async Task Sign()
    {
        ...
        // Plano 21: oferece "Minha rubrica" só quando há uma salva no config.
        var result = _signDialog.PromptForSignature(certificates, allowDocMdp: !hasSignatures, hasRubrica: _config.TemRubrica);
        ...
        {
            // Plano 21: se o usuário escolheu "Minha rubrica", carrega os bytes AGORA (coerente com o
            // snapshot capturado). Rubrica sumiu entre abrir o diálogo e aqui (removida em Configurações
            // noutra janela) -> aborta com aviso, sem entrar em modo de colocação.
            byte[]? rubricaBytes = null;
            if (result.UseRubrica)
            {
                rubricaBytes = _config.LerRubrica();
                if (rubricaBytes is null) { _notifyError("Rubrica não encontrada — defina-a em Configurações."); return; }
            }
            ...
            _pendingSignPlacement = new PendingSignPlacement(result, snapshotAtDialogOk, didForcedSave, rubricaBytes);
