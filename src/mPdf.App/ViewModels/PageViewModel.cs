...
public sealed partial class PageViewModel : ObservableObject
{
    ...
    /// Plano 21: rótulo do botão de confirmar do cartão flutuante — "Assinar aqui" (carimbo/rubrica) ou
    /// "Inserir aqui" (imagem). Empurrado por RefreshStampBoxOverlay conforme o propósito da caixa.
    [ObservableProperty] private string stampBoxConfirmLabel = "Assinar aqui";
    /// Plano 21: prévia da IMAGEM dentro da caixa (rubrica ou imagem sendo posicionada) — null no carimbo
    /// padrão (aí a prévia é o texto `StampBoxPreviewText`). `HasStampBoxPreviewImage`/`HasStampBoxPreviewText`
    /// são exclusivos (a View mostra um OU outro). Empurrados por RefreshStampBoxOverlay.
    [ObservableProperty] private ImageSource? stampBoxPreviewImage;
    [ObservableProperty] private bool hasStampBoxPreviewImage;
    [ObservableProperty] private bool hasStampBoxPreviewText = true;
