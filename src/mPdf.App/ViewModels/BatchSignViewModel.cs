...
public sealed partial class BatchSignViewModel : ObservableObject
{
    ...
    private async Task Start()
    {
        ...
        for (int i = 0; i < toProcess.Count; i++)
        {
            ...
            // StaTask (NÃO Task.Run): SignOneFile chama o motor de assinatura, que toca a chave do
            // token/A3 — o diálogo de PIN do middleware (ex.: SafeSign) só abre em thread STA. Ver StaTask.
            var result = await StaTask.Run(() => SignOneFile(path, cert, reasonValue, locationValue, placeStampValue));
