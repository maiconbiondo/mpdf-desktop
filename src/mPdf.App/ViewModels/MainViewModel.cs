...
public sealed partial class MainViewModel : ObservableObject
{
    ...
    /// Versão do app (ex.: "2.11.0"), mostrada ao lado do nome "mPDF" na barra de título. Lida uma
    /// vez, sem rede — mesma fonte de SobreViewModel.VersaoAtual (UpdateService.CurrentVersionText(),
    /// derivada do <Version> do csproj via AssemblyVersion).
    public string VersaoApp { get; } = UpdateService.CurrentVersionText();
        ...
        IConfiguracoesDialogService? configuracoesDialog = null)
    {
        ...
        // Plano 23: estado inicial do tema lido do config PERSISTIDO -- setado no CAMPO (não na
        // propriedade), é o estado inicial, não uma troca de UI (nada de aplicar/gravar na construção).
        temaEscuro = _config.ThemeMode == mPdf.Documents.ThemeMode.Escuro;
        RefreshStampItems();
    }

    // ==== Plano 23: alternância de tema na barra superior (movida de Configurações) ==================
    //
    // Bota o toggle claro/escuro DIRETO na barra de comandos, com ícone próprio (sol quando está escuro =
    // "vá pro claro"; lua quando está claro = "vá pro escuro"). Mesmo mecanismo do que estava em
    // Configurações: grava em AppConfig.ThemeMode e aplica AO VIVO via ThemeService (troca o dicionário de
    // tokens; os {DynamicResource Cor.*} re-pintam a UI sem reiniciar). Escuro continua o PADRÃO de fábrica.
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(TemaGlifo))]
    [NotifyPropertyChangedFor(nameof(TemaTooltip))]
    private bool temaEscuro;

    /// Glifo do botão: SOL quando está escuro (clique = ir pro claro), LUA quando está claro (clique = ir
    /// pro escuro). Ícone = a AÇÃO/destino, casado com o tooltip. Ver Icons/Ph (sun/moon, Phosphor 2.1).
    public string TemaGlifo => TemaEscuro ? Icons.Ph.Glyph("sun") : Icons.Ph.Glyph("moon");
    public string TemaTooltip => TemaEscuro ? "Mudar para tema claro" : "Mudar para tema escuro";

    [RelayCommand]
    private void ToggleTema()
    {
        TemaEscuro = !TemaEscuro;
        var modo = TemaEscuro ? mPdf.Documents.ThemeMode.Escuro : mPdf.Documents.ThemeMode.Claro;
        _config.ThemeMode = modo;
        // Aplica ao vivo (troca o dicionário de tokens). Guarda de teste headless: sem `Application`
        // ativa (`Application.Current == null`), só persiste — a troca visual não faz sentido nem é
        // possível sem app; em produção `Application.Current` nunca é null quando o comando roda.
        if (System.Windows.Application.Current is not null)
            Services.ThemeService.AplicarNoApp(modo);
    }
    ...
    // MESMO `ConfirmCloseAll` que fechar a janela já usa, iniciar o instalador via `Process.Start`, e
    // encerrar via `Application.Current.Shutdown()`) e delega toda a lógica pro VM/diálogo (ver
    // `ConfiguracoesViewModel`, testável sem esta VM/sem janela nenhuma). SEMPRE habilitado, mesmo
    // espírito de `Sobre()`.
    //
    // Plano 18 (Task 2): o launch agora é SILENCIOSO — `SilentUpdateInstaller.BuildStartInfo` acrescenta
    // `/VERYSILENT /SUPPRESSMSGBOXES /NORESTART` ao MESMO caminho já verificado por SHA-256 (a guarda "só
    // o VerifiedUpdateFile chega ao Process.Start" continua intacta — só muda os argumentos, não a escolha
    // do caminho). O UAC (1 clique) permanece: `UseShellExecute=true` faz o manifesto de admin do
    // instalador disparar a elevação. O relaunch do app pós-instalação é da seção `[Run]` do `mpdf.iss`
    // (`runasoriginaluser`), por isso `/NORESTART`.
    [RelayCommand]
    private void Configuracoes()
    {
        var vm = new ConfiguracoesViewModel(
            confirmCloseAllDocuments: ConfirmCloseAll,
            startInstaller: path => Process.Start(SilentUpdateInstaller.BuildStartInfo(path)),
