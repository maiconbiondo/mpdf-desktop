...
public sealed partial class MainViewModel : ObservableObject
{
    ...
    partial void OnSelectedDocumentChanged(DocumentViewModel? oldValue, DocumentViewModel? newValue)
    {
        ...
        // Fix painel recolhível: reabre o painel de miniaturas quando se sai do estado VAZIO (nenhuma
        // aba) para um documento aberto — comportamento herdado ("já vem visível"). Trocar de aba com
        // OUTRAS abas já abertas não mexe aqui (preserva um recolher explícito do usuário entre abas).
        if (oldValue is null && newValue is not null) ThumbnailsVisible = true;
    }
    ...
    // Default TRUE: mantém o comportamento herdado (painel já vem visível quando um documento abre);
    // sem documento a Visibility do painel não depende disto (SelectedDocument==null já colapsa).
    [ObservableProperty]
    private bool thumbnailsVisible = true;
