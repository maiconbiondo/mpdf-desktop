...
public sealed partial class MainViewModel : ObservableObject
{
    ...
    private void Configuracoes()
    {
        var vm = new ConfiguracoesViewModel(
            ...
            // Plano 21: seletor+validação da rubrica — mesma validação de imagem de ToggleImageTool
            // (formato/pixels/CMYK), notificando o erro por _notifyError; devolve bytes prontos ou null.
            escolherRubricaImagem: EscolherRubricaImagem);
        _configuracoesDialog.ShowConfiguracoesDialog(vm);
    }

    /// Plano 21: abre o seletor de imagem e valida como rubrica (mesmos critérios de `ToggleImageTool` em
    /// DocumentViewModel: formato JPG/PNG, ≤50MP, não CMYK), notificando o erro em pt-BR e devolvendo
    /// `null` se cancelado ou rejeitado. Consumido pelo `ConfiguracoesViewModel` (que salva no config).
    private byte[]? EscolherRubricaImagem()
    {
